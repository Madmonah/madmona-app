...
export default function TeamPage() {
  ...
  if (!ready) return <div dir="rtl" style={{ minHeight: '100vh', display: 'grid', placeItems: 'center', background: 'linear-gradient(160deg,#1F6F5F,#2FA084)', color: '#fff', fontFamily: 'system-ui' }}>لحظة…</div>

  if (!uid) return (
    <div dir="rtl" style={{ minHeight: '100vh', display: 'grid', placeItems: 'center', background: 'linear-gradient(160deg,#1F6F5F,#2FA084)', color: '#fff', fontFamily: 'system-ui', textAlign: 'center', padding: 20 }}>
      <div>
  ...
  if (!active) return (
    <div dir="rtl" style={{ height: '100vh', display: 'flex', flexDirection: 'column', background: '#FAFAF7', fontFamily: "'Cairo', system-ui, sans-serif" }}>
      ...
      <header style={{ background: 'linear-gradient(135deg,#1F6F5F 0%,#2FA084 100%)', color: '#fff', padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 10, boxShadow: '0 2px 14px rgba(31,111,95,.25)' }}>
        <div style={{ flex: 1, fontWeight: 800, fontSize: 18 }}>👥 محادثاتك</div>
        <button onClick={startDM} title="اختار شخص من جهات الاتصال" style={{ background: 'rgba(255,255,255,.18)', color: '#fff', border: 'none', borderRadius: 16, padding: '6px 10px', fontWeight: 700, cursor: 'pointer', fontSize: 13, whiteSpace: 'nowrap' }}>💬 خاصة</button>
        <button onClick={createRoom} style={{ background: '#F4C430', color: '#3a2e05', border: 'none', borderRadius: 16, padding: '6px 10px', fontWeight: 700, cursor: 'pointer', fontSize: 13, whiteSpace: 'nowrap' }}>+ جروب</button>
      </header>
      <div style={{ flex: 1, overflowY: 'auto', padding: 10 }}>
        ...
        {rooms.map((r) => {
          ...
          return (
            <button key={r.id} onClick={() => openRoom(r)} style={{ display: 'flex', width: '100%', textAlign: 'right', alignItems: 'center', gap: 12, background: '#fff', border: '1px solid #f0efe9', borderRadius: 14, padding: 12, marginBottom: 8, cursor: 'pointer', boxShadow: '0 1px 4px rgba(0,0,0,.06)' }}>
              <div style={{ width: 46, height: 46, borderRadius: '50%', background: isDirect ? 'linear-gradient(135deg,#2FA084,#1F6F5F)' : 'linear-gradient(135deg,#F4C430,#6FCF97)', color: '#fff', display: 'grid', placeItems: 'center', fontSize: 18, fontWeight: 800 }}>{(title || 'م').trim()[0]}</div>
              <div style={{ flex: 1 }}><div style={{ fontWeight: 700, color: '#111' }}>{title}</div><div style={{ fontSize: 12, color: '#888' }}>{isDirect ? 'محادثة خاصة' : 'جروب'}</div></div>
            </button>
          )
        })}
      </div>
  ...
  return (
    <div dir="rtl" style={{ height: '100vh', display: 'flex', flexDirection: 'column', background: '#FAFAF7', fontFamily: "'Cairo', system-ui, sans-serif" }}>
      <style>{"@import url('https://fonts.googleapis.com/css2?family=Cairo:wght@400;500;600;700;800&display=swap');"}</style>
      <header style={{ background: 'linear-gradient(135deg,#1F6F5F 0%,#2FA084 100%)', color: '#fff', padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 10, boxShadow: '0 2px 14px rgba(31,111,95,.25)', zIndex: 2 }}>
        ...
        <button onClick={summonMaridInRoom} disabled={busy} title="استدعِ المارد" style={{ background: '#F4C430', color: '#3a2e05', border: 'none', borderRadius: 14, padding: '5px 10px', fontSize: 12, fontWeight: 700, cursor: 'pointer', whiteSpace: 'nowrap', opacity: busy ? 0.6 : 1 }}>🧞 المارد</button>
        <button onClick={() => setGallery(true)} title="ميديا المحادثة" style={{ background: 'rgba(255,255,255,.18)', color: '#fff', border: 'none', borderRadius: 14, padding: '5px 9px', fontSize: 13, cursor: 'pointer', whiteSpace: 'nowrap' }}>🖼️</button>
        {active.kind !== 'direct' && (
      ...
      <div ref={scrollRef} style={{ flex: 1, overflowY: 'auto', padding: '12px 10px', background: 'radial-gradient(circle at 20% 10%, #f2efe6 0, #FAFAF7 60%)' }}>
        {messages.map((m) => {
          ...
          return (
            <div key={m.id} style={{ display: 'flex', justifyContent: mine ? 'flex-start' : 'flex-end', marginBottom: 8 }}>
              <div style={{ maxWidth: '82%', background: mine ? 'linear-gradient(135deg,#DFF3E4,#CFEeDA)' : (marid ? '#FFF7E0' : '#fff'), padding: '9px 13px', borderRadius: 16, borderTopRightRadius: mine ? 16 : 4, borderTopLeftRadius: mine ? 4 : 16, boxShadow: '0 1px 5px rgba(0,0,0,.08)', whiteSpace: 'pre-wrap', wordBreak: 'break-word', fontSize: 15, lineHeight: 1.65 }}>
                {!mine && <div style={{ fontSize: 11, fontWeight: 700, color: marid ? '#b8860b' : '#2FA084', marginBottom: 2 }}>{marid ? '🧞 المارد' : (m.sender_name || 'عضو')}</div>}
                ...
                {m.media_url && m.kind !== 'image' && m.kind !== 'video' && m.kind !== 'audio' && (
                  <a href={m.media_url} target="_blank" rel="noreferrer" style={{ display: 'inline-block', marginBottom: m.body ? 4 : 0, color: '#1F6F5F', textDecoration: 'underline', fontSize: 14 }}>📎 ملف مرفق</a>
                )}
                {m.media_url && (
                  <button onClick={() => setForwardMsg(m)} title="تحويل لمحادثة تانية" style={{ display: 'block', marginBottom: 4, background: 'none', border: 'none', color: '#2FA084', cursor: 'pointer', fontSize: 12, padding: 0 }}>↗️ تحويل</button>
                )}
                {m.body}
                {m.body && (
                  <button onClick={() => addTask(m)} title="حوّل الرسالة لمهمة" style={{ display: 'block', marginTop: 4, background: 'none', border: 'none', color: '#1F6F5F', cursor: 'pointer', fontSize: 11, padding: 0, fontWeight: 700 }}>➕ حوّل لمهمة</button>
                )}
        ...
        {(busy || maridThinking) && <div style={{ textAlign: 'end', color: '#b8860b', fontSize: 13, padding: '4px 10px', fontWeight: 600 }}>🧞 المارد بيفكر…</div>}
      </div>
      <div style={{ display: 'flex', gap: 8, padding: 10, background: '#fff', borderTop: '1px solid #eee', boxShadow: '0 -2px 12px rgba(0,0,0,.04)', alignItems: 'center' }}>
        <input ref={fileRef} type="file" accept="image/*,video/*" style={{ display: 'none' }} onChange={(e) => { const f = e.target.files?.[0]; if (f) sendMedia(f) }} />
        <button onClick={() => fileRef.current?.click()} disabled={busy} title="أرفق صورة أو فيديو" style={{ background: '#fff', color: '#1F6F5F', border: '1px solid #ddd', borderRadius: '50%', width: 44, height: 44, fontSize: 18, cursor: 'pointer', flexShrink: 0, opacity: busy ? 0.6 : 1 }}>📎</button>
        <button onClick={toggleRec} disabled={busy} title={recording ? 'إيقاف وإرسال' : 'رسالة صوتية'} style={{ background: recording ? '#c0392b' : '#fff', color: recording ? '#fff' : '#1F6F5F', border: '1px solid #ddd', borderRadius: '50%', width: 44, height: 44, fontSize: 18, cursor: 'pointer', flexShrink: 0, opacity: busy ? 0.6 : 1 }}>{recording ? '⏹️' : '🎤'}</button>
        <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && sendMsg()} placeholder="اكتب رسالة…" style={{ flex: 1, padding: '12px 14px', border: '1px solid #ddd', borderRadius: 22, fontSize: 15, outline: 'none', fontFamily: 'inherit' }} />
        <button onClick={sendMsg} disabled={busy} style={{ background: 'linear-gradient(135deg,#F4C430,#2FA084,#1F6F5F)', color: '#fff', border: 'none', borderRadius: '50%', width: 48, height: 48, fontSize: 18, cursor: 'pointer', opacity: busy ? 0.6 : 1, flexShrink: 0 }}>➤</button>
      </div>
    </div>
  )
}
