...
## ⚠️ تحديث 2026-07-22 (متأخر) — قنوات الواتساب الاتنين واقعة

أثناء محاولة تشغيل ping-الواتساب-لصاحب-الإعلان-من-غير-حساب لقيت:
1. **بوت المارد (Baileys):** الكود **مش موجود** لا على GitHub (كل الريبوهات فاضية،
   `Madmona-whatsapp` فاضي) ولا على الجهاز (دوّرت C:/D:/E: كلهم، مفيش WSL). يعني
   مش قادر أعدّله ولا أنشره.
2. **WhatsApp Cloud API:** موجود helper كامل (`src/lib/whatsapp.ts`) بيبعت مباشرة،
   **بس التوكن منتهي** — Meta بترجّع `code 190 / user logged out`. ده user token
   مؤقت مش System User token دائم (زي ما الكومنت في الملف نفسه بيقول المفروض).
   + أي إرسال بارد (>24 ساعة) محتاج **template معتمد** — ومفيش أي sendTemplate
   مستخدم في الكود، فمفيش template معروف.

### الحل اللي اتعمل عشان الفيتشر يشتغل دلوقتي من غير القنوات الواقعة:
- **صاحب إعلان عنده حساب:** روم + بوش. ✅ شغّال.
- **صاحب إعلان من غير حساب:** بنسجّل + نحطّه في طابور المارد (لما يرجع) + **نبعت
  إيميل تنبيه للأدمن** (Resend شغّال، دومين متحقّق) فيه رقم صاحب الإعلان + الإعلان،
  عشان الأدمن يوصله يدوي. **مفيش استفسار بيضيع.**

### علشان يشتغل واتساب-لصاحب-الإعلان تلقائي (خطوة المالك — 5 دقايق):
جدّد `WHATSAPP_ACCESS_TOKEN` في Vercel بـ **System User token دائم** من Meta Business.
وبعدها نضيف sendTemplate بـtemplate معتمد للرسالة الباردة (أو نصلّح بوت المارد).
