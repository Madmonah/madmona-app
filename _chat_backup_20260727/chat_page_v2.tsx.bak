...
export default function ChatHub() {
  ...
  return (
    <div dir="rtl" style={{ height: '100vh', display: 'flex', flexDirection: 'column', background: '#FAFAF7', fontFamily: "'Cairo', system-ui, sans-serif" }}>
      ...
      <header style={{ background: 'linear-gradient(135deg,#1F6F5F 0%,#2FA084 100%)', color: '#fff', padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 10, boxShadow: '0 2px 14px rgba(31,111,95,.25)' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        ...
        <div style={{ padding: '12px 16px 4px', fontSize: 12, fontWeight: 700, color: '#1F6F5F' }}>الجروبات</div>
        {rooms.length === 0 ? (
          ...
          rooms.map((r) => (
            <Link key={r.id} href={`/team?room=${r.id}`} style={rowStyle}>
              <div style={{ ...avatarStyle, background: 'linear-gradient(135deg,#F4C430,#6FCF97)', color: '#fff', display: 'grid', placeItems: 'center', fontSize: 18, fontWeight: 800 }}>{(r.name || '؟').trim().charAt(0)}</div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8 }}>
