...
// ⚠️ حساباتنا الداخلية والتجريبية — «مضمونة ماركت» · «صيدلية مضمونة» ·
//    «مضمونة التجريبي». مش موردين حقيقيين، وأرقامهم غالبًا مش على واتساب
//    أصلاً. جروب معاهم = جروب مع نفسنا.
const OUR_PHONES = ['201002229982', '201500100100', '201500100200', '201000000090', ...TEAM]
...
function introFor(name, listings) {
  // ⚠️ مورّد لسه ماعندهوش إعلانات — «عندك ٠ إعلان» رسالة محرجة
  //    وبتوحي إننا مش متابعين. نعرض نعملهاله بدل ما نعدّها عليه.
  const firstLine =
    listings > 0
      ? `• عندك ${listings} إعلان على مضمونة\n`
      : `• لسه ماضفناش إعلاناتك — ابعتلي صورهم وأسعارهم وأنا أعملّك الصفحات جاهزة\n`

  return (
    `أهلاً ${name} 👋\n\n` +
    `أنا *المارد* من فريق مضمونة.\n\n` +
