...
;(async () => {
  ...
  for (const s of SLUGS) {
    try {
      await p.goto(`https://www.madmonacairo.com/s/${s}`, { waitUntil: 'domcontentloaded', timeout: 60000 })
      // لازم تعدّي نقطة تفتيش فيرسل الأول
      await p.waitForFunction(() => document.body.innerText.length > 400, { timeout: 120000 }).catch(() => {})
