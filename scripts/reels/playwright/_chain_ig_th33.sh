...
S=biz-nike-trunk
restart_social(){ powershell -NoProfile -Command "Get-CimInstance Win32_Process -Filter \"name='chrome.exe'\" | ?{ \IGP posters/post-instagram-reel.cjs.CommandLine -like '*chrome-social-profile*' } | %{ Stop-Process -Id \IGP posters/post-instagram-reel.cjs.ProcessId -Force -ErrorAction SilentlyContinue }"; sleep 3; powershell -NoProfile -ExecutionPolicy Bypass -File launch-chrome-social.ps1 >/dev/null 2>&1; for i in $(seq 1 30); do node -e "fetch('http://127.0.0.1:9223/json/version').then(()=>process.exit(0)).catch(()=>process.exit(1))" && break; sleep 2; done; sleep 20; }
