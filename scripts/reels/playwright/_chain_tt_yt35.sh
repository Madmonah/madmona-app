...
node _yt_short.cjs $S > _yt35.txt 2>&1; echo "YT_EXIT $?"; tail -5 _yt35.txt
YID=$(grep -o 'shorts/[A-Za-z0-9_-]\{11\}\|youtu.be/[A-Za-z0-9_-]\{11\}\|"id":"[A-Za-z0-9_-]\{11\}"' _yt35.txt | tail -1 | grep -o '[A-Za-z0-9_-]\{11\}$\|[A-Za-z0-9_-]\{11\}"$' | tr -d '"'); echo "YT_ID $YID"
if [ -n "$YID" ]; then node _yt_comment.cjs $YID $S; echo "YT_COMMENT $?"; node _yt_comment_text.cjs $YID "زبونك يعرف يوصف محلك في جملة واحدة؟ اكتبها في كومنت 👇"; echo "YT_Q $?"; fi
echo DONE_TTYT
