...
(async () => {
  ...
  await p.goto('https://www.madmonacairo.com/start?utm_source=e2e&utm_medium=funnel', { waitUntil: 'networkidle', timeout: 60000 });
  await p.waitForSelector('form', { timeout: 30000 });
  await p.fill('input[placeholder*="عيادة د"]', 'عيادة اختبار ستارت (تتمسح)');
  await p.fill('input[inputmode="tel"]', '01999888792');
  ...
  const wh = { event: 'message.received', sessionId: '201114621551', data: { from: '201999888792@c.us', to: '201114621551@c.us', body: code, fromMe: false, id: 'e2e-' + Date.now(), timestamp: Math.floor(Date.now() / 1000), type: 'chat' } };
