// 💬 كومنت بنص حر (سؤال مثبّت) — argv: <videoId> "<text>"
...
const [, , vid, freeText] = process.argv; const slug = ''
let txt = String(freeText || '').trim()
...
;(async () => {
  ...
  await page.evaluate((t) => { window.__t = t }, txt.slice(0, 20)); const already = await page.evaluate(() => document.body.innerText.includes(window.__t))
