...
const AVG_PRICE = 7876630      // المتوسط الحسابي لسعر الوحدة من ٩٤ عرض معلن سعره
const DOWN_PCT = 5             // النسبة الشائعة للمقدم: ٢٨ مشروع من ٥٨ معلنين النسبة
const YEARS = 8                // مدة التقسيط الشائعة: ١٨ مشروع من ٦٧ معلنين المدة
const MONTHS = YEARS * 12
const REST = Math.round(AVG_PRICE * (1 - DOWN_PCT / 100))
...
const S = [
  ...
  { len: 4.6, kind: 'stat', big: ar(AVG_PRICE), sub: 'متوسط سعر الوحدة', note: 'من ٩٤ عرض معلن سعره' },
  { len: 5.0, kind: 'two', a: '٥٪', aSub: 'المقدم الشائع — في ٢٨ مشروع', b: '٨ سنين', bSub: 'التقسيط الشائع — في ١٨ مشروع' },

  // الحسبة قدام عينه — ده اللي بيخلي القرار ممكن
  { len: 7.0, kind: 'calc', title: 'يبقى القسط بكام؟',
    lines: [
      ar(AVG_PRICE) + ' − ٥٪ مقدم = ' + ar(REST),
      ar(REST) + ' ÷ ٩٦ شهر',
    ],
...
const vo = [
  ...
  'متوسط سعر الوحدة عندنا سبعة مليون وتمنمية وستة وسبعين ألف جنيه، من أربعة وتسعين عرض معلن سعره.',
  'والمقدم الشائع خمسة في المية، ومدة التقسيط الشائعة تمن سنين.',
  'يبقى الحسبة كده: سبعة مليون وتمنمية وستة وسبعين ألف، ناقص خمسة في المية مقدم، على ستة وتسعين شهر… سبعة وسبعين ألف وتسعمية وستة وأربعين جنيه في الشهر.',
