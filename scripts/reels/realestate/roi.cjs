...
const PRICE = 7876630          // المتوسط الحسابي لسعر الوحدة (٩٤ عرض)
const INSTALLMENT = 77946          // من فيديو الفلوس: (المتوسط − ٥٪) ÷ ٩٦ شهر
const Y_PUB = 8.3                  // عائد القاهرة المنشور
const Y_OURS = 4.5                 // متوسط إيجار ٢٨٬٠٠٠ × ١٢ ÷ متوسط بيع ٧٬٤٣٣٬٣٣٣ (١٠ إعلانات بالقاهرة)
const rentMo = y => Math.round(PRICE * (y / 100) / 12)
const R_PUB = rentMo(Y_PUB)        // 54,480
const R_OURS = rentMo(Y_OURS)      // 29,537
...
const S = [
  { len: 4.0, kind: 'hook', big: 'اشتريت بسبعة مليون ونص', sub: 'وأجّرتها… تكسب ولا تخسر؟' },

  { len: 4.6, kind: 'stat', big: ar(INSTALLMENT), sub: 'القسط الشهري', note: 'متوسط السعر ٧٫٨٧ مليون · مقدم ٥٪ · ٨ سنين' },

  { len: 7.0, kind: 'calc', title: 'طيب الإيجار هيجيب كام؟',
    lines: ['العائد الإيجاري في القاهرة ٨٫٣٪ سنويًا', 'متوسط السعر × ٨٫٣٪ ÷ ١٢ شهر'],
  ...
  { len: 7.0, kind: 'calc', title: 'وبأرقام إعلاناتنا إحنا؟',
    lines: ['متوسط إيجار شقة في القاهرة ٢٨ ألف — من ٧ إعلانات', 'متوسط بيع شقة في القاهرة ٧٫٤ مليون — من ٣ إعلانات'],
    big: '٤٫٥٪', bigSub: 'عائد سنوي — أقل من المنشور' },
...
const body = sc.map(s => {
  ...
  if (s.kind === 'proj') {
    ...
    inner = `<div class="mid">
      <div class="ctitle" style="animation-delay:${d(.15)}s">${esc(s.title)}</div>
      ${s.rows.map((r, i) => `<div class="row" style="animation-delay:${d(.7 + i * .85)}s"><b>${ar(i + 1)}</b><span>${esc(r)}</span></div>`).join('')}
    </div>`
...
const vo = [
  'اشتريت شقة بسبعة مليون ونص… وأجّرتها. تكسب ولا تخسر؟',
  'القسط الشهري سبعة وسبعين ألف وتسعمية وستة وأربعين جنيه.',
  'طيب الإيجار هيجيب كام؟ العائد الإيجاري في القاهرة تمنية وتلاتة من عشرة في المية سنويًا… يعني أربعة وخمسين ألف وأربعمية وتمانين جنيه في الشهر.',
  'يبقى الفرق تلاتة وعشرين ألف وأربعمية وستة وستين جنيه بتدفعهم من جيبك كل شهر.',
  'وبأرقام إعلاناتنا إحنا؟ متوسط إيجار الشقة في القاهرة تمنية وعشرين ألف، ومتوسط البيع سبعة مليون وأربعمية وتلاتة وتلاتين ألف. يعني عائد أربعة ونص في المية بس.',
  'وساعتها الفرق بيبقى تمنية وأربعين ألف وأربعمية وتسعة في الشهر.',
  'يعني فلوسك ترجع في اتناشر سنة بالعائد المنشور، أو اتنين وعشرين سنة بأرقامنا.',
