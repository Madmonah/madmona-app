    ...
    <div dir="rtl" className="min-h-screen bg-[#FAF7F2] pb-28">
      <div className="sticky top-0 z-30 bg-[#FAF7F2]/90 backdrop-blur border-b border-black/5">
        <div className="max-w-md mx-auto flex items-center gap-3 px-4 py-3">
          ...
          <h1 className="font-bold text-lg flex items-center gap-2"><Wallet className="w-5 h-5 text-[#2B4521]" /> سجل المعاملات</h1>
        </div>
      </div>
      ...
      <div className="max-w-md mx-auto px-4 pt-4">
        <div className="bg-white rounded-3xl shadow-soft overflow-hidden">
          {loading && txns.length === 0 ? (
            <div className="py-16 grid place-items-center"><Loader2 className="w-6 h-6 animate-spin text-[#2B4521]" /></div>
          ) : txns.length === 0 ? (
        ...
        {hasMore && (
          <button onClick={loadMore} disabled={loading}
            className="w-full mt-4 bg-white border border-gray-200 rounded-2xl py-3 font-bold text-[#2B4521] flex items-center justify-center gap-2 disabled:opacity-60">
