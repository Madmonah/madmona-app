...
export default function WalletPage() {
  ...
  if (stage === 'loading') {
    return <div className="min-h-screen grid place-items-center bg-[#FAF7F2]"><Loader2 className="w-7 h-7 animate-spin text-[#1F6F5F]" /></div>
  }
    ...
    return (
      <div dir="rtl" className="min-h-screen grid place-items-center bg-[#FAF7F2] px-6">
        <div className="text-center max-w-sm">
          <Wallet className="w-12 h-12 mx-auto text-[#1F6F5F] mb-3" />
          ...
          <Link href="/login" className="inline-block bg-[#1F6F5F] text-white px-6 py-3 rounded-2xl font-bold no-underline">تسجيل الدخول</Link>
        </div>
      </div>
    )
  }
    ...
    <div dir="rtl" className="min-h-screen bg-[#FAF7F2] pb-28">
      ...
      <div className="sticky top-0 z-30 bg-[#FAF7F2]/90 backdrop-blur border-b border-black/5">
        <div className="max-w-md mx-auto flex items-center gap-3 px-4 py-3">
          ...
          <h1 className="font-bold text-lg flex items-center gap-2"><Wallet className="w-5 h-5 text-[#1F6F5F]" /> محفظتي</h1>
        </div>
      </div>

      <div className="max-w-md mx-auto px-4 pt-4 space-y-4">
        {/* Balance card */}
        <div className="rounded-3xl p-5 text-white shadow-lg bg-gradient-to-br from-[#1F6F5F] to-[#2FA084]">
        ...
        <div className="bg-white rounded-3xl shadow-soft overflow-hidden">
          <div className="px-5 py-3 border-b border-gray-100 flex items-center justify-between">
            ...
            <Link href="/account/wallet/history" className="text-xs text-[#1F6F5F] no-underline">عرض الكل</Link>
          </div>
          {txns.length === 0 ? (
...
function ActionBtn({ icon, label, onClick }: { icon: React.ReactNode; label: string; onClick: () => void }) {
  return (
    <button onClick={onClick} className="bg-white rounded-2xl shadow-soft py-3 flex flex-col items-center gap-1.5 active:scale-95 transition">
      <span className="w-10 h-10 rounded-full bg-[#1F6F5F]/10 text-[#1F6F5F] grid place-items-center">{icon}</span>
  ...
  return (
    <Shell title="شحن المحفظة" onClose={onClose}>
      <div className="flex gap-2 mb-3">
        {quick.map(q => (
          <button key={q} onClick={() => setAmount(String(q))} className="flex-1 py-2 rounded-xl border border-gray-200 text-sm font-bold hover:border-[#1F6F5F]">{q}</button>
        ))}
      </div>
      <input value={amount} onChange={e => setAmount(e.target.value.replace(/[^\d.]/g, ''))} inputMode="decimal"
        placeholder="المبلغ بالجنيه" className="w-full border border-gray-200 rounded-2xl px-4 py-3 text-lg font-bold mb-2 outline-none focus:border-[#1F6F5F]" />
      <p className="text-[11px] text-gray-400 mb-4">ملاحظة: الشحن حاليًا داخلي/تجريبي. هيتربط ببوابة دفع حقيقية لاحقًا.</p>
      <button disabled={busy} onClick={submit} className="w-full bg-[#1F6F5F] text-white py-3 rounded-2xl font-bold flex items-center justify-center gap-2 disabled:opacity-60">
  ...
  return (
    <Shell title="تحويل رصيد" onClose={onClose}>
      ...
      <input value={phone} onChange={e => setPhone(e.target.value)} inputMode="tel" placeholder="01xxxxxxxxx"
        className="w-full border border-gray-200 rounded-2xl px-4 py-3 mb-3 outline-none focus:border-[#1F6F5F]" />
      ...
      <input value={amount} onChange={e => setAmount(e.target.value.replace(/[^\d.]/g, ''))} inputMode="decimal" placeholder="المبلغ"
        className="w-full border border-gray-200 rounded-2xl px-4 py-3 text-lg font-bold mb-4 outline-none focus:border-[#1F6F5F]" />
      <button disabled={busy} onClick={submit} className="w-full bg-[#1F6F5F] text-white py-3 rounded-2xl font-bold flex items-center justify-center gap-2 disabled:opacity-60">
...
function WithdrawModal({ token, max, onClose, onDone }: { token: string; max: number; onClose: () => void; onDone: () => void }) {
  ...
  return (
    <Shell title="سحب رصيد" onClose={onClose}>
      <label className="block text-xs font-bold text-gray-500 mb-1">المبلغ (متاح {formatMoney(max)})</label>
      <input value={amount} onChange={e => setAmount(e.target.value.replace(/[^\d.]/g, ''))} inputMode="decimal" placeholder="المبلغ"
        className="w-full border border-gray-200 rounded-2xl px-4 py-3 text-lg font-bold mb-3 outline-none focus:border-[#1F6F5F]" />
      <label className="block text-xs font-bold text-gray-500 mb-1">طريقة الاستلام</label>
      <select value={method} onChange={e => setMethod(e.target.value)}
        className="w-full border border-gray-200 rounded-2xl px-4 py-3 mb-3 outline-none focus:border-[#1F6F5F] bg-white">
      ...
      <input value={details} onChange={e => setDetails(e.target.value)} placeholder="مثال: 01xxxxxxxxx"
        className="w-full border border-gray-200 rounded-2xl px-4 py-3 mb-4 outline-none focus:border-[#1F6F5F]" />
      <button disabled={busy} onClick={submit} className="w-full bg-[#1F6F5F] text-white py-3 rounded-2xl font-bold flex items-center justify-center gap-2 disabled:opacity-60">
