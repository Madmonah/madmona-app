...
export default function AdBuilderClient({ listings }: { listings: ListingExtras[] }) {
  ...
  return (
    <div>
      ...
      <div style={cardStyle}>
        <h3 style={{ color: '#059669', margin: '0 0 12px' }}>1️⃣ إعدادات الـ Campaign</h3>
      ...
      <div style={cardStyle}>
        <h3 style={{ color: '#059669', margin: '0 0 12px' }}>2️⃣ اختار إعلان (اختياري)</h3>
        <input
          type="text"
          ...
          {filteredListings.length === 0 ? (
            ...
            filteredListings.map((l) => (
              <button
                ...
                style={{
                  background: selectedSlug === l.slug ? '#059669' : '#fff',
                  color: selectedSlug === l.slug ? '#FAF7F0' : '#1a1a1a',
                  border: `2px solid ${selectedSlug === l.slug ? '#059669' : '#eee'}`,
                  borderRadius: 12,
                  padding: 10,
                  textAlign: 'right',
                  cursor: 'pointer',
...
const copyBtnStyle = (active: boolean): React.CSSProperties => ({
  background: active ? '#28a745' : '#059669',
  color: '#fff',
  border: 'none',
  padding: '6px 14px',
  borderRadius: 6,
