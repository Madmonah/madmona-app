    ...
    <div dir="rtl" style={pageStyle}>
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        <header style={{ marginBottom: 20, textAlign: 'center' }}>
          <h1 style={{ color: '#1F5F3F', margin: 0, fontSize: 26 }}>🔔 Admin Alerts</h1>
          ...
          {alerts.map(a => (
            <article key={a.id} style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', gap: 8 }}>
                <h3 style={{ margin: 0, color: '#1F5F3F', fontSize: 14 }}>{a.title}</h3>
              ...
              {a.action_url && (
                <div style={{ marginTop: 8 }}>
                  <a href={a.action_url} style={{
                    color: '#1F5F3F', fontSize: 12, textDecoration: 'none',
...
function statusBg(status: string): string {
  switch (status) {
    ...
    case 'acknowledged': return '#B8860B'
