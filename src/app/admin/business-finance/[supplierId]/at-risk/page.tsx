...
const TIER_LABELS: Record<string, { label: string; cls: string }> = {
  ...
  vip: { label: 'VIP', cls: 'bg-[#34D399] text-[#04352A]' },
  regular: { label: 'منتظمة', cls: 'bg-[#34D399]/10 text-[#059669]' },
      ...
      <header className="bg-white border-b border-gray-100 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 py-4">
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              <p className="text-[10px] font-bold tracking-[0.3em] uppercase text-[#059669] mb-1">B2B PARTNER · CHURN PREDICTION</p>
          ...
          <div className="flex gap-2 flex-wrap">
            {[30, 60, 90, 180].map(d => (
              <button key={d} onClick={() => setDays(d)} className={`px-4 py-2 rounded-xl text-sm font-bold ${
                days === d ? 'bg-[#34D399] text-[#04352A]' : 'bg-[#FAFAF7] text-[#1A2E26]'
              }`}>{d} يوم</button>
            ))}
          </div>
        </section>
          ...
          <div className="divide-y divide-gray-100">
            {loading ? (
              ...
              <div className="py-12 text-center">
                <Star className="w-10 h-10 text-[#059669] opacity-30 mx-auto mb-2" />
              ...
              return (
                <div key={c.customer_id} className="grid grid-cols-12 gap-3 px-4 py-3 items-center text-sm">
                  <div className="col-span-3">
                    <Link href={`/admin/business-finance/${supplierId}/customers/${c.customer_id}`} className="font-bold text-[#1A2E26] hover:text-[#059669]">{c.full_name}</Link>
                  ...
                  <div className="col-span-2 text-center">
                    <a 
                      ...
                      target="_blank" 
                      rel="noopener"
                      className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg bg-[#34D399] text-[#04352A] text-xs font-bold"
                    >
