      ...
      <header className="bg-white border-b border-gray-100 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 py-4">
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              <p className="text-[10px] font-bold tracking-[0.3em] uppercase text-[#2B4521] mb-1">B2B PARTNER · ATTENDANCE</p>
                ...
                <tbody className="divide-y divide-gray-100">
                  {(data?.employee_summary || []).map((e: any) => (
                    <tr key={e.employee_id}>
                      ...
                      <td className="px-4 py-2 text-center font-black text-[#2B4521] font-mono">{Math.round(e.total_hours || 0)}</td>
  ...
  return (
    <div className="px-4 py-3 flex items-center gap-3 text-sm">
      ...
      <div className="flex items-center gap-3 text-xs">
        {inTime && (
          <div className={`flex items-center gap-1 ${isLate ? 'text-amber-700' : 'text-[#2B4521]'}`}>
        ...
        {outTime ? (
          ...
          <span className="px-2 py-0.5 rounded bg-[#2B4521]/10 text-[#2B4521] text-[10px] font-bold">في الشغل</span>
        )}
        {log.hours_worked && (
...
function TabBtn({ active, onClick, children }: any) {
  return (
    <button onClick={onClick} className={`flex-1 px-4 py-3 text-sm font-bold transition-colors ${
      active ? 'bg-[#2B4521]/5 text-[#2B4521] border-b-2 border-[#2B4521]' : 'text-[#6B7280] hover:text-[#1A2E26]'
    }`}>{children}</button>
  )
}
