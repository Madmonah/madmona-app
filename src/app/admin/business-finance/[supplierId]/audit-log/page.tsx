...
const ACTION_LABELS: Record<string, { label: string; icon: any; cls: string }> = {
  insert: { label: 'إضافة', icon: Plus, cls: 'text-[#FA8125]' },
      ...
      <header className="bg-white border-b border-gray-100 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 py-4">
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              <p className="text-[10px] font-bold tracking-[0.3em] uppercase text-[#FA8125] mb-1">B2B PARTNER · AUDIT LOG</p>
          ...
          <div className="flex gap-2 flex-wrap items-center">
            ...
            {TABLES.map(t => (
              <button key={t.value || 'all'} onClick={() => setTableFilter(t.value)} className={`px-3 py-1.5 rounded-lg text-xs font-bold ${
                tableFilter === t.value ? 'bg-[#FA8125] text-white' : 'bg-[#FAFAF7] text-[#1A2E26]'
              }`}>{t.label}</button>
            ))}
          </div>
        </section>
