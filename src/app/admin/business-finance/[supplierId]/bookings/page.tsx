...
const STATUS_CONFIG: Record<string, { label: string; cls: string }> = {
  ...
  confirmed: { label: 'مؤكد', cls: 'bg-[#34D399]/10 text-[#059669] border-[#059669]/20' },
      ...
      <header className="bg-white border-b border-gray-100 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 py-4">
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              <p className="text-[10px] font-bold tracking-[0.3em] uppercase text-[#059669] mb-1">B2B PARTNER · BOOKINGS</p>
            ...
            <div className="flex gap-2">
              <button onClick={() => setShowWalkin(true)} className="px-4 py-2 rounded-xl bg-[#34D399] text-[#04352A] text-sm font-bold flex items-center gap-2"><Plus className="w-4 h-4" /> حجز walk-in</button>
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 space-y-4">
        ...
        <section className="bg-white rounded-2xl border border-gray-100 p-4 space-y-3">
          <div className="flex gap-2 flex-wrap items-center">
            ...
            <button onClick={() => setDateFilter('')} className={`px-3 py-1.5 rounded-lg text-xs font-bold ${!dateFilter ? 'bg-[#34D399] text-[#04352A]' : 'bg-[#FAFAF7] text-[#1A2E26]'}`}>كل الأيام</button>
          </div>
          <div className="flex gap-2 flex-wrap items-center">
          ...
          <div className="flex gap-2 flex-wrap items-center">
            ...
            <button onClick={() => setStatusFilter(null)} className={`px-3 py-1.5 rounded-lg text-xs font-bold ${!statusFilter ? 'bg-[#34D399] text-[#04352A]' : 'bg-[#FAFAF7] text-[#1A2E26]'}`}>الكل</button>
            {Object.entries(STATUS_CONFIG).map(([k, v]) => (
              <button key={k} onClick={() => setStatusFilter(k)} className={`px-3 py-1.5 rounded-lg text-xs font-bold ${statusFilter === k ? 'bg-[#34D399] text-[#04352A]' : 'bg-[#FAFAF7] text-[#1A2E26]'}`}>{v.label}</button>
            ))}
          </div>
        </section>
