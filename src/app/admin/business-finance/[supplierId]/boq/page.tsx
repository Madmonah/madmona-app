      ...
      <header className="bg-white border-b border-gray-100 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 py-4">
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              <p className="text-[10px] font-bold tracking-[0.3em] uppercase text-[#059669] mb-1">مقاولات · جدول الكميات</p>
              <h1 className="text-2xl md:text-3xl font-black text-[#1A2E26] flex items-center gap-2"><Table2 className="w-7 h-7 text-[#059669]" /> جدول الكميات (BOQ)</h1>
            </div>
            <div className="flex gap-2 flex-wrap items-center">
              ...
              <button onClick={() => { setForm({ ...emptyForm }); setShowForm(true) }} disabled={!project} className="px-4 py-2 rounded-xl bg-[#34D399] text-[#04352A] text-sm font-bold flex items-center gap-2 disabled:opacity-50"><Plus className="w-4 h-4" /> بند جديد</button>
        ...
        {!project ? (
          ...
          <>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-5">
              <div className="flex items-center justify-between mb-2"><span className="text-xs font-bold text-[#6B7280]">نسبة تنفيذ الأعمال (بالقيمة)</span><span className="text-lg font-black text-[#059669]">{execPct.toFixed(1)}%</span></div>
              <div className="h-3 rounded-full bg-gray-100 overflow-hidden"><div className="h-full bg-gradient-to-l from-[#2FA084] to-[#34D399] rounded-full transition-all" style={{ width: `${Math.min(execPct, 100)}%` }} /></div>
            </div>
            ...
            {items.length === 0 ? (
              ...
              <div className="bg-white rounded-2xl border border-gray-100 overflow-x-auto">
                <table className="w-full text-sm">
                    ...
                    {items.map((it) => {
                      ...
                      return (
                        <tr key={it.id} className="border-b border-gray-50 hover:bg-[#FAFAF7]/50">
                          ...
                          <td className="px-3 py-2.5 text-left font-mono font-black text-[#059669]">{money0(it.amount)}</td>
                          <td className="px-3 py-2.5 text-center"><input type="number" value={it.executed_qty ?? ''} onChange={(e) => setExec(it.id, e.target.value)} onBlur={() => saveExec(it)} className="w-20 px-2 py-1 rounded-lg border border-gray-200 text-xs text-center font-mono focus:outline-none focus:border-[#059669]" /></td>
                          <td className="px-3 py-2.5 text-left font-mono text-xs font-bold" ><span className={itemPct >= 100 ? 'text-[#059669]' : 'text-[#6B7280]'}>{itemPct.toFixed(0)}%</span></td>
                  ...
                  <tfoot>
                    <tr className="bg-[#FAFAF7] border-t-2 border-[#059669]/20">
                      <td colSpan={5} className="px-3 py-3 text-left font-black text-[#1A2E26]">الإجمالي</td>
                      <td className="px-3 py-3 text-left font-mono font-black text-[#059669]">{money0(total)}</td>
                    ...
                    <tr className="bg-[#FAFAF7] border-t border-[#059669]/20">
                      <td colSpan={5} className="px-3 py-3 text-left font-black text-[#1A2E26]">الإجمالي شامل الإشراف</td>
                      <td className="px-3 py-3 text-left font-mono font-black text-[#059669]">{money0(totalWithSupervision)}</td>
                      <td colSpan={4}></td>
                    </tr>
                  </tfoot>
                </table>
      ...
      {showForm && project && (
        <Modal title="بند جديد" onClose={() => setShowForm(false)} onSave={save} saving={saving} saveLabel="إضافة البند">
          ...
          <div className="rounded-xl bg-[#FAFAF7] p-3 flex items-center justify-between">
            ...
            <span className="text-lg font-black text-[#059669] font-mono">{money(num(form.quantity) * num(form.unit_price))} ج</span>
          </div>
        </Modal>
      )}
    </div>
...
function Empty({ supplierId }: { supplierId: string }) {
  return <div className="bg-white rounded-2xl border border-gray-100 p-12 text-center"><FolderKanban className="w-12 h-12 text-[#6B7280] opacity-30 mx-auto mb-3" /><p className="text-sm font-bold text-[#1A2E26]">مفيش مشاريع لسه</p><Link href={`/admin/business-finance/${supplierId}/projects`} className="mt-4 px-4 py-2 rounded-xl bg-[#34D399] text-[#04352A] text-sm font-bold inline-flex items-center gap-2"><FolderKanban className="w-4 h-4" /> روح للمشاريع</Link></div>
}
