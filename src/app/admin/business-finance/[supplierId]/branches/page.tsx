    ...
    <div className="min-h-screen bg-[#FAFAF7]" dir="rtl">
      <header className="bg-white border-b border-gray-100 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 py-4">
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              <p className="text-[10px] font-bold tracking-[0.3em] uppercase text-[#1F6F5F] mb-1">B2B PARTNER · BRANCHES</p>
      ...
      <main className="max-w-7xl mx-auto px-4 py-6">
        <section className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {loading ? (
            <div className="col-span-2 py-12 text-center"><Loader2 className="w-6 h-6 text-[#1F6F5F] animate-spin inline" /></div>
          ) : branches.map(b => {
            ...
            return (
              <div key={b.id} className="bg-white rounded-2xl border border-gray-100 p-5">
                <div className="flex items-start justify-between gap-3 mb-3">
                  ...
                  <div className="flex items-center gap-1">
                    {b.booking_enabled !== false ? (
                      <span className="px-2 py-0.5 rounded bg-[#1F6F5F]/10 text-[#1F6F5F] text-[10px] font-bold flex items-center gap-1"><Power className="w-3 h-3" /> حجز مفتوح</span>
                    ) : (
                ...
                <div className="flex gap-2">
                  <button onClick={() => setEditing(b)} className="flex-1 px-3 py-2 rounded-xl bg-[#1F6F5F] text-white text-xs font-bold">تعديل الإعدادات</button>
                  <button onClick={() => copyBookingLink(b.code)} className="px-3 py-2 rounded-xl bg-[#FAFAF7] text-[#1A2E26] text-xs font-bold flex items-center gap-1">
                    {copiedCode === b.code ? <><CheckCircle2 className="w-3.5 h-3.5 text-[#1F6F5F]" /> اتنسخ</> : <><Copy className="w-3.5 h-3.5" /> رابط الحجز</>}
                  </button>
                </div>
    ...
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center" dir="rtl">
      ...
      <div className="relative bg-white rounded-t-3xl md:rounded-3xl w-full md:max-w-md md:mx-4 max-h-[90vh] overflow-y-auto shadow-2xl">
        ...
        <div className="p-5 space-y-3">
          {/* Booking toggle */}
          <button onClick={() => setForm({ ...form, booking_enabled: !form.booking_enabled })} className={`w-full p-3 rounded-xl border-2 flex items-center justify-between transition-all ${form.booking_enabled ? 'border-[#1F6F5F] bg-[#1F6F5F]/5' : 'border-gray-200 bg-gray-50'}`}>
            <div className="flex items-center gap-2">
              <Power className={`w-4 h-4 ${form.booking_enabled ? 'text-[#1F6F5F]' : 'text-gray-400'}`} />
              <span className="text-sm font-bold text-[#1A2E26]">الحجز الأونلاين</span>
            </div>
            <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${form.booking_enabled ? 'bg-[#1F6F5F] text-white' : 'bg-gray-200 text-gray-500'}`}>
