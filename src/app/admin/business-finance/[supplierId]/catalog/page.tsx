    ...
    <div className="min-h-screen bg-[#FAFAF7]" dir="rtl">
      ...
      <main className="max-w-5xl mx-auto px-4 py-6 space-y-6">
        <section className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 space-y-3">
          <h2 className="text-sm font-black text-[#1A2E26] flex items-center gap-1.5"><Plus className="w-4 h-4 text-[#1F6F5F]" /> ضيف جديد</h2>
          <Field label="الاسم *">
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            <input type="checkbox" checked={form.on_request} onChange={e => setForm({ ...form, on_request: e.target.checked })} className="w-4 h-4 accent-[#1F6F5F]" />
          ...
          <button onClick={addListing} disabled={saving} className="w-full py-3 rounded-xl bg-[#1F6F5F] text-white font-black text-sm disabled:opacity-50 flex items-center justify-center gap-2">
              ...
              {items.map((u) => (
                <div key={u.id} className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
                  <div className="aspect-[4/3] bg-gray-100 relative">
                    ...
                    <span className={`absolute top-2 right-2 px-2 py-0.5 rounded text-[10px] font-bold ${u.status === 'published' ? 'bg-[#1F6F5F] text-white' : 'bg-amber-100 text-amber-700'}`}>
                  ...
                  <div className="p-3">
                    ...
                    <div className="flex items-center justify-between mt-2">
                      <span className="text-sm font-mono font-black text-[#1F6F5F]">
