  ...
  return (
    <div className="min-h-screen bg-[#FAFAF7]" dir="rtl">
      <header className="bg-white border-b border-gray-100 sticky top-0 z-10">
        <div className="max-w-4xl mx-auto px-4 py-4">
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="text-[10px] font-bold tracking-[0.3em] uppercase text-[#FA8125] mb-1">B2B PARTNER · CONFIRMATIONS</p>
      ...
      <main className="max-w-4xl mx-auto px-4 py-6 space-y-6">
        {loading ? (
          <div className="py-20 text-center"><Loader2 className="w-7 h-7 text-[#FA8125] animate-spin inline" /></div>
        ) : totalPending === 0 ? (
          <div className="bg-white rounded-2xl border border-gray-100 p-12 text-center">
            <CheckCircle2 className="w-12 h-12 text-[#FA8125] opacity-40 mx-auto mb-2" />
                ...
                <Card key={j.id}>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-bold text-[#1A2E26] flex items-center gap-2">
                      {j.full_name}
                      {j.name_match && <span className="text-[9px] font-bold bg-[#FA8125]/10 text-[#FA8125] px-1.5 py-0.5 rounded">مطابق للمرتبات ✓</span>}
                    </p>
                    <p className="text-[11px] text-[#6B7280] mt-0.5 flex items-center gap-2 flex-wrap">
  ...
  return (
    <section>
      <h2 className="text-sm font-bold text-[#1A2E26] mb-3 flex items-center gap-2">
        <span className="text-[#FA8125]">{icon}</span> {title}
        <span className="text-[10px] font-bold bg-[#FA8125] text-white px-2 py-0.5 rounded-full">{count}</span>
      </h2>
...
function Actions({ busy, onYes, onNo, yesLabel }: any) {
  return (
    <div className="flex items-center gap-2 flex-shrink-0">
      <button onClick={onYes} disabled={busy} className="px-3 py-2 rounded-xl bg-[#FA8125] text-white text-xs font-bold flex items-center gap-1 disabled:opacity-50">
