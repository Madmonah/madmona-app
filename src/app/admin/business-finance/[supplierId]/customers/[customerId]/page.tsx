...
const TIER_LABELS: Record<string, { label: string; class: string; icon?: React.ReactNode }> = {
  ...
  vip: { label: 'VIP', class: 'bg-[#FA8125] text-white', icon: <Star className="w-3.5 h-3.5" /> },
  regular: { label: 'منتظمة', class: 'bg-[#FA8125]/10 text-[#FA8125]' },
    ...
    <div className="min-h-screen bg-[#FAFAF7]" dir="rtl">
      <header className="bg-white border-b border-gray-100 sticky top-0 z-10">
        <div className="max-w-5xl mx-auto px-4 py-4">
          <Link href={`/admin/business-finance/${supplierId}/customers`} className="text-xs font-bold text-[#6B7280] hover:text-[#FA8125] flex items-center gap-1 mb-2">
            <ChevronLeft className="w-3.5 h-3.5" /> رجوع للعملاء
          </Link>
          <div className="flex items-start gap-4">
            <div className="inline-grid place-items-center w-16 h-16 rounded-2xl bg-[#FA8125]/10 text-[#FA8125] font-black text-2xl flex-shrink-0">
            ...
            <div className="flex gap-2">
              <a href={`https://wa.me/${c.phone?.replace(/[^0-9]/g, '')}`} target="_blank" rel="noopener"
                className="px-3 py-2 rounded-xl bg-[#FA8125] hover:opacity-90 text-white text-xs font-bold flex items-center gap-1.5">
              ...
              {!editing ? (
                ...
                <>
                  <button onClick={save} disabled={saving} className="px-3 py-2 rounded-xl bg-[#FA8125] hover:opacity-90 text-white text-xs font-bold flex items-center gap-1.5 disabled:opacity-50">
        ...
        {ltv && ltv.total_visits > 0 && (
          <section className="bg-white rounded-2xl border border-gray-100 p-5">
            <h3 className="text-sm font-bold tracking-wider uppercase text-[#6B7280] mb-4 flex items-center gap-2">
              <TrendingUp className="w-4 h-4 text-[#FA8125]" /> Customer Lifetime Value
            </h3>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-center">
              ...
              <div className="p-3 rounded-xl bg-[#FA8125]/5 border border-[#FA8125]/20">
                <p className="text-[10px] font-bold uppercase text-[#FA8125]">عميلة عندنا</p>
                <p className="text-sm font-black text-[#FA8125] mt-1">{ltv.days_as_customer} يوم</p>
              </div>
              <div className="p-3 rounded-xl bg-[#FAFAF7]">
                ...
                <div key={b.id} className="p-4 flex items-start gap-3">
                  ...
                  <div className="text-left">
                    ...
                    <p className={`text-[10px] font-bold mt-0.5 ${
                      b.status === 'completed' ? 'text-[#FA8125]' :
      ...
      {editing ? (
        multiline ? (
          <textarea value={value} onChange={(e) => onChange(e.target.value)} rows={2}
            className="w-full px-3 py-2 bg-[#FAFAF7] rounded-lg text-sm text-[#1A2E26] focus:outline-none focus:ring-2 focus:ring-[#FA8125]" />
        ) : (
