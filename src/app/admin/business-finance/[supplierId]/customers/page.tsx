...
const TIER_LABELS: Record<string, { label: string; class: string }> = {
  ...
  vip: { label: 'VIP', class: 'bg-[#FA8125] text-white' },
      ...
      <header className="bg-white border-b border-gray-100 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 py-4">
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              <p className="text-[10px] font-bold tracking-[0.3em] uppercase text-[#FA8125] mb-1">B2B PARTNER · CUSTOMER CRM</p>
        ...
        {birthdayCustomers.length > 0 && (
          <section className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
            <div className="px-5 py-3 border-b border-gray-100 flex items-center gap-2">
              <Cake className="w-4 h-4 text-[#FA8125]" />
            ...
            <div className="p-3 flex gap-3 overflow-x-auto">
              {birthdayCustomers.map((c) => (
                <Link key={c.id} href={`/admin/business-finance/${supplierId}/customers/${c.id}`}
                  className={`flex-shrink-0 px-4 py-2.5 rounded-xl flex items-center gap-2 transition-all ${c.bday_today ? 'bg-[#FA8125] text-white hover:opacity-90' : 'bg-[#FAFAF7] hover:bg-gray-100 text-[#1A2E26]'}`}>
          ...
          <div className="flex gap-2 flex-wrap">
            {[
              ...
              <button key={f.v ?? 'all'} onClick={() => setTierFilter(f.v)}
                className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-colors ${tierFilter === f.v ? 'bg-[#FA8125] text-white' : 'bg-[#FAFAF7] text-[#1A2E26] hover:bg-gray-100'}`}>
                {f.label}
              </button>
            ))}
          </div>
...
function CustomerCard({ c, supplierId }: { c: Customer; supplierId: string }) {
  ...
  return (
    <Link href={`/admin/business-finance/${supplierId}/customers/${c.id}`}
      className="block bg-white rounded-2xl border border-gray-100 p-4 hover:shadow-md hover:border-[#FA8125] transition-all">
      <div className="flex items-start gap-3 mb-3">
        <div className="inline-grid place-items-center w-11 h-11 rounded-xl bg-[#FA8125]/10 text-[#FA8125] font-black text-base flex-shrink-0">{c.full_name.charAt(0)}</div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5">
            ...
            {c.bday_this_month && !c.bday_today && <Cake className="w-3 h-3 text-[#FA8125] flex-shrink-0" />}
          </div>
      ...
      <div className="grid grid-cols-3 gap-2 pt-3 border-t border-gray-100">
        ...
        <div><p className="text-[9px] text-[#6B7280] tracking-wider uppercase mb-0.5">نقاط</p><p className="text-sm font-black text-[#FA8125]">{c.loyalty_points}</p></div>
      </div>
