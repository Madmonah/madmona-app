      ...
      <header className="bg-white border-b border-gray-100 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 py-4">
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              <p className="text-[10px] font-bold tracking-[0.3em] uppercase text-[#2B4521] mb-1">B2B PARTNER · ANALYTICS</p>
            ...
            <div className="flex gap-2 flex-wrap items-center">
              {PERIODS.map(p => (
                <button key={p.days} onClick={() => setPeriod(p.days || 1)} className={`px-3 py-1.5 rounded-lg text-xs font-bold ${
                  period === (p.days || 1) ? 'bg-[#2B4521] text-white' : 'bg-[#FAFAF7] text-[#1A2E26] hover:bg-gray-100'
                }`}>{p.label}</button>
              ))}
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 space-y-5">
              ...
              return (
                <div key={i} className="rounded-2xl border border-gray-100 bg-white p-4">
                  ...
                  <div className="grid grid-cols-3 gap-2 text-center text-xs">
                    <div><p className="text-[#6B7280]">إيرادات</p><p className="font-black text-[#2B4521]">{rev.toLocaleString()}</p></div>
                    <div><p className="text-[#6B7280]">مصاريف</p><p className="font-black text-[#1A2E26]">{exp.toLocaleString()}</p></div>
                    <div><p className="text-[#6B7280]">صافي</p><p className={`font-black ${branchProfit >= 0 ? 'text-[#2B4521]' : 'text-red-600'}`}>{branchProfit.toLocaleString()}</p></div>
                  </div>
        ...
        <Section title="المصاريف حسب الفئة">
          {Object.keys(kpis?.expenses?.by_category || {}).length === 0 ? (
            <p className="text-sm text-[#6B7280] py-4 text-center">مفيش مصاريف مسجلة في الفترة دي. <Link href={`/admin/business-finance/${supplierId}/expenses`} className="text-[#2B4521] font-bold">أضف مصاريف</Link></p>
          ) : (
...
function KpiCard({ label, value, icon, tone, primary }: any) {
  const toneClass = tone === 'warning' ? 'text-amber-700' : tone === 'danger' ? 'text-red-600' : tone === 'positive' ? 'text-[#2B4521]' : 'text-[#1A2E26]'
  return (
    ...
    <div className="space-y-2">
      {items.map((item, i) => (
        <div key={i} className="flex items-center gap-3 py-2 border-b border-gray-100 last:border-0">
          <div className={`w-7 h-7 rounded-lg grid place-items-center text-xs font-black ${i === 0 ? 'bg-[#2B4521] text-white' : 'bg-[#FAFAF7] text-[#6B7280]'}`}>
          ...
          <span className="text-sm font-black font-mono text-[#2B4521]">{item.value}</span>
        </div>
      ))}
    </div>
  )
