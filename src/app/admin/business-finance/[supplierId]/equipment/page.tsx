...
const STATUSES = [
  { value: 'available',   label: 'متاحة',     color: 'bg-[#1F6F5F]/10 text-[#1F6F5F]' },
      ...
      <header className="bg-white border-b border-gray-100 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 py-4">
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              <p className="text-[10px] font-bold tracking-[0.3em] uppercase text-[#1F6F5F] mb-1">مقاولات · المعدات</p>
              <h1 className="text-2xl md:text-3xl font-black text-[#1A2E26] flex items-center gap-2"><Wrench className="w-7 h-7 text-[#1F6F5F]" /> المعدات</h1>
            </div>
            <div className="flex gap-2 flex-wrap items-center">
              <button onClick={openAdd} className="px-4 py-2 rounded-xl bg-[#1F6F5F] text-white text-sm font-bold flex items-center gap-2"><Plus className="w-4 h-4" /> معدة جديدة</button>
              ...
              <tbody>
                {rows.map((r) => (
                  <tr key={r.id} className="border-b border-gray-50 hover:bg-[#FAFAF7]/50">
                    ...
                    <td className="px-3 py-2.5 text-left font-mono text-[#1F6F5F]">{r.ownership === 'rented' && r.rental_cost > 0 ? money0(r.rental_cost) : '—'}</td>
