      ...
      <header className="bg-white border-b border-gray-100 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 py-4">
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              <p className="text-[10px] font-bold tracking-[0.3em] uppercase text-[#FA8125] mb-1">B2B PARTNER · EXPENSES</p>
          ...
          <div className="divide-y divide-gray-100">
            {loading ? (
              ...
              <div className="py-12 text-center">
                ...
                <button onClick={() => setShowAdd(true)} className="mt-3 px-4 py-2 rounded-xl bg-[#FA8125] text-white text-sm font-bold">أضف أول مصروف</button>
              </div>
            ...
            <div className="grid grid-cols-3 gap-1.5">
              {CATEGORIES.map(c => (
                <button key={c.value} onClick={() => setForm({ ...form, category: c.value })} className={`px-2 py-1.5 rounded-lg text-xs font-bold ${
                  form.category === c.value ? 'bg-[#FA8125] text-white' : 'bg-[#FAFAF7] text-[#1A2E26]'
                }`}>{c.label}</button>
              ))}
            </div>
          </Field>
            ...
            <div className="grid grid-cols-3 gap-1.5">
              {PAYMENT_METHODS.map(m => (
                <button key={m.value} onClick={() => setForm({ ...form, payment_method: m.value })} className={`px-2 py-1.5 rounded-lg text-xs font-bold ${
                  form.payment_method === m.value ? 'bg-[#FA8125] text-white' : 'bg-[#FAFAF7] text-[#1A2E26]'
                }`}>{m.label}</button>
              ))}
            </div>
          </Field>
