      ...
      <header className="bg-white border-b border-gray-100 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 py-4">
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              <p className="text-[10px] font-bold tracking-[0.3em] uppercase text-[#059669] mb-1">VEHICLE AGENCY · IMPORT</p>
            ...
            <div className="flex gap-2">
              <button onClick={() => setShowAdd(true)} className="px-4 py-2 rounded-xl bg-[#34D399] text-[#04352A] text-sm font-bold flex items-center gap-2"><Plus className="w-4 h-4" /> شحنة جديدة</button>
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 space-y-3">
        {loading ? (
          ...
          <div className="py-12 text-center bg-white rounded-2xl border border-gray-100">
            ...
            <button onClick={() => setShowAdd(true)} className="mt-3 px-4 py-2 rounded-xl bg-[#34D399] text-[#04352A] text-sm font-bold">أضف أول شحنة</button>
          </div>
        ) : rows.map((c: any) => {
            ...
            <div key={c.id} className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
              <div className="p-4">
                  ...
                  {STAGES.map(([k, label], i) => (
                    <div key={k} className="flex items-center gap-1 shrink-0">
                      <div className={`px-2.5 py-1 rounded-lg text-[10px] font-bold whitespace-nowrap ${
                        i < ci ? 'bg-[#34D399]/10 text-[#059669]' :
                        i === ci ? 'bg-[#34D399] text-[#04352A]' :
                        'bg-[#FAFAF7] text-[#6B7280]'
                      }`}>{label}</div>
                      {i < STAGES.length - 1 && <div className={`w-3 h-0.5 ${i < ci ? 'bg-[#34D399]' : 'bg-gray-200'}`} />}
                    </div>
                  ))}
                </div>
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  {c.stage !== 'released' && (
                    <button onClick={() => advance(c)} className="px-3 py-1.5 rounded-lg bg-[#34D399] text-[#04352A] text-xs font-bold flex items-center gap-1">
                      <ArrowLeft className="w-3 h-3" /> المرحلة التالية
                    </button>
                  )}
                  <button onClick={() => setEditFor(c)} className="px-3 py-1.5 rounded-lg bg-[#34D399]/10 text-[#059669] text-xs font-bold flex items-center gap-1">
                ...
                <div className="border-t border-gray-100 bg-[#FAFAF7]/40">
                  ...
                  <div className="px-4 pb-2">
                    <p className="text-[10px] font-bold tracking-wider uppercase text-[#6B7280] mb-1 flex items-center justify-between">
                      ...
                      <span className="font-mono text-[#059669]">إجمالي FOB: {c.total_fob ? `${num(c.total_fob)} ${c.currency || ''}` : '—'}</span>
                    </p>
                  </div>
                  <div className="border-t border-gray-100 divide-y divide-gray-100">
                    {(c.units || []).length === 0 ? (
                      ...
                      <div key={u.id} className="px-4 py-2.5 flex items-center justify-between text-xs">
                        ...
                        <span className="font-mono text-[#059669] font-bold">{u.unit_fob ? `${num(u.unit_fob)} fob` : '—'}</span>
                      </div>
                    ))}
                  </div>
                </div>
...
function EditDetailsModal({ consignment: c, onClose, onSaved }: any) {
  ...
  return (
    <Modal title={`تفاصيل ملف الاستيراد · ${c.ref || ''}`} onClose={onClose}>
      ...
      <button onClick={save} disabled={saving} className="w-full py-3 rounded-xl bg-[#34D399] text-[#04352A] font-black text-sm disabled:opacity-50 sticky bottom-0">{saving ? 'جاري الحفظ...' : 'احفظ التفاصيل'}</button>
    </Modal>
  )
}
...
function Group({ title, children }: any) {
  return (
    <div className="rounded-2xl border border-gray-100 p-3 space-y-3">
      <p className="text-[11px] font-black text-[#059669]">{title}</p>
      {children}
    </div>
  )
}
function Section({ title, icon, children }: any) {
  return (
    <div className="bg-white rounded-xl border border-gray-100 p-3">
      <p className="text-[10px] font-black tracking-wider uppercase text-[#059669] mb-2 flex items-center gap-1">{icon} {title}</p>
