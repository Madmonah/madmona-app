...
const STATUSES = [
  ...
  { value: 'paid',      label: 'مدفوع',   color: 'bg-[#2B4521]/10 text-[#2B4521]' },
]
      ...
      <header className="bg-white border-b border-gray-100 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 py-4">
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              <p className="text-[10px] font-bold tracking-[0.3em] uppercase text-[#2B4521] mb-1">مقاولات · المستخلصات</p>
              <h1 className="text-2xl md:text-3xl font-black text-[#1A2E26] flex items-center gap-2">
                <ScrollText className="w-7 h-7 text-[#2B4521]" /> المستخلصات
              </h1>
            </div>
            <div className="flex gap-2 flex-wrap items-center">
              ...
              <button onClick={() => { setForm({ ...emptyForm }); setShowForm(true) }} disabled={!project} className="px-4 py-2 rounded-xl bg-[#2B4521] text-white text-sm font-bold flex items-center gap-2 disabled:opacity-50 hover:shadow-md transition-shadow">
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 space-y-6">
        {!project ? (
          <div className="bg-white rounded-2xl border border-gray-100 p-12 text-center">
            ...
            <Link href={`/admin/business-finance/${supplierId}/projects`} className="mt-4 px-4 py-2 rounded-xl bg-[#2B4521] text-white text-sm font-bold inline-flex items-center gap-2">
                    ...
                    {certs.map((c) => {
                      ...
                      return (
                        <tr key={c.id} className="border-b border-gray-50 hover:bg-[#FAFAF7]/50">
                          ...
                          <td className="px-3 py-2.5 text-left font-mono font-black text-[#2B4521]">{money0(c.net_payable)} ج</td>
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-end md:items-center justify-center p-0 md:p-4" onClick={() => setShowForm(false)}>
          <div className="bg-white w-full md:max-w-2xl rounded-t-3xl md:rounded-3xl max-h-[92vh] overflow-y-auto" dir="rtl" onClick={(e) => e.stopPropagation()}>
            <div className="sticky top-0 bg-white border-b border-gray-100 px-5 py-4 flex items-center justify-between">
              <h2 className="text-lg font-black text-[#1A2E26] flex items-center gap-2"><Calculator className="w-5 h-5 text-[#2B4521]" /> مستخلص جديد · {project.name}</h2>
            ...
            <div className="p-5 grid md:grid-cols-2 gap-5">
              ...
              <div className="bg-[#FAFAF7] rounded-2xl p-4">
                ...
                <div className="mt-3 pt-3 border-t-2 border-[#2B4521]/20 flex items-center justify-between">
                  <span className="text-sm font-black text-[#1A2E26]">المستحق صرفه</span>
                  <span className="text-xl font-black text-[#2B4521] font-mono">{money(livePreview.net_payable)} <span className="text-xs">ج</span></span>
                </div>
              </div>
            </div>
      ...
      {detail && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-end md:items-center justify-center p-0 md:p-4" onClick={() => setDetail(null)}>
          <div className="bg-white w-full md:max-w-md rounded-t-3xl md:rounded-3xl max-h-[92vh] overflow-y-auto" dir="rtl" onClick={(e) => e.stopPropagation()}>
            <div className="sticky top-0 bg-white border-b border-gray-100 px-5 py-4 flex items-center justify-between">
              <h2 className="text-lg font-black text-[#1A2E26] flex items-center gap-2"><FileText className="w-5 h-5 text-[#2B4521]" /> {detail.cert_no}</h2>
            ...
            <div className="p-5">
              <div className="bg-[#FAFAF7] rounded-2xl p-4">
                ...
                <div className="mt-3 pt-3 border-t-2 border-[#2B4521]/20 flex items-center justify-between">
                  <span className="text-sm font-black text-[#1A2E26]">المستحق صرفه</span>
                  <span className="text-xl font-black text-[#2B4521] font-mono">{money(detail.net_payable)} <span className="text-xs">ج</span></span>
                </div>
              </div>
