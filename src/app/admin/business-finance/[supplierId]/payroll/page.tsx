      ...
      <header className="bg-white border-b border-gray-100 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 py-4">
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              <p className="text-[10px] font-bold tracking-[0.3em] uppercase text-[#059669] mb-1">B2B PARTNER · PAYROLL</p>
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              <button onClick={() => setShowStart(true)} className="px-4 py-2 rounded-xl bg-[#34D399] text-[#04352A] text-sm font-bold flex items-center gap-2">
          ...
          <div className="divide-y divide-gray-100">
            {loading ? (
              ...
              <div className="py-12 text-center">
                ...
                <button onClick={() => setShowStart(true)} className="mt-3 px-4 py-2 rounded-xl bg-[#34D399] text-[#04352A] text-sm font-bold">ابدأ أول Run</button>
              </div>
            ) : runs.map((r: any) => (
              <button key={r.id} onClick={() => loadRun(r.id)} className="w-full text-right grid grid-cols-12 gap-3 px-4 py-3 items-center hover:bg-[#FAFAF7] transition-colors text-sm">
                ...
                <div className="col-span-2 text-center">
                  ...
                  <p className="font-mono font-bold text-[#059669]">+{Number(r.total_commissions || 0).toLocaleString()}</p>
                </div>
    ...
    <section className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
      <div className="px-4 py-3 bg-[#FAFAF7] border-b border-gray-100 flex items-center justify-between gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-3">
          <button onClick={() => setShowHist(true)} className="px-3 py-1.5 rounded-xl bg-white border border-gray-200 text-[#1A2E26] text-xs font-bold flex items-center gap-1.5 hover:border-[#059669] transition-colors">
            <History className="w-3.5 h-3.5 text-[#059669]" /> سجل التغييرات
          </button>
      ...
      {msg && (
        <div className={`px-4 py-2 text-xs font-bold ${msg.ok ? 'bg-[#34D399]/10 text-[#059669]' : 'bg-red-50 text-red-600'}`}>
          {msg.text}
        </div>
      )}
            ...
            {editId === e.id ? (
              <div className="flex items-center gap-2">
                <input type="number" value={val} onChange={ev => setVal(ev.target.value)} autoFocus
                  ...
                  className="w-28 px-3 py-1.5 rounded-lg bg-[#FAFAF7] text-sm font-mono text-left border border-[#059669]" />
                <button onClick={() => save(e.id)} disabled={saving}
                  className="p-1.5 rounded-lg bg-[#34D399] text-[#04352A] disabled:opacity-50" title="حفظ">
              ...
              <div className="flex items-center gap-3">
                ...
                <button onClick={() => startEdit(e)}
                  className="p-1.5 rounded-lg bg-[#FAFAF7] hover:bg-[#34D399]/10 text-[#059669] transition-colors" title="تعديل المرتب">
      ...
      <div className="relative bg-[#FAFAF7] rounded-t-3xl md:rounded-3xl w-full md:max-w-2xl md:mx-4 max-h-[85vh] flex flex-col shadow-2xl">
        <header className="px-5 py-4 border-b border-gray-100 bg-white rounded-t-3xl flex items-center justify-between">
          <h2 className="text-lg font-black text-[#1A2E26] flex items-center gap-2">
            <History className="w-5 h-5 text-[#059669]" /> سجل تغييرات المرتبات
          </h2>
                ...
                return (
                  <div key={i} className="bg-white rounded-2xl border border-gray-100 p-3 flex items-center justify-between gap-3 flex-wrap">
                    ...
                    <div className="flex items-center gap-2 font-mono text-sm">
                      ...
                      <span className={`font-black ${up ? 'text-[#059669]' : 'text-red-600'}`}>{Number(h.new_salary).toLocaleString()} ج</span>
                    </div>
                  </div>
                )
              })}
      ...
      <div className="relative bg-[#FAFAF7] rounded-t-3xl md:rounded-3xl w-full md:max-w-5xl md:mx-4 max-h-[90vh] flex flex-col shadow-2xl">
        <header className="px-5 py-4 border-b border-gray-100 bg-white rounded-t-3xl flex items-center justify-between flex-wrap gap-2">
          ...
          <div className="flex gap-2">
            {r.status !== 'paid' && (
              <button onClick={closeRun} disabled={closing} className="px-4 py-2 rounded-xl bg-[#34D399] text-[#04352A] text-sm font-bold flex items-center gap-2 disabled:opacity-50">
                ...
                <tbody className="divide-y divide-gray-100">
                  {items.map((it: any) => (
                    <tr key={it.id}>
                      ...
                      <td className="px-3 py-2 text-center font-mono text-[#059669]">{it.commissions > 0 ? `+${Number(it.commissions).toLocaleString()}` : '—'}</td>
...
function StartRunModal({ supplierId, onClose, onSaved }: any) {
  ...
  return (
    <Modal onClose={onClose} title="ابدأ Run مرتبات">
      ...
      <button onClick={start} disabled={saving} className="w-full py-3 rounded-xl bg-[#34D399] text-[#04352A] font-black text-sm disabled:opacity-50">
...
function AddAdvanceModal({ supplierId, employees, onClose, onSaved }: any) {
  ...
  return (
    <Modal onClose={onClose} title="تسجيل سلفة موظف">
      ...
      <button onClick={save} disabled={saving} className="w-full py-3 rounded-xl bg-[#34D399] text-[#04352A] font-black text-sm disabled:opacity-50">
...
function Mini({ label, value, positive, negative, primary }: any) {
  const cls = primary ? 'bg-[#34D399] text-[#04352A]' : 'bg-white'
  const textCls = primary ? 'text-white' : positive ? 'text-[#059669]' : negative ? 'text-red-600' : 'text-[#1A2E26]'
  return (
...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, { label: string; cls: string }> = {
    ...
    ready: { label: 'جاهز', cls: 'bg-[#34D399]/10 text-[#059669]' },
    paid: { label: 'مدفوع ✓', cls: 'bg-[#34D399] text-[#04352A]' },
