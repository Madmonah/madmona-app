  ...
  return (
    <div className="min-h-screen bg-[#FAFAF7]" dir="rtl">
      <header className="bg-white border-b border-gray-100 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 py-4">
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              <p className="text-[10px] font-bold tracking-[0.3em] uppercase text-[#059669] mb-1">مقاولات · الربحية</p>
              <h1 className="text-2xl md:text-3xl font-black text-[#1A2E26] flex items-center gap-2"><BarChart3 className="w-7 h-7 text-[#059669]" /> ربحية المشاريع</h1>
            </div>
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 space-y-4">
        ...
        <div className="bg-[#34D399]/5 border border-[#059669]/15 rounded-2xl p-4 text-xs text-[#1A2E26] leading-relaxed">
              ...
              <tbody>
                {rows.map((r) => (
                  <tr key={r.id} className="border-b border-gray-50 hover:bg-[#FAFAF7]/50">
                    <td className="px-3 py-2.5"><Link href={`/admin/business-finance/${supplierId}/projects/${r.id}`} className="font-bold text-[#1A2E26] hover:text-[#059669]">{r.name}</Link><span className="block text-[10px] text-[#6B7280] font-mono">{r.code}</span></td>
                    ...
                    <td className={`px-3 py-2.5 text-left font-mono font-black ${r.profit >= 0 ? 'text-[#059669]' : 'text-red-600'}`}>{money0(r.profit)}</td>
                    <td className={`px-3 py-2.5 text-left font-mono font-bold ${r.margin >= 0 ? 'text-[#059669]' : 'text-red-600'}`}>{r.margin.toFixed(1)}%</td>
