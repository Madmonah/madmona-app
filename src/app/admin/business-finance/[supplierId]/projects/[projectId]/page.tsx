...
const STATUS_LABELS: Record<string, { label: string; color: string }> = {
  ...
  active:     { label: 'جاري التنفيذ', color: 'bg-[#FA8125]/10 text-[#FA8125]' },
...
export default function ProjectDetailPage({ params }: { params: { supplierId: string; projectId: string } }) {
  ...
  if (!project) return (
    <div className="min-h-screen bg-[#FAFAF7] flex flex-col items-center justify-center gap-3" dir="rtl">
      ...
      <Link href={`/admin/business-finance/${supplierId}/projects`} className="px-4 py-2 rounded-xl bg-[#FA8125] text-white text-sm font-bold">رجوع للمشاريع</Link>
    </div>
  )
    ...
    <div className="min-h-screen bg-[#FAFAF7]" dir="rtl">
      <header className="bg-white border-b border-gray-100 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 py-4">
          <Link href={`/admin/business-finance/${supplierId}/projects`} className="text-xs font-bold text-[#6B7280] hover:text-[#FA8125] flex items-center gap-1 mb-2"><ChevronLeft className="w-3.5 h-3.5" /> كل المشاريع</Link>
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              <div className="flex items-center gap-2 mb-1">
                <p className="text-[10px] font-bold tracking-[0.3em] uppercase text-[#FA8125]">{project.code}</p>
                <span className={`px-2 py-0.5 rounded-md text-[10px] font-bold ${st.color}`}>{st.label}</span>
              </div>
              <h1 className="text-2xl md:text-3xl font-black text-[#1A2E26] flex items-center gap-2"><Building2 className="w-7 h-7 text-[#FA8125]" /> {project.name}</h1>
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 space-y-6">
        ...
        <section>
          <h2 className="text-sm font-black text-[#1A2E26] mb-3 flex items-center gap-2"><TrendingUp className="w-4 h-4 text-[#FA8125]" /> الملخص المالي</h2>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-5 mt-3">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-lg font-black text-[#FA8125]">{progress.toFixed(1)}%</span>
            </div>
        ...
        <section>
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-sm font-black text-[#1A2E26] flex items-center gap-2"><FileText className="w-4 h-4 text-[#FA8125]" /> مستندات المشروع</h2>
            <button onClick={() => setShowDoc(true)} className="px-3 py-1.5 rounded-xl bg-[#FA8125] text-white text-xs font-bold flex items-center gap-1.5"><Plus className="w-3.5 h-3.5" /> مستند</button>
          </div>
          {docs.length === 0 ? (
            ...
            <div className="bg-white rounded-2xl border border-gray-100 divide-y divide-gray-50">
              {docs.map((d) => (
                <div key={d.id} className="flex items-center gap-3 px-4 py-3">
                  <span className="px-2 py-0.5 rounded-md text-[10px] font-bold bg-[#FA8125]/10 text-[#FA8125] shrink-0">{docLabel(d.doc_type)}</span>
                  ...
                  {d.url && <a href={d.url} target="_blank" rel="noreferrer" className="p-1.5 rounded-lg bg-[#FAFAF7] text-[#FA8125] hover:bg-gray-100"><ExternalLink className="w-3.5 h-3.5" /></a>}
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-end md:items-center justify-center p-0 md:p-4" onClick={() => setShowDoc(false)}>
          <div className="bg-white w-full md:max-w-md rounded-t-3xl md:rounded-3xl" dir="rtl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="border-t border-gray-100 px-5 py-4 flex gap-2">
              <button onClick={saveDoc} disabled={savingDoc} className="flex-1 py-3 rounded-xl bg-[#FA8125] text-white font-black text-sm flex items-center justify-center gap-2 disabled:opacity-60">{savingDoc ? <Loader2 className="w-4 h-4 animate-spin" /> : null} إضافة</button>
...
function Stat({ label, value, primary }: { label: string; value: string; primary?: boolean }) {
  return <div className={`rounded-2xl p-4 border ${primary ? 'bg-[#FA8125] border-[#FA8125] text-white' : 'bg-white border-gray-100'}`}><p className={`text-[10px] font-bold tracking-wider uppercase ${primary ? 'text-white/80' : 'text-[#6B7280]'}`}>{label}</p><p className={`text-lg md:text-xl font-black mt-1 ${primary ? 'text-white' : 'text-[#1A2E26]'}`}>{value}</p></div>
}
function ModLink({ href, icon, label, hint, primary }: { href: string; icon: ReactNode; label: string; hint: string; primary?: boolean }) {
  return (
    <Link href={href} className={`rounded-2xl p-4 border transition-all hover:shadow-md ${primary ? 'bg-[#FA8125] border-[#FA8125] text-white' : 'bg-white border-gray-100 hover:border-[#FA8125]/30'}`}>
      <div className={`w-9 h-9 rounded-xl flex items-center justify-center mb-2 [&_svg]:w-5 [&_svg]:h-5 ${primary ? 'bg-white/15 text-white' : 'bg-[#FA8125]/10 text-[#FA8125]'}`}>{icon}</div>
