      ...
      <header className="bg-white border-b border-gray-100 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 py-4">
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              <p className="text-[10px] font-bold tracking-[0.3em] uppercase text-[#059669] mb-1">B2B PARTNER · PROMOTIONS</p>
            ...
            <div className="flex gap-2">
              <button onClick={() => setShowAdd(true)} className="px-4 py-2 rounded-xl bg-[#34D399] text-[#04352A] text-sm font-bold flex items-center gap-2"><Plus className="w-4 h-4" /> عرض جديد</button>
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {loading ? (
            ...
            <div className="col-span-3 py-12 text-center bg-white rounded-2xl border border-gray-100">
              ...
              <button onClick={() => setShowAdd(true)} className="mt-3 px-4 py-2 rounded-xl bg-[#34D399] text-[#04352A] text-sm font-bold">أنشئ أول عرض</button>
            </div>
          ) : promos.map((p: any) => {
            const expired = p.expires_at && new Date(p.expires_at) < new Date()
            return (
              <div key={p.id} className={`bg-white rounded-2xl border p-4 ${expired ? 'border-gray-200 opacity-60' : p.is_active ? 'border-[#059669]/30' : 'border-gray-100'}`}>
                <div className="flex items-start justify-between mb-2">
                  ...
                  {expired ? <span className="text-[10px] font-bold text-red-600">منتهي</span>
                   : p.is_active ? <span className="text-[10px] font-bold text-[#059669]">نشط ✓</span>
                   : <span className="text-[10px] font-bold text-[#6B7280]">معطل</span>}
                </div>
                <div className="bg-[#34D399] text-[#04352A] rounded-xl p-3 my-3 text-center">
        ...
        <div className="p-5 space-y-3">
          ...
          <Field label="النوع">
            <div className="grid grid-cols-3 gap-1.5">
              {TYPES.map(t => (
                <button key={t.value} onClick={() => setForm({ ...form, type: t.value })} className={`px-2 py-1.5 rounded-lg text-xs font-bold ${form.type === t.value ? 'bg-[#34D399] text-[#04352A]' : 'bg-[#FAFAF7] text-[#1A2E26]'}`}>{t.label}</button>
              ))}
            </div>
          </Field>
          ...
          <button onClick={save} disabled={saving} className="w-full py-3 rounded-xl bg-[#34D399] text-[#04352A] font-black text-sm disabled:opacity-50">{saving ? 'جاري الحفظ...' : 'احفظ العرض'}</button>
        </div>
      </div>
    </div>
  )
