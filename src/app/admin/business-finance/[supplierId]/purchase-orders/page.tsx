      ...
      <header className="bg-white border-b border-gray-100 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 py-4">
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              <p className="text-[10px] font-bold tracking-[0.3em] uppercase text-[#2B4521] mb-1">B2B PARTNER · PURCHASE ORDERS</p>
          ...
          <div className="flex gap-2 flex-wrap items-center">
            ...
            {STATUSES.map(s => (
              <button key={s.value || 'all'} onClick={() => setStatusFilter(s.value)} className={`px-3 py-1.5 rounded-lg text-xs font-bold ${
                statusFilter === s.value ? 'bg-[#2B4521] text-white' : 'bg-[#FAFAF7] text-[#1A2E26]'
              }`}>{s.label}</button>
            ))}
          </div>
        </section>
          ...
          <div className="divide-y divide-gray-100">
            {loading ? (
              ...
              <div className="py-12 text-center">
                ...
                <button onClick={() => setShowAdd(true)} className="mt-3 px-4 py-2 rounded-xl bg-[#2B4521] text-white text-sm font-bold">أنشئ أول طلب</button>
              </div>
              ...
              <div key={po.id} className="px-4 py-3 grid grid-cols-12 gap-3 items-center text-sm">
                ...
                <div className="col-span-3 text-left">
                  {po.status === 'pending' || po.status === 'ordered' ? (
                    <button onClick={() => receive(po.id)} className="px-3 py-1.5 rounded-lg bg-[#2B4521] text-white text-xs font-bold inline-flex items-center gap-1">
                      <Truck className="w-3 h-3" /> سجل وصول
                    </button>
                  ) : po.status === 'received' ? (
                    <span className="text-[10px] text-[#2B4521] font-bold flex items-center justify-end gap-1">
    ...
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center" dir="rtl">
      ...
      <div className="relative bg-white rounded-t-3xl md:rounded-3xl w-full md:max-w-2xl md:mx-4 max-h-[90vh] flex flex-col shadow-2xl">
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-3">
          ...
          <div className="pt-2">
            <div className="flex items-center justify-between mb-2">
              ...
              <button onClick={addItem} className="text-xs font-bold text-[#2B4521] flex items-center gap-1"><Plus className="w-3 h-3" /> اضف منتج</button>
            </div>
          ...
          {total > 0 && (
            <div className="rounded-xl bg-[#2B4521]/5 border border-[#2B4521]/20 p-3 flex justify-between font-bold">
              <span>الإجمالي:</span>
              <span className="font-mono font-black text-[#2B4521]">{total.toLocaleString()} ج</span>
            </div>
          )}
        ...
        <footer className="px-5 py-3 border-t border-gray-100">
          <button onClick={save} disabled={saving || items.length === 0} className="w-full py-3 rounded-xl bg-[#2B4521] text-white font-black text-sm disabled:opacity-50">
...
function POStatusBadge({ status }: { status: string }) {
  const map: Record<string, { label: string; cls: string }> = {
    ...
    received: { label: 'وصلت ✓', cls: 'bg-[#2B4521]/10 text-[#2B4521]' },
