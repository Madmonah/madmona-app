  ...
  return (
    <div className="min-h-screen bg-[#FAFAF7] pb-16" dir="rtl">
      ...
      <main className="max-w-3xl mx-auto px-4 py-5 space-y-4">
        {flash && <div className="bg-[#FA8125]/10 text-[#FA8125] text-sm font-bold px-4 py-3 rounded-2xl">{flash}</div>}
        ...
        {loading && orders.length === 0 && (
          <div className="text-center py-16"><Loader2 className="w-7 h-7 text-[#FA8125] animate-spin mx-auto" /></div>
        )}
        ...
        {quoted.length > 0 && (
          <p className="text-[11px] font-black tracking-widest uppercase text-[#FA8125] flex items-center gap-1.5 pt-2"><Send className="w-3.5 h-3.5" /> اتبعت للعميل ({quoted.length})</p>
        )}
        {quoted.map(o => (
...
function OrderCard({ o, draft, busy, editable, total, onPrice, onQty, onFee, onNote, onSend }: {
  ...
  const fld = 'px-2 py-1.5 bg-[#FAFAF7] border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-[#FA8125]/50 focus:bg-white'
  return (
    <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-5">
      <div className="flex items-start justify-between gap-3 mb-3">
        ...
        {o.status === 'quoted' && (
          <span className="inline-flex items-center gap-1 text-[11px] font-bold text-[#FA8125] bg-[#FA8125]/10 px-2.5 py-1 rounded-full"><CheckCircle className="w-3 h-3" /> مسعّر</span>
        )}
      </div>
      ...
      <div className="border-t border-gray-100 mt-3 pt-3 space-y-2">
        ...
        <div className="flex items-center justify-between font-black text-[#FA8125]">
          <span>الإجمالي</span>
      ...
      {editable && (
        <button onClick={onSend} disabled={busy} className="mt-4 w-full flex items-center justify-center gap-2 bg-[#FA8125] text-white py-3 rounded-2xl font-bold text-sm shadow-sm hover:-translate-y-0.5 transition-all disabled:opacity-50">
