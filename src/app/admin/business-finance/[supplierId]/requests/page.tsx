  ...
  return (
    <div className="min-h-screen bg-[#FAFAF7]" dir="rtl">
      <header className="bg-white border-b border-gray-100 sticky top-0 z-10">
        <div className="max-w-4xl mx-auto px-4 py-4">
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              <p className="text-[10px] font-bold tracking-[0.3em] uppercase text-[#1F6F5F] mb-1">الموارد البشرية</p>
              <h1 className="text-2xl md:text-3xl font-black text-[#1A2E26] flex items-center gap-2"><ClipboardCheck className="w-7 h-7 text-[#1F6F5F]" /> طلبات الموظفين</h1>
      ...
      <main className="max-w-4xl mx-auto px-4 py-6 space-y-6">
        ...
        {leave.length > 0 && (
          <section>
            <h2 className="text-sm font-bold tracking-wider uppercase text-[#6B7280] mb-3 flex items-center gap-1.5"><CalendarDays className="w-4 h-4 text-[#1F6F5F]" /> إجازات ({leave.length})</h2>
                ...
                <div key={r.id} className="bg-white rounded-2xl border border-gray-100 p-4">
                  <div className="flex items-start justify-between gap-3 flex-wrap">
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => decideLeave(r.id, true)} disabled={busyId === r.id} className="px-3 py-2 rounded-xl bg-[#1F6F5F] text-white text-[13px] font-bold flex items-center gap-1.5 disabled:opacity-50">{busyId === r.id ? <Loader2 className="w-4 h-4 animate-spin" /> : <Check className="w-4 h-4" />} قبول</button>
        ...
        {advances.length > 0 && (
          <section>
            <h2 className="text-sm font-bold tracking-wider uppercase text-[#6B7280] mb-3 flex items-center gap-1.5"><Wallet className="w-4 h-4 text-[#1F6F5F]" /> سُلف ({advances.length})</h2>
            <div className="space-y-2">
              {advances.map((r) => (
                <div key={r.id} className="bg-white rounded-2xl border border-gray-100 p-4">
                  <div className="flex items-start justify-between gap-3 flex-wrap">
                    <div className="min-w-0">
                      <p className="font-black text-[#1A2E26]">{r.employee}</p>
                      <p className="text-[15px] font-black text-[#1F6F5F] mt-0.5" dir="ltr">{money0(r.amount)} ج</p>
                      {r.reason && <p className="text-[12px] text-[#1A2E26] mt-1">السبب: {r.reason}</p>}
                    </div>
                    <div className="flex gap-2">
                      <button onClick={() => decideAdvance(r.id, true)} disabled={busyId === r.id} className="px-3 py-2 rounded-xl bg-[#1F6F5F] text-white text-[13px] font-bold flex items-center gap-1.5 disabled:opacity-50">{busyId === r.id ? <Loader2 className="w-4 h-4 animate-spin" /> : <Check className="w-4 h-4" />} قبول وصرف</button>
