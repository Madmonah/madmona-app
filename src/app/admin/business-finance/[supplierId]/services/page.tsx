      ...
      <header className="bg-white border-b border-gray-100 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 py-4">
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              <p className="text-[10px] font-bold tracking-[0.3em] uppercase text-[#1F6F5F] mb-1">B2B PARTNER · SERVICES</p>
          ...
          {loading ? (
            ...
            <button key={s.id} onClick={() => setSelectedService(s)} className="bg-white rounded-2xl border border-gray-100 p-4 text-right hover:shadow-md hover:border-[#1F6F5F] transition-all">
              ...
              <div className="flex items-center justify-between text-xs">
                ...
                <span className="font-mono font-black text-[#1F6F5F]">{Number(s.price_egp).toLocaleString()} ج</span>
              </div>
              {s.performer_commission_pct > 0 && (
                <p className="text-[10px] text-[#1F6F5F] mt-1 font-bold">عمولة {s.performer_commission_pct}%</p>
              )}
    ...
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center" dir="rtl">
      ...
      <div className="relative bg-[#FAFAF7] rounded-t-3xl md:rounded-3xl w-full md:max-w-2xl md:mx-4 max-h-[90vh] flex flex-col shadow-2xl">
        <header className="px-5 py-4 border-b border-gray-100 bg-white rounded-t-3xl flex items-center justify-between">
          <div>
            <p className="text-[10px] font-bold tracking-wider uppercase text-[#1F6F5F]">ربط منتجات</p>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-3">
          {loading ? (
            ...
            <>
              ...
              <button onClick={addProduct} className="w-full p-2.5 rounded-xl border-2 border-dashed border-gray-200 hover:border-[#1F6F5F] text-[#6B7280] hover:text-[#1F6F5F] text-sm font-bold transition-colors flex items-center justify-center gap-2">
