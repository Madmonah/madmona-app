...
const STATUSES = [
  { value: 'active',     label: 'شغّال',  color: 'bg-[#34D399]/10 text-[#059669]' },
      ...
      <header className="bg-white border-b border-gray-100 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 py-4">
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              <p className="text-[10px] font-bold tracking-[0.3em] uppercase text-[#059669] mb-1">مقاولات · مقاولي الباطن</p>
              <h1 className="text-2xl md:text-3xl font-black text-[#1A2E26] flex items-center gap-2"><HardHat className="w-7 h-7 text-[#059669]" /> مقاولي الباطن</h1>
            </div>
            <div className="flex gap-2 flex-wrap items-center">
              ...
              <button onClick={openAdd} disabled={!project} className="px-4 py-2 rounded-xl bg-[#34D399] text-[#04352A] text-sm font-bold flex items-center gap-2 disabled:opacity-50"><Plus className="w-4 h-4" /> مقاول جديد</button>
                ...
                {rows.map((r) => {
                  ...
                  return (
                    <div key={r.id} className="bg-white rounded-2xl border border-gray-100 p-5">
                      ...
                      <div className="h-2 rounded-full bg-gray-100 overflow-hidden mt-3"><div className="h-full bg-[#34D399] rounded-full" style={{ width: `${pct}%` }} /></div>
...
function Row({ label, value, bold }: { label: string; value: any; bold?: boolean }) { return <div className="flex items-center justify-between"><span className="text-[#6B7280]">{label}</span><span className={`font-mono ${bold ? 'font-black text-[#059669] text-sm' : 'text-[#1A2E26]'}`}>{money0(value)} ج</span></div> }
