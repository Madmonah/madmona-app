  ...
  return (
    <div className="min-h-screen bg-[#FAFAF7]" dir="rtl">
      <header className="bg-white border-b border-gray-100 sticky top-0 z-10 print:hidden">
        <div className="max-w-7xl mx-auto px-4 py-4">
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              <p className="text-[10px] font-bold tracking-[0.3em] uppercase text-[#1F6F5F] mb-1">B2B PARTNER · VAT REPORT</p>
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 space-y-5">
        ...
        {/* VAT Payable Hero */}
        <section className={`rounded-3xl p-6 border-2 ${vatPayable > 0 ? 'bg-amber-50 border-amber-300' : 'bg-[#1F6F5F]/5 border-[#1F6F5F]/30'}`}>
          <p className="text-xs font-bold tracking-wider uppercase text-[#6B7280] mb-2">المستحق سدادها للمصلحة</p>
          <p className={`text-5xl font-black font-mono ${vatPayable > 0 ? 'text-amber-800' : 'text-[#1F6F5F]'}`}>
          ...
          {vatPayable > 0 ? (
            ...
            <p className="text-xs text-[#1F6F5F] mt-2 font-bold">✓ المصلحة مدينة لك (استرداد)</p>
          )}
        </section>
        ...
        <section className="bg-white rounded-2xl border border-gray-100 p-5">
          <h3 className="text-sm font-bold tracking-wider uppercase text-[#6B7280] mb-4 flex items-center gap-2">
            <TrendingUp className="w-4 h-4 text-[#1F6F5F]" /> VAT المحصلة (Output VAT)
          </h3>
              ...
              <tbody className="divide-y divide-gray-100">
                {(data?.by_branch || []).map((b: any, i: number) => (
                  <tr key={i}>
                    ...
                    <td className="px-4 py-2 text-center font-mono font-bold text-[#1F6F5F]">{Number(b.vat_collected || 0).toLocaleString()} ج</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
function KV({ label, value, highlight }: any) {
  return (
    <div className={`p-3 rounded-xl ${highlight ? 'bg-[#1F6F5F]/5 border border-[#1F6F5F]/20' : 'bg-[#FAFAF7]'}`}>
      <p className="text-[10px] font-bold tracking-wider uppercase text-[#6B7280]">{label}</p>
      <p className={`text-lg font-black font-mono mt-1 ${highlight ? 'text-[#1F6F5F]' : 'text-[#1A2E26]'}`}>{value}</p>
    </div>
  )
}
