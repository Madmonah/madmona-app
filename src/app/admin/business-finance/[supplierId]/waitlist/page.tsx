      ...
      <header className="bg-white border-b border-gray-100 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 py-4">
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              <p className="text-[10px] font-bold tracking-[0.3em] uppercase text-[#1F6F5F] mb-1">B2B PARTNER · WAITLIST</p>
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 space-y-4">
        ...
        <section className="bg-white rounded-2xl border border-gray-100 p-4 space-y-3">
          <div className="flex gap-2 flex-wrap items-center">
            {STATUS_TABS.map(t => (
              <button key={t.value} onClick={() => setStatusFilter(t.value)} className={`px-3 py-1.5 rounded-lg text-xs font-bold ${statusFilter === t.value ? 'bg-[#1F6F5F] text-white' : 'bg-[#FAFAF7] text-[#1A2E26]'}`}>{t.label}</button>
            ))}
          </div>
          <div className="flex gap-2 flex-wrap items-center">
            ...
            <div key={e.waitlist_id} className="bg-white rounded-2xl border border-gray-100 p-4">
              <div className="flex items-start justify-between gap-3 flex-wrap">
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  <div className="w-8 h-8 rounded-full bg-[#1F6F5F]/10 text-[#1F6F5F] grid place-items-center font-black text-sm">{i + 1}</div>
                ...
                {e.status === 'waiting' && (
                  <div className="flex gap-1.5 flex-wrap">
                    {busyId === e.waitlist_id ? <Loader2 className="w-5 h-5 text-[#1F6F5F] animate-spin" /> : (
                      <>
                        <a
                          ...
                          className="px-2.5 py-1.5 rounded-lg bg-[#1F6F5F] text-white text-[10px] font-bold flex items-center gap-1"
                        >
              ...
              {e.notes && <p className="text-[10px] text-[#1F6F5F] mt-2 pt-2 border-t border-gray-100">💡 {e.notes}</p>}
            </div>
          ))}
        </section>
      </main>
    ...
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center" dir="rtl">
      ...
      <div className="relative bg-white rounded-t-3xl md:rounded-3xl w-full md:max-w-md md:mx-4 shadow-2xl">
        <header className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
          <div>
            <p className="text-[10px] font-bold tracking-wider uppercase text-[#1F6F5F]">تحويل لحجز</p>
        ...
        <div className="p-5 space-y-3">
          ...
          <button onClick={convert} disabled={saving} className="w-full py-3 rounded-xl bg-[#1F6F5F] text-white font-black text-sm disabled:opacity-50 flex items-center justify-center gap-2">
