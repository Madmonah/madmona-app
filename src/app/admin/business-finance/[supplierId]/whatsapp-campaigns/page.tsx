      ...
      <header className="bg-white border-b border-gray-100 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 py-4">
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              <p className="text-[10px] font-bold tracking-[0.3em] uppercase text-[#1F6F5F] mb-1">B2B PARTNER · WHATSAPP</p>
            ...
            {loading ? (
              ...
              <div key={c.id} className="px-4 py-3 grid grid-cols-12 gap-3 items-center text-sm">
                ...
                <div className="col-span-2 text-center">
                  ...
                  <p className="font-mono font-bold text-[#1F6F5F]">{c.read_count}</p>
                </div>
                <div className="col-span-2 text-center">
...
function CampaignStatusBadge({ status }: { status: string }) {
  const map: Record<string, { label: string; cls: string }> = {
    ...
    completed: { label: 'اكتمل ✓', cls: 'bg-[#1F6F5F]/10 text-[#1F6F5F]' },
