    ...
    <div className="min-h-screen bg-[#FAFAF7]" dir="rtl">
      <header className="bg-white border-b border-gray-100 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 py-4">
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              <p className="text-[10px] font-bold tracking-[0.3em] uppercase text-[#059669] mb-1">VEHICLE AGENCY · WORKSHOP</p>
      ...
      <main className="max-w-7xl mx-auto px-4 py-6">
        <section className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {loading ? (
            <div className="md:col-span-2 py-12 text-center"><Loader2 className="w-6 h-6 text-[#059669] animate-spin inline" /></div>
          ) : services.length === 0 ? (
            ...
            return (
              <div key={s.id} className="bg-white rounded-2xl border border-gray-100 p-4">
                <div className="flex items-start justify-between gap-2 mb-3">
                  ...
                  <span className="text-sm font-mono font-black text-[#059669] whitespace-nowrap">{price.toLocaleString('ar-EG')} ج</span>
                </div>
...
function KV({ label, value, accent }: { label: string; value: string; accent?: boolean }) {
  return (
    <div>
      ...
      <p className={`text-sm font-mono font-black ${accent ? 'text-[#059669]' : 'text-[#1A2E26]'}`}>{value}</p>
    </div>
  )
}
