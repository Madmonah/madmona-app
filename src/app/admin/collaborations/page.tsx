...
const STATUS_COLOR: Record<string, string> = {
  active: '#0EA5E9', completed: '#28a745', failed: '#C2410C', paused: '#666',
}
    ...
    <div dir="rtl" style={{ fontFamily: 'Tahoma', background: '#FAF7F0', minHeight: '100vh', padding: 24 }}>
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
          <h1 style={{ color: '#1F5F3F', margin: 0, fontSize: 26 }}>🎯 Agent Collaborations</h1>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 24 }}>
          {[
            { label: 'Collaborations', val: collaborations.length, color: '#1F5F3F' },
            { label: 'Active', val: collaborations.filter(c => c.status === 'active').length, color: '#0EA5E9' },
            { label: 'Messages', val: messages.length, color: '#B8860B' },
          ].map((s, i) => (
        ...
        <h2 style={{ color: '#1F5F3F', fontSize: 18, marginBottom: 12 }}>🤝 Active Collaborations</h2>
          ...
          <div style={{ display: 'grid', gap: 12, marginBottom: 24 }}>
            {collaborations.map(c => (
              <div key={c.id} style={{
                ...
                <div style={{ fontSize: 12, color: '#1F5F3F' }}>
        ...
        <h2 style={{ color: '#1F5F3F', fontSize: 18, marginBottom: 12 }}>📬 Recent Messages</h2>
