  ...
  if (stage === 'unauthenticated') return (
    <div className="min-h-screen bg-[#FAFAF7] flex items-center justify-center p-4" dir="rtl">
      <div className="bg-white rounded-2xl border p-8 text-center max-w-sm">
        ...
        <Link href="/auth/login?redirect=/admin/email-queue" className="block bg-[#2B4521] text-white py-3 rounded-xl font-semibold">دخول</Link>
      </div>
    </div>
  )
    ...
    <div className="min-h-screen bg-[#FAFAF7] pb-20" dir="rtl">
      <header className="sticky top-0 z-40 bg-white border-b border-gray-100">
        <div className="max-w-6xl mx-auto px-4 py-4 flex items-center gap-3">
          ...
          <Mail className="w-5 h-5 text-[#2B4521]" />
          ...
          <button onClick={fireProcessor} disabled={firing} className="bg-[#2B4521] text-white px-3 py-2 rounded-xl text-sm font-bold flex items-center gap-1.5 hover:bg-[#2B4521]/90 disabled:opacity-50">
          ...
          {([
            ...
            <button key={k} onClick={() => setQueue(k as any)}
              className={`flex-1 py-2.5 rounded-xl border text-sm font-bold flex items-center justify-center gap-1.5 transition ${
                queue === k ? 'bg-[#2B4521] text-white border-[#2B4521]' : 'bg-white border-gray-200 text-gray-700'
              }`}>
            ...
            return (
              <button key={s} onClick={() => setStatus(s)}
                className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-bold border ${
                  isActive ? 'bg-[#2B4521] text-white border-[#2B4521]' : `${cfg.bg} ${cfg.color}`
                }`}>
                ...
                <div className="flex items-start gap-3">
                  ...
                  {canRetry && (
                    <button onClick={() => retryRow(r)} disabled={isRetrying}
                      className="flex-shrink-0 px-3 py-1.5 rounded-lg bg-[#2B4521] text-white text-xs font-bold flex items-center gap-1 disabled:opacity-50">
