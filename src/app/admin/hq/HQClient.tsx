...
const TABS: Array<{ id: TabId; label: string; icon: string; color: string }> = [
  { id: 'dashboard', label: 'لوحة القيادة', icon: '📊', color: '#2B4521' },
  { id: 'marketplace', label: 'السوق', icon: '🏪', color: '#2B4521' },
  ...
  { id: 'collaborations', label: 'تعاون', icon: '🎯', color: '#2B4521' },
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 12, marginBottom: 20 }}>
        <BigMetric icon="💎" label="عمولة الشهر" value={`${k.monthCommission.toLocaleString('ar-EG')} ج`} subtitle={`إجمالي: ${k.totalCommission.toLocaleString('ar-EG')} ج`} color="#2B4521" />
                  ...
                  <a key={String(b.id)} href={`/bookings/${String(b.id)}`} style={{ display: 'block', padding: 12, borderBottom: i < 7 ? '1px solid #eee' : 'none', textDecoration: 'none', color: 'inherit' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
                      ...
                      <div style={{ textAlign: 'left' }}>
                        <div style={{ fontSize: 14, fontWeight: 'bold', color: '#2B4521' }}>{Number(b.total_amount).toLocaleString('ar-EG')} ج</div>
            ...
            {data.topListings.length === 0 ? <Empty msg="مفيش إعلانات" /> :
              data.topListings.slice(0, 8).map((l, i) => (
                <a key={String(l.id)} href={`/marketplace/${String(l.slug)}`} target="_blank" rel="noopener" style={{ display: 'flex', alignItems: 'center', gap: 10, padding: 12, borderBottom: i < 7 ? '1px solid #eee' : 'none', textDecoration: 'none', color: 'inherit' }}>
                  <span style={{ width: 22, height: 22, background: '#2B4521', color: '#fff', borderRadius: 11, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 'bold' }}>{i + 1}</span>
  ...
  return (
    <div style={{ background: '#fff', padding: 14, borderRadius: 10, border: '1px solid #eee', borderRight: `4px solid ${statusColor}` }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
        <div style={{ flex: 1 }}>
          <h3 style={{ margin: 0, fontSize: 15, color: '#2B4521' }}>🏪 {String(supplier.business_name)}</h3>
          {profile?.full_name && (
      ...
      {msg && (
        <div style={{ marginTop: 8, padding: 6, background: '#FAF7F0', borderRadius: 4, fontSize: 11, color: '#2B4521', textAlign: 'center', fontWeight: 'bold' }}>
          {msg}
        </div>
      )}
    </div>
...
function RunAgentButton({ agentName }: { agentName: string }) {
  ...
  return (
    <button onClick={run} disabled={running} style={{ padding: '4px 10px', background: '#2B4521', color: '#fff', border: 'none', borderRadius: 4, cursor: running ? 'wait' : 'pointer', fontSize: 11, fontFamily: 'inherit' }}>
      {running ? '⏳' : result ?? '▶'}
    </button>
  )
}
          ...
          {ads.length === 0 ? <Empty msg="مفيش إعلانات" /> :
            ads.map((a, i) => (
              <div key={i} style={card('#6FCF97')}>
                <h3 style={{ margin: 0, color: '#2B4521', fontSize: 15 }}>{String(a.headline)}</h3>
      ...
      {sub === 'reels' && (
        <div style={{ display: 'grid', gap: 12 }}>
          {reels.length === 0 ? <Empty msg="مفيش reels" /> :
            reels.map((r, i) => (
              <div key={i} style={card('#6FCF97')}>
                <h3 style={{ margin: 0, color: '#2B4521', fontSize: 14 }}>{String(r.title)}</h3>
                <div style={{ background: '#2B4521', color: '#FAF7F0', padding: 8, borderRadius: 6, margin: '6px 0', fontSize: 12, fontWeight: 'bold' }}>💥 {String(r.hook)}</div>
              </div>
            ))}
        </div>
      )}
        ...
        <div style={{ display: 'grid', gap: 12 }}>
          {demand.length === 0 ? <Empty msg="مفيش توقعات" /> :
            demand.map((f, i) => {
              ...
              return (
                <div key={i} style={card(gap < -5 ? '#6FCF97' : '#2B4521')}>
                  ...
                  {Boolean(f.recommended_action) && (
                    <div style={{ background: '#2B4521', color: '#FAF7F0', padding: 8, borderRadius: 6, fontSize: 11, marginTop: 6 }}>👉 {String(f.recommended_action)}</div>
                  )}
                </div>
              )
            })}
          ...
          {fraud.length === 0 ? <Empty msg="✅ المنصة آمنة" /> :
            fraud.map((a, i) => (
              <div key={i} style={card(a.severity === 'critical' || a.severity === 'high' ? '#6FCF97' : '#2FA084')}>
                ...
                <p style={{ fontSize: 10, color: '#2B4521' }}>👉 {String(a.recommended_action)}</p>
              </div>
            ))}
        </div>
      )}
          ...
          {pricing.length === 0 ? <Empty msg="مفيش اقتراحات" /> :
            pricing.map((p, i) => (
              <div key={i} style={card('#2FA084')}>
                ...
                <span style={{ fontSize: 20, color: '#2B4521', fontWeight: 'bold' }}> {String(p.suggested_price)}ج</span>
        ...
        <div style={{ display: 'grid', gap: 12 }}>
          {partnerships.length === 0 ? <Empty msg="مفيش فرص" /> :
            partnerships.map((o, i) => (
              <div key={i} style={card(o.priority === 'urgent' ? '#6FCF97' : '#2B4521')}>
                ...
                <div style={{ background: '#FAF7F0', padding: 6, borderRadius: 4, fontSize: 10, color: '#2B4521' }}>💎 {String(o.potential_value)}</div>
              </div>
            ))}
        </div>
      )}
            ...
            newInsights.map((ins, i) => (
              <div key={i} style={card(ins.priority === 'high' ? '#6FCF97' : '#0EA5E9')}>
                ...
                {Boolean(ins.recommended_action) && (
                  <div style={{ background: '#FAF7F0', padding: 6, borderRadius: 4, fontSize: 10, color: '#2B4521' }}>👉 {String(ins.recommended_action)}</div>
                )}
              </div>
            ))}
        </div>
...
function CollaborationsTab({ collabs, messages }: { collabs: Array<Record<string, unknown>>; messages: Array<Record<string, unknown>>; activeCollabs: number }) {
  ...
  return (
    <div>
      <div style={{ background: 'linear-gradient(135deg, #2B4521 0%, #10B981 100%)', color: '#fff', padding: 16, borderRadius: 12, marginBottom: 20 }}>
      ...
      <div style={{ background: '#fff', borderRadius: 12, overflow: 'hidden', maxHeight: 350, overflowY: 'auto' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
          <thead><tr style={{ background: '#2B4521', color: '#FAF7F0' }}><th style={th}>From → To</th><th style={th}>Subject</th><th style={th}>Status</th></tr></thead>
          <tbody>
            {messages.map((m, i) => (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 10 }}>
        {links.map((l, i) => (
          <a key={i} href={l.href} target={l.href.startsWith('/') ? '_self' : '_blank'} rel="noopener" style={{ background: '#fff', padding: 12, borderRadius: 10, border: '1px solid #eee', textDecoration: 'none', color: 'inherit', position: 'relative' }}>
            ...
            <div style={{ fontSize: 12, fontWeight: 'bold', color: '#2B4521', marginBottom: 2 }}>{l.title}</div>
...
function FilterBtn({ label, active, onClick }: { label: string; active: boolean; onClick: () => void }) {
  return (
    <button onClick={onClick} style={{ padding: '5px 12px', background: active ? '#2B4521' : '#fff', color: active ? '#FAF7F0' : '#2B4521', border: '1px solid #2B4521', borderRadius: 6, cursor: 'pointer', fontSize: 11, fontWeight: 'bold', fontFamily: 'inherit' }}>{label}</button>
  )
}
...
function Stat({ label, val }: { label: string; val: string }) {
  return <div style={{ textAlign: 'center' }}><div style={{ fontSize: 14, fontWeight: 'bold', color: '#2B4521' }}>{val}</div><div style={{ fontSize: 9, color: '#666' }}>{label}</div></div>
}
...
const sectionHeader: React.CSSProperties = { color: '#2B4521', fontSize: 14, marginTop: 16, marginBottom: 10, fontWeight: 'bold' }
...
const primaryBtn: React.CSSProperties = { display: 'inline-block', background: '#2B4521', color: '#fff', padding: '8px 14px', borderRadius: 6, textDecoration: 'none', fontSize: 12, fontWeight: 'bold' }
...
function priorityBadge(priority: string): React.CSSProperties {
  const colors: Record<string, string> = { urgent: '#6FCF97', high: '#2FA084', medium: '#2B4521', low: '#666' }
