...
const PRIORITY_COLORS: Record<string, { bg: string; fg: string; label: string }> = {
  high: { bg: '#fee', fg: '#6FCF97', label: '🔥 عالي' },
  medium: { bg: '#fff3cd', fg: '#2FA084', label: '⚡ متوسط' },
    ...
    <div dir="rtl" style={{
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
          <div>
            <h1 style={{ color: '#059669', margin: 0, fontSize: 26 }}>💡 AI Insights</h1>
        ...
        <div style={{
          ...
          {[
            ...
            { label: '📊 إجمالي', val: stats.length, color: '#059669' },
          ].map((s, i) => (
            <div key={i} style={{
              background: '#fff',
              padding: 18,
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
          {[
            ...
            <a
              ...
              style={{
                background: filter === f.value ? '#059669' : '#fff',
                color: filter === f.value ? '#FAF7F0' : '#059669',
                padding: '6px 14px',
                borderRadius: 8,
                textDecoration: 'none',
                fontSize: 12,
        ...
        {agentCounts.size > 0 && (
          <div style={{ marginBottom: 20, fontSize: 12 }}>
            ...
            <a href={`?filter=${filter}`} style={{
              padding: '4px 10px',
              background: agentFilter === 'all' ? '#059669' : '#fff',
              color: agentFilter === 'all' ? '#FAF7F0' : '#059669',
              borderRadius: 6,
              textDecoration: 'none',
              marginLeft: 6,
              border: '1px solid #059669',
            ...
            {Array.from(agentCounts.entries()).map(([name, count]) => (
              <a key={name} href={`?filter=${filter}&agent=${name}`} style={{
                padding: '4px 10px',
                background: agentFilter === name ? '#059669' : '#fff',
                ...
                <div key={insight.id} style={{
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      ...
                      <div>
                        <h3 style={{ margin: 0, color: '#059669', fontSize: 15 }}>
                          {insight.title}
                        </h3>
