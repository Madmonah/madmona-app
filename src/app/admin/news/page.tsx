...
export default function AdminNewsPage() {
  ...
  if (stage === 'loading') {
    return <div className="min-h-screen bg-[#FAFAF7] flex items-center justify-center" dir="rtl"><Loader2 className="w-6 h-6 text-[#FA8125] animate-spin" /></div>
  }
    ...
    return (
      <div className="min-h-screen bg-[#FAFAF7] flex items-center justify-center p-4" dir="rtl">
        <div className="bg-white rounded-3xl shadow-luxe p-8 text-center max-w-sm">
          ...
          <Link href="/auth/login?redirect=/admin/news" className="block bg-[#FA8125] text-white py-3 rounded-xl font-semibold">دخول</Link>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="min-h-screen bg-[#FAFAF7] flex items-center justify-center p-4" dir="rtl">
        <div className="bg-white rounded-3xl shadow-luxe p-8 text-center max-w-sm">
          ...
          <Link href="/account" className="inline-block bg-[#FA8125] text-white px-5 py-2.5 rounded-xl font-semibold">ارجع للحساب</Link>
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-[#FAFAF7] pb-20" dir="rtl">
      <header className="sticky top-0 z-40 bg-white/80 backdrop-blur-xl border-b border-gray-100">
        <div className="max-w-5xl mx-auto px-4 py-4 flex items-center gap-3">
          ...
          {!showForm && (
            <button onClick={startNew} className="flex items-center gap-1.5 px-4 py-2 bg-[#FA8125] text-white rounded-xl text-sm font-bold hover:bg-[#FA8125]/90 transition-colors">
      ...
      <main className="max-w-5xl mx-auto px-4 py-6 space-y-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <textarea
                  ...
                  rows={2}
                  className="w-full px-4 py-2.5 bg-[#FAFAF7] border border-gray-100 rounded-xl text-sm focus:outline-none focus:bg-white focus:border-[#FA8125]/40 resize-none"
                />
                  ...
                  {CATEGORIES.map(c => {
                    ...
                    return (
                      <button
                        ...
                        className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold transition-all ${isActive ? 'bg-[#FA8125] text-white' : 'bg-[#FAFAF7] text-gray-700 hover:bg-gray-100'}`}
                      >
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 bg-[#FAFAF7] border border-gray-100 rounded-xl text-sm focus:outline-none focus:bg-white focus:border-[#FA8125]/40 font-mono mb-2"
                  dir="ltr"
                  style={{ textAlign: 'left' }}
                />
                <input
              ...
              <div className="flex gap-2 pt-2">
                <button
                  ...
                  className="flex-1 px-4 py-3 bg-[#FA8125] hover:bg-[#FA8125]/90 text-white font-bold rounded-xl text-sm flex items-center justify-center gap-2 disabled:opacity-50"
                >
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-hide">
          <button onClick={() => setFilter('all')} className={`flex-shrink-0 px-3 py-1.5 rounded-lg text-xs font-bold transition-colors ${filter === 'all' ? 'bg-[#FA8125] text-white' : 'bg-white text-gray-600 hover:bg-gray-50'}`}>
            <Filter className="w-3 h-3 inline-block ml-1" /> الكل ({items.length})
          </button>
          {CATEGORIES.map(c => {
            const count = items.filter(i => i.category === c.id).length
            return (
              <button key={c.id} onClick={() => setFilter(c.id)} className={`flex-shrink-0 px-3 py-1.5 rounded-lg text-xs font-bold transition-colors ${filter === c.id ? 'bg-[#FA8125] text-white' : 'bg-white text-gray-600 hover:bg-gray-50'}`}>
                {c.label} ({count})
              </button>
            )
          })}
        ...
        <div className="space-y-2">
          {filtered.length === 0 ? (
            <div className="bg-white rounded-3xl p-12 text-center shadow-soft">
              ...
              <button onClick={startNew} className="inline-flex items-center gap-1.5 px-4 py-2 bg-[#FA8125] text-white rounded-xl text-sm font-bold">
                ...
                <div key={row.id} className={`bg-white rounded-2xl shadow-soft p-4 flex items-stretch gap-3 ${!row.is_published ? 'opacity-60' : ''}`}>
                  <div className="w-20 h-20 sm:w-24 sm:h-24 flex-shrink-0 rounded-xl overflow-hidden bg-gray-100 relative">
                    {row.image_url ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center bg-[#FA8125]">
