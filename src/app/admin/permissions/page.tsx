...
export default function PermissionsPage() {
  ...
  if (stage === 'loading') return <Center><Loader2 className="w-8 h-8 text-[#2B4521] animate-spin" /></Center>
  if (stage === 'unauthenticated') return (
    <Center>
      <div className="bg-white rounded-3xl p-8 text-center max-w-sm shadow-sm">
        ...
        <Link href="/auth/login?redirect=/admin/permissions" className="block bg-[#2B4521] text-white py-3 rounded-xl font-bold">تسجيل دخول</Link>
      </div>
    </Center>
  )
    ...
    <div className="min-h-screen text-[#1A2E26]" dir="rtl" style={{ background: 'radial-gradient(1100px 560px at 88% -8%, rgba(47,160,132,0.10), transparent 60%), radial-gradient(900px 480px at -5% 4%, rgba(43, 69, 33,0.09), transparent 55%), radial-gradient(800px 500px at 50% 118%, rgba(212,160,23,0.06), transparent 60%), #FAFAF7' }}>
      <header className="sticky top-0 z-30 border-b border-[#2B4521]/10 bg-white/70 backdrop-blur-xl">
        <div className="max-w-5xl mx-auto px-4 py-4 flex items-center gap-3">
          ...
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[#D4A017] via-[#2FA084] to-[#2B4521] flex items-center justify-center text-white shadow"><ShieldCheck className="w-5 h-5" /></div>
          <div className="flex-1 min-w-0">
            <p className="text-[10px] font-black tracking-[0.3em] uppercase bg-gradient-to-r from-[#D4A017] to-[#2B4521] bg-clip-text text-transparent">PERMISSIONS</p>
            <h1 className="text-base md:text-lg font-black leading-none">صلاحيات الموظفين</h1>
          </div>
          <button onClick={() => setAddOpen(true)} className="flex items-center gap-1.5 bg-[#2B4521] text-white text-xs font-bold px-3 py-2 rounded-xl hover:bg-[#185547]"><Plus className="w-4 h-4" /> صلاحية جديدة</button>
        </div>
      </header>
      ...
      <main className="max-w-5xl mx-auto px-4 py-5 space-y-6 pb-24">
        <div className="relative">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="دوّر باسم الموظف أو الـ PIN…"
            className="w-full bg-white border border-black/5 rounded-2xl pr-10 pl-4 py-3 text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-[#2B4521]/30" />
        </div>
...
function SupplierGroup({ s, catalog, search, open, onToggleOpen, onToggle, onBulk, busy, platform }: {
  ...
  return (
    <div className={`bg-white rounded-2xl border ${platform ? 'border-[#2B4521]/30' : 'border-black/5'} shadow-sm overflow-hidden`}>
      <button onClick={onToggleOpen} className="w-full flex items-center justify-between gap-3 px-4 py-3 hover:bg-[#FAFAF7]/60">
        <div className="flex items-center gap-2.5 min-w-0">
          <div className={`w-9 h-9 rounded-xl flex items-center justify-center text-white flex-shrink-0 ${platform ? 'bg-gradient-to-br from-[#D4A017] via-[#2FA084] to-[#2B4521]' : 'bg-[#2B4521]/90'}`}><Building2 className="w-4 h-4" /></div>
  ...
  return (
    <div className="px-4 py-3">
      <div className="flex items-center justify-between gap-3 mb-2.5 flex-wrap">
        <div className="flex items-center gap-2.5 min-w-0">
          <div className="w-9 h-9 rounded-full bg-[#2B4521]/10 text-[#2B4521] flex items-center justify-center font-black text-sm flex-shrink-0">{(e.full_name || '؟').trim().charAt(0)}</div>
        ...
        <div className="flex items-center gap-1.5 flex-shrink-0">
          <span className="text-[10px] font-bold text-[#2B4521]">{onCount}/{catalog.length}</span>
          <button disabled={bulkBusy} onClick={() => onBulk(e, true)} className="text-[10px] font-bold px-2 py-1 rounded-lg bg-[#2B4521]/10 text-[#2B4521] hover:bg-[#2B4521]/20 disabled:opacity-40">الكل</button>
          ...
          return (
            <button key={c.key} disabled={b} onClick={() => onToggle(e, c.key)}
              className={`text-[11px] font-bold px-2.5 py-1.5 rounded-full border transition-all disabled:opacity-50 flex items-center gap-1 ${
                on ? 'bg-gradient-to-br from-[#2FA084] to-[#2B4521] text-white border-transparent shadow-sm'
                   : 'bg-white text-[#6B7280] border-black/10 hover:border-[#2B4521]/40'
              }`}>
    ...
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4" dir="rtl" onClick={onClose}>
      <div className="bg-white rounded-3xl p-6 w-full max-w-sm shadow-xl" onClick={e => e.stopPropagation()}>
        ...
        <input value={labelAr} onChange={e => setLabelAr(e.target.value)} placeholder="مثلاً: يعدّل الأسعار"
          className="w-full bg-[#FAFAF7] border border-black/5 rounded-xl px-3 py-2.5 text-sm mb-3 focus:outline-none focus:ring-2 focus:ring-[#2B4521]/30" />
        ...
        <input value={key} onChange={e => setKey(e.target.value)} placeholder="can_edit_pricing" dir="ltr"
          className="w-full bg-[#FAFAF7] border border-black/5 rounded-xl px-3 py-2.5 text-sm mb-4 font-mono focus:outline-none focus:ring-2 focus:ring-[#2B4521]/30" />
        <button onClick={save} disabled={saving || !labelAr.trim()}
          className="w-full bg-[#2B4521] text-white py-3 rounded-xl font-bold disabled:opacity-50 flex items-center justify-center gap-2">
...
function SectionHead({ icon, title, note }: { icon: ReactNode; title: string; note: string }) {
  return (
    <div className="mb-3 flex items-start gap-2.5">
      <span className="mt-0.5 w-8 h-8 rounded-xl bg-gradient-to-br from-[#D4A017] to-[#2B4521] text-white flex items-center justify-center flex-shrink-0">{icon}</span>
      <div>
