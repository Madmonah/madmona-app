...
export default function PhotoAuditPage() {
  ...
  return (
    <div dir="rtl" lang="ar" className="min-h-screen bg-[#FAFAF7] px-4 py-6 md:px-8">
      <header className="mx-auto mb-6 flex max-w-6xl items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-600">
            {remaining != null && (
              <>باقي <b className="text-[#1F6F5F]">{remaining}</b> إعلان يحتاج مراجعة · </>
            )}
        ...
        <button
          onClick={load}
          className="rounded-lg bg-[#1F6F5F] px-4 py-2 text-sm font-bold text-white hover:bg-[#2ea080]"
        >
          🔄 تحديث
        </button>
      </header>
      ...
      {toast && (
        <div className={`fixed bottom-6 right-6 z-50 rounded-xl px-5 py-3 shadow-lg ${toast.ok ? 'bg-[#1F6F5F] text-white' : 'bg-red-600 text-white'}`}>
          {toast.msg}
        </div>
      )}
              ...
              <section key={l.id} className="rounded-2xl bg-white p-5 shadow-sm">
                <div className="flex flex-wrap items-baseline justify-between gap-2">
                  ...
                  <div className="flex items-center gap-3 text-sm text-gray-600">
                    {l.category_name && <span className="rounded-full bg-[#1F6F5F]/10 px-3 py-0.5 text-[#1F6F5F] font-medium">{l.category_name}</span>}
                    {l.supplier_name && <span>👤 {l.supplier_name}</span>}
                    {l.price_egp && <span className="font-bold text-[#1F6F5F]">{fmtPrice(l.price_egp)} ج</span>}
                  </div>
                </div>
                <div className="mt-1 text-sm text-gray-500">
                  {[l.district, l.city].filter(Boolean).join('، ') || 'بدون منطقة'}
                  {l.slug && (
                    <> · <a href={`/marketplace/${l.slug}`} target="_blank" className="text-[#1F6F5F] hover:underline">عرض الإعلان ↗</a></>
                  )}
                </div>
                      ...
                      <div key={p.id} className="relative">
                        <button
                          ...
                          className={`block aspect-square w-full overflow-hidden rounded-xl border-4 transition ${
                            isPrimary ? 'border-[#1F6F5F] ring-2 ring-[#1F6F5F]/40' : isHidden ? 'border-red-400 opacity-40' : 'border-transparent hover:border-gray-300'
                          }`}
                        ...
                        {isPrimary && (
                          <span className="absolute right-2 top-2 rounded-full bg-[#1F6F5F] px-2 py-0.5 text-xs font-bold text-white shadow">⭐ رئيسية</span>
                        )}
                ...
                <div className="mt-5 flex items-center justify-between gap-3 border-t border-gray-100 pt-4">
                  <div className="text-sm text-gray-600">
                    ...
                    {hasPrimary ? <span className="text-[#1F6F5F] font-medium">✓ الرئيسية متحددة</span> : <span className="text-amber-600">لسه ما اخترتش الرئيسية</span>}
                  </div>
                  <button
                    onClick={() => confirm(l.id)}
                    disabled={!hasPrimary || busy[l.id]}
                    className="rounded-lg bg-[#1F6F5F] px-5 py-2 text-sm font-bold text-white shadow disabled:opacity-40 hover:bg-[#2ea080]"
                  >
