    ...
    <div dir="rtl" className="min-h-screen bg-[#FAFAF7] p-4 md:p-8">
      <div className="max-w-5xl mx-auto">
        <header className="flex items-center justify-between gap-3 mb-6">
          <div>
            <h1 className="text-2xl font-bold text-[#FA8125]">📩 استفسارات المشاريع</h1>
            <p className="text-xs text-gray-500 mt-1">{rows.length} استفسار</p>
          </div>
          <Link
            href="/admin/projects"
            className="px-4 py-2 rounded-full border border-gray-200 bg-white text-sm font-semibold text-gray-700 hover:border-[#FA8125]/40"
          >
            ← المشاريع
          </Link>
        </header>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-5 mb-6">
            ...
            <div className="flex flex-wrap gap-2">
              {top.map(([title, count]) => (
                <span
                  key={title}
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-[#FA8125]/8 text-[#FA8125] text-xs font-semibold"
                >
                  {title}
                  <span className="bg-[#FA8125] text-white rounded-full px-1.5 text-[10px]">{count}</span>
                </span>
              ))}
            </div>
          </div>
              ...
              <div key={r.id} className="bg-white rounded-xl border border-gray-100 p-4">
                ...
                {r.contact_phone && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener"
                    className="text-xs text-[#FA8125] font-semibold hover:underline"
                    dir="ltr"
                  >
                    {r.contact_phone}
                  </a>
