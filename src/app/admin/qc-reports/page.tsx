    ...
    <div dir="rtl" style={{ fontFamily: 'Tahoma', background: '#FAF7F0', minHeight: '100vh', padding: 24 }}>
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
          <h1 style={{ color: '#059669', margin: 0, fontSize: 26 }}>✅ QC Reports</h1>
                ...
                <div key={r.id} style={{
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8, marginBottom: 12 }}>
                    <div style={{ background: '#FAF7F0', padding: 8, borderRadius: 6, textAlign: 'center' }}>
                      <div style={{ fontSize: 16, fontWeight: 'bold', color: '#059669' }}>{r.title_quality_score ?? 0}</div>
                      <div style={{ fontSize: 10, color: '#666' }}>عنوان</div>
                    </div>
                    <div style={{ background: '#FAF7F0', padding: 8, borderRadius: 6, textAlign: 'center' }}>
                      <div style={{ fontSize: 16, fontWeight: 'bold', color: '#059669' }}>{r.description_quality_score ?? 0}</div>
                      <div style={{ fontSize: 10, color: '#666' }}>وصف</div>
                    </div>
                    <div style={{ background: '#FAF7F0', padding: 8, borderRadius: 6, textAlign: 'center' }}>
                      <div style={{ fontSize: 16, fontWeight: 'bold', color: '#059669' }}>{r.photos_quality_score ?? 0}</div>
                      ...
                      <div style={{ marginTop: 8 }}>
                        {r.improvements.map((imp: Record<string, unknown>, i) => (
                          <div key={i} style={{
                            ...
                            <span style={{ color: '#059669' }}>المقترح: {String(imp.suggested ?? '')}</span>
