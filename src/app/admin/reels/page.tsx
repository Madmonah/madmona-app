    ...
    <div dir="rtl" style={{ fontFamily: 'Tahoma', background: '#FAF7F0', minHeight: '100vh', padding: 24 }}>
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
          <h1 style={{ color: '#059669', margin: 0, fontSize: 26 }}>🎬 Reel Scripts</h1>
            ...
            {all.map(reel => (
              <div key={reel.id} style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                  <h2 style={{ margin: 0, color: '#059669', fontSize: 18 }}>{reel.title}</h2>
                  <span style={{
                    background: reel.video_url ? '#059669' : reel.status === 'approved' ? '#d4edda' : '#fff3cd',
                ...
                {reel.video_url && (
                  <div style={{ margin: '12px 0', background: '#000', borderRadius: 8, overflow: 'hidden', maxWidth: 360 }}>
                    ...
                    />
                    <div style={{ padding: 8, background: '#059669', display: 'flex', gap: 8, alignItems: 'center' }}>
                      <a href={reel.video_url} download style={{ color: '#FAF7F0', fontSize: 12, textDecoration: 'underline' }}>
                        ⬇ تحميل MP4
                      </a>
                      <span style={{ color: '#2FA084', fontSize: 11 }}>·</span>
                ...
                {/* Hook */}
                <div style={{
                  background: '#059669', color: '#FAF7F0', padding: 16,
                ...
                {reel.hashtags && reel.hashtags.length > 0 && (
                  <p style={{ fontSize: 11, color: '#059669', margin: '8px 0', wordSpacing: '4px' }}>
                    {reel.hashtags.join(' ')}
                  </p>
                )}
