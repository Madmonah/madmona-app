          ...
          {entries.map(e => (
            <article key={e.id} style={cardStyle}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', gap: 12 }}>
                <h3 style={{ margin: 0, color: '#059669', fontSize: 16 }}>{e.title}</h3>
              ...
              {(e.blocker || e.next_steps) && (
                <div style={{ marginTop: 8, fontSize: 12 }}>
                  ...
                  {e.next_steps && <div style={{ color: '#059669' }}><strong>next:</strong> {e.next_steps}</div>}
                </div>
              )}
            </article>
          ))}
...
function Header({ title, subtitle }: { title: string; subtitle: string }) {
  return (
    <header style={{ marginBottom: 20, textAlign: 'center' }}>
      <h1 style={{ color: '#059669', margin: 0, fontSize: 26 }}>{title}</h1>
