    ...
    <div dir="rtl" style={{ fontFamily: 'Tahoma', background: '#FAF7F0', minHeight: '100vh', padding: 24 }}>
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
          <h1 style={{ color: '#1F5F3F', margin: 0, fontSize: 26 }}>🧠 Strategy Plays</h1>
          ...
          <div style={{ display: 'grid', gap: 16 }}>
            {all.map(play => (
              <div key={play.id} style={{
                ...
                borderRight: `4px solid ${play.priority === 'urgent' ? '#C2410C' : play.priority === 'high' ? '#B8860B' : '#1F5F3F'}`,
              }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: 8 }}>
                  <h2 style={{ margin: 0, color: '#1F5F3F', fontSize: 18 }}>{play.title}</h2>
                  <div style={{ display: 'flex', gap: 6 }}>
                    ...
                    <span style={{
                      ...
                      color: play.priority === 'urgent' ? '#C2410C' : '#856404',
                ...
                {play.steps && play.steps.length > 0 && (
                  <details style={{ marginTop: 12 }}>
                    <summary style={{ cursor: 'pointer', color: '#1F5F3F', fontSize: 13 }}>
