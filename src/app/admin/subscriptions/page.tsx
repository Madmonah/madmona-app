    ...
    <div className="min-h-screen bg-[#FAFAF7]" dir="rtl">
      <header className="bg-white border-b border-gray-100 sticky top-0 z-10">
        <div className="max-w-6xl mx-auto px-4 py-4">
          <Link href="/admin/business-partners" className="text-xs font-bold text-[#6B7280] hover:text-[#1F6F5F] flex items-center gap-1 mb-2">
            <ChevronLeft className="w-3.5 h-3.5" /> رجوع للشركاء
          </Link>
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              <p className="text-[10px] font-bold tracking-[0.3em] uppercase text-[#1F6F5F] mb-1">ADMIN · SUBSCRIPTIONS</p>
                ...
                <div key={r.supplier_id} className={`bg-white rounded-2xl border p-4 flex items-center gap-3 flex-wrap ${isSusp ? 'border-red-200' : 'border-gray-100'}`}>
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-[11px] text-[#6B7280] mt-1 flex items-center gap-3 flex-wrap">
                      ...
                      <Link href={`/admin/business-finance/${r.supplier_id}`} className="text-[#1F6F5F] font-bold">فتح اللوحة ↗</Link>
                    </div>
                  </div>
                  {isSusp ? (
                    <button onClick={() => reactivate(r.supplier_id, r.business_name)} disabled={busy === r.supplier_id} className="px-4 py-2 rounded-xl bg-[#1F6F5F] text-white text-sm font-bold flex items-center gap-2 disabled:opacity-50">
