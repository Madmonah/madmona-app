...
export default function WaNumbersPage() {
  ...
  // ── حالات الدخول ──────────────────────────────────────────────────────
  if (stage === 'loading') {
    return <div className="min-h-screen grid place-items-center"><Loader2 className="w-8 h-8 animate-spin text-[#2B4521]" /></div>
  }
    ...
    return (
      <div className="min-h-screen grid place-items-center px-6" dir="rtl">
        <div className="text-center"><Lock className="w-12 h-12 mx-auto mb-4 text-gray-400" />
          ...
          <Link href="/auth/login" className="text-[#2B4521] font-bold">تسجيل الدخول ←</Link>
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-[#FAFAF7] px-4 py-8 md:px-8" dir="rtl">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl md:text-3xl font-black flex items-center gap-2">
              <Smartphone className="w-7 h-7 text-[#2B4521]" /> أرقام المارد
            </h1>
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="text-xs mt-1 flex items-center gap-1.5" dir="ltr">
                    <Globe className={`w-3.5 h-3.5 ${s.proxy ? 'text-[#2B4521]' : 'text-gray-400'}`} />
                    <span className={s.proxy ? 'text-[#2B4521] font-bold' : 'text-gray-400'}>
              ...
              <div className="flex items-center gap-2">
                ...
                {!s.connected && (
                  <button onClick={() => setQrFor(s.id)}
                    className="flex items-center gap-1.5 bg-[#2B4521] text-white px-3 py-2 rounded-xl text-sm font-bold">
        ...
        <div className="bg-white rounded-2xl p-5 shadow-sm">
          <h2 className="font-black mb-4 flex items-center gap-2"><Plus className="w-5 h-5 text-[#2B4521]" /> إضافة رقم</h2>
          <div className="grid md:grid-cols-3 gap-3">
            ...
            <button onClick={addNumber} disabled={adding}
              className="bg-[#2B4521] text-white rounded-xl px-4 py-3 font-bold text-sm disabled:opacity-50">
        ...
        {editFor && (
          <div className="fixed inset-0 bg-black/60 grid place-items-center z-50 p-4" onClick={() => setEditFor(null)}>
            <div className="bg-white rounded-3xl overflow-hidden max-w-lg w-full" onClick={(e) => e.stopPropagation()}>
              <div className="flex items-center justify-between p-4 border-b">
                <h3 className="font-black flex items-center gap-2"><Settings2 className="w-5 h-5 text-[#2B4521]" /> إعداد +{editFor}</h3>
              ...
              <div className="p-5 space-y-4">
                ...
                <div className="border-t pt-4">
                  <label className="block text-sm font-bold mb-1 flex items-center gap-2">
                    <Globe className="w-4 h-4 text-[#2B4521]" /> قناة الخروج (الـIP)
                  </label>
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    <button onClick={() => editFor && saveProxy(editFor, draftProxy.trim())}
                      ...
                      className="bg-[#2B4521] text-white px-4 py-2 rounded-xl text-sm font-bold disabled:opacity-40">
              ...
              <div className="p-4 border-t flex justify-end gap-2">
                ...
                <button onClick={saveConfig} disabled={savingCfg}
                  className="bg-[#2B4521] text-white px-5 py-2 rounded-xl text-sm font-bold disabled:opacity-50">
                  {savingCfg ? 'بيحفظ…' : 'حفظ'}
                </button>
              </div>
            </div>
        ...
        <div className="mt-8 text-center">
          <Link href="/admin" className="text-[#2B4521] font-bold text-sm inline-flex items-center gap-1">
