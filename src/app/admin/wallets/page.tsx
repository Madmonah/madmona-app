    ...
    return (
      <div dir="rtl" className="min-h-screen grid place-items-center bg-[#FAF7F2] px-6">
        <form onSubmit={handleLogin} className="bg-white rounded-3xl shadow-soft p-6 w-full max-w-sm text-center">
          <Wallet className="w-10 h-10 mx-auto text-[#059669] mb-2" />
          ...
          <div className="relative mb-3">
            ...
            <input type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="كلمة سر الأدمن"
              className="w-full border border-gray-200 rounded-2xl pr-9 pl-4 py-3 outline-none focus:border-[#059669]" />
          </div>
          {authError && <p className="text-rose-500 text-sm mb-3">{authError}</p>}
          <button className="w-full bg-[#34D399] text-[#04352A] py-3 rounded-2xl font-bold">دخول</button>
        </form>
      </div>
    )
  }
    ...
    <div dir="rtl" className="min-h-screen bg-[#FAF7F2] pb-16">
      ...
      <div className="sticky top-0 z-30 bg-white border-b border-gray-100">
        <div className="max-w-3xl mx-auto flex items-center gap-3 px-4 py-3">
          ...
          <h1 className="font-bold text-lg flex items-center gap-2"><Wallet className="w-5 h-5 text-[#059669]" /> المحافظ</h1>
      ...
      <div className="max-w-3xl mx-auto px-4 pt-4 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-white rounded-2xl shadow-soft p-4">
            ...
            <p className="text-2xl font-black text-[#059669]">{formatMoney(totals.cash)}</p>
          </div>
          <div className="bg-white rounded-2xl shadow-soft p-4">
        ...
        <div className="flex gap-2">
          <button onClick={() => setTab('wallets')} className={`flex-1 py-2 rounded-2xl font-bold text-sm ${tab === 'wallets' ? 'bg-[#34D399] text-[#04352A]' : 'bg-white text-gray-600'}`}>المحافظ ({wallets.length})</button>
          <button onClick={() => setTab('withdrawals')} className={`flex-1 py-2 rounded-2xl font-bold text-sm ${tab === 'withdrawals' ? 'bg-[#34D399] text-[#04352A]' : 'bg-white text-gray-600'}`}>
          ...
          <>
            <div className="relative">
              ...
              <input value={q} onChange={e => setQ(e.target.value)} onKeyDown={e => e.key === 'Enter' && fetchData(password)}
                placeholder="ابحث بالاسم أو رقم الموبايل" className="w-full bg-white border border-gray-200 rounded-2xl pr-9 pl-4 py-2.5 outline-none focus:border-[#059669]" />
            </div>
              ...
              {wallets.map(w => (
                <div key={w.id} className="bg-white rounded-2xl shadow-soft p-4">
                  <div className="flex items-start justify-between">
                    ...
                    <button onClick={() => setAdjustFor(w)} className="text-xs bg-[#34D399]/10 text-[#059669] font-bold px-3 py-1.5 rounded-xl flex items-center gap-1">
                      <BadgeDollarSign className="w-3.5 h-3.5" /> تعديل
                    </button>
                  </div>
                  <div className="flex gap-4 mt-3 text-sm">
                    <span className="font-bold text-[#059669]">نقدي: {formatMoney(w.balance_cash, w.currency)}</span>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40 p-0 sm:p-4" onClick={onClose}>
      <div dir="rtl" className="bg-white w-full sm:max-w-md rounded-t-3xl sm:rounded-3xl p-5" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex gap-2 mb-3">
          <button onClick={() => setKind('cash')} className={`flex-1 py-2 rounded-xl font-bold text-sm ${kind === 'cash' ? 'bg-[#34D399] text-[#04352A]' : 'bg-gray-100 text-gray-600'}`}>نقدي</button>
        ...
        <div className="flex gap-2">
          ...
          <button disabled={busy} onClick={submit} className="flex-1 py-3 rounded-2xl font-bold bg-[#34D399] text-[#04352A] flex items-center justify-center gap-2 disabled:opacity-60">
