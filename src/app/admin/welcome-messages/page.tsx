  ...
  if (stage === 'unauthenticated') {
    return <div className="min-h-screen bg-[#FAFAF7] flex items-center justify-center p-4" dir="rtl">
      <div className="bg-white rounded-3xl p-8 text-center max-w-sm">
        ...
        <Link href="/auth/login?redirect=/admin/welcome-messages" className="block bg-[#1F6F5F] text-white py-3 rounded-xl font-bold mt-3">
          دخول
        </Link>
      </div>
    </div>
      ...
      <header className="bg-white border-b border-gray-100 sticky top-0 z-30 backdrop-blur-md bg-white/95">
        <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[10px] font-bold tracking-[0.3em] uppercase text-[#1F6F5F]">MADMONA · WELCOME</p>
      ...
      <main className="max-w-6xl mx-auto px-4 py-6 space-y-6 pb-12">
        {feedback && (
          <div className={`rounded-2xl p-3.5 text-sm font-bold flex items-center gap-2 ${
            feedback.type === 'success' ? 'bg-[#1F6F5F]/10 text-[#1F6F5F]' : 'bg-red-50 text-red-700'
          }`}>
        ...
        <section className="bg-white rounded-2xl border border-gray-100 p-4">
          <div className="flex items-center gap-2 mb-2">
            <TestTube className="w-4 h-4 text-[#1F6F5F]" />
          ...
          <input type="email" value={testEmail} onChange={(e) => setTestEmail(e.target.value)}
            ...
            className="w-full px-3 py-2.5 rounded-xl border border-gray-200 focus:border-[#1F6F5F] focus:outline-none text-sm font-mono" />
        </section>
              ...
              <div key={t.template_key} className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
                <div className="px-4 py-3 bg-[#FAFAF7] border-b border-gray-100 flex items-center justify-between gap-3 flex-wrap">
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="w-9 h-9 rounded-xl bg-[#1F6F5F]/10 text-[#1F6F5F] flex items-center justify-center flex-shrink-0">
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className={`text-[10px] font-bold px-2 py-1 rounded-full ${
                      t.is_active ? 'bg-[#1F6F5F]/10 text-[#1F6F5F]' : 'bg-gray-100 text-gray-600'
                    ...
                    <button onClick={() => toggleTemplate(t.template_key, t.is_active)} disabled={toggling === t.template_key}
                      className={`text-[10px] font-bold px-3 py-1.5 rounded-lg ${
                        t.is_active ? 'bg-gray-100 text-gray-700 hover:bg-gray-200' : 'bg-[#1F6F5F] text-white hover:bg-[#185547]'
                      } disabled:opacity-50`}>
                    ...
                    <button onClick={() => sendTest(t.template_key)} disabled={testingKey === t.template_key || !testEmail}
                      className="text-[10px] font-bold bg-[#1F6F5F] text-white px-3 py-1.5 rounded-lg hover:bg-[#185547] flex items-center gap-1 disabled:opacity-50">
                    ...
                    <div key={i} className="px-4 py-2.5 flex items-center justify-between gap-2">
                      ...
                      <div className="text-left flex-shrink-0">
                        <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${
                          r.status === 'sent' ? 'bg-[#1F6F5F]/10 text-[#1F6F5F]' :
    ...
    <div className="bg-white rounded-2xl border border-gray-100 p-4">
      <div className="flex items-start justify-between gap-2 mb-3">
        <div className="flex items-center gap-2.5 min-w-0">
          <div className="w-9 h-9 rounded-xl bg-[#1F6F5F]/10 text-[#1F6F5F] flex items-center justify-center flex-shrink-0">{icon}</div>
          <div className="min-w-0">
...
function TriggerStatus({ label, exists, fix, warning, note }: { label: string; exists: boolean; fix?: string; warning?: string; note?: string }) {
  return (
    <div className="flex items-start gap-2.5">
      <div className={`w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 ${exists ? 'bg-[#1F6F5F]/10 text-[#1F6F5F]' : 'bg-red-50 text-red-600'}`}>
      ...
      <div className="flex-1 min-w-0">
        ...
        {note && <p className="text-[11px] text-[#1F6F5F] mt-0.5">✓ {note}</p>}
...
function QuickLink({ href, icon, title }: { href: string; icon: ReactNode; title: string }) {
  return (
    <Link href={href} className="bg-white rounded-2xl border border-gray-100 hover:border-[#1F6F5F] p-3 transition-colors group">
