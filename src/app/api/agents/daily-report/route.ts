...
function buildReportEmailHtml(report: ReportOutput, m: DailyMetrics): string {
  ...
  <div style="background: #1F5F3F; color: #FAF7F0; padding: 24px; border-radius: 8px; margin-bottom: 24px;">
  ...
  <div style="background: #FAF7F0; padding: 16px 20px; border-right: 4px solid #B8860B; margin-bottom: 24px;">
    <p style="margin: 0; font-size: 15px;">${escapeHtml(report.summary)}</p>
  </div>

  <h3 style="color: #1F5F3F; margin-top: 24px;">📈 أرقام النهارده</h3>
  ...
  <h3 style="color: #1F5F3F; margin-top: 24px;">✅ Wins</h3>
  <ul style="padding-right: 20px;">${wins}</ul>

  <h3 style="color: #C2410C; margin-top: 24px;">⚠️ محتاج انتباه</h3>
  <ul style="padding-right: 20px;">${concerns}</ul>

  <h3 style="color: #1F5F3F; margin-top: 24px;">🎯 Actions لبكره</h3>
  <ul style="padding-right: 20px;">${actions}</ul>

  <h3 style="color: #1F5F3F; margin-top: 24px;">📌 Highlights</h3>
