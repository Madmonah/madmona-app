...
function inline(s: string) {
  return esc(s)
    ...
    .replace(/\[([^\]]+)\]\((https?:\/\/[^)\s]+)\)/g, '<a href="$2" class="text-[#1F6F5F] font-bold underline underline-offset-2">$1</a>')
}
...
export default async function BlogPost({ params }: P) {
    ...
    return (
      <main className="min-h-screen bg-[#FAFAF7] flex items-center justify-center" dir="rtl">
        <div className="text-center">
          ...
          <Link href="/blog" className="text-[#1F6F5F] font-bold text-sm">← رجوع للمدونة</Link>
        </div>
      </main>
    )
  }
  ...
  return (
    <main className="min-h-screen bg-[#FAFAF7]" dir="rtl">
      ...
      <div className="bg-gradient-to-l from-[#14231E] to-[#1F6F5F] text-white px-4 py-10">
      ...
      <article className="max-w-3xl mx-auto px-4 py-8">
        ...
        <div className="mt-6 bg-gradient-to-l from-[#1F6F5F] to-[#2FA084] rounded-2xl p-5 text-white text-center">
          ...
          <Link href="/marketplace" className="inline-block mt-3 bg-white text-[#1F6F5F] font-black text-sm rounded-full px-5 py-2 no-underline">اتفرج على الإعلانات</Link>
        </div>
      </article>
    </main>
  )
