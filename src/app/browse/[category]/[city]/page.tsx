...
export default async function BrowseCityPage({ params }: P) {
  ...
  return (
    <main dir="rtl" className="min-h-screen bg-[#FAFAF7] pb-24" style={{ fontFamily: 'Cairo,sans-serif' }}>
      ...
      <div className="bg-gradient-to-br from-[#FA8125] to-[#F98F2A] text-white px-5 pt-10 pb-8">
        ...
        <div className="grid grid-cols-1 gap-3">
            ...
            <Link key={l.slug} href={`/marketplace/${l.slug}`} className="block bg-white rounded-2xl p-4 shadow-sm no-underline">
              <div className="flex items-center justify-between gap-3">
                ...
                <div className="text-left flex-shrink-0">
                  {l.price ? <p className="text-sm font-black text-[#FA8125]">{Number(l.price).toLocaleString('ar-EG')} ج.م</p> : <p className="text-xs font-bold text-[#d4a017]">السعر عند الطلب</p>}
          ...
          {listings.length === 0 && (
            <div className="bg-white rounded-2xl p-6 text-center text-sm text-gray-600">
              مفيش إعلانات منشورة هنا حاليًا — <Link href="/add-listing" className="text-[#FA8125] font-black">ضيف الليستنج بتاعك</Link> وكن أول واحد.
            </div>
          )}
        </div>
          ...
          <div className="mt-6">
            ...
            <div className="flex flex-wrap gap-2">
              {otherCities.slice(0, 20).map((c) => (
                <Link key={c} href={`/browse/${encodeURIComponent(dec(category))}/${encodeURIComponent(c)}`} className="bg-white border border-[#FA8125]/25 text-[#FA8125] text-xs font-bold rounded-full px-3 py-1.5 no-underline">
                  {catAr} في {c}
                </Link>
              ))}
            </div>
