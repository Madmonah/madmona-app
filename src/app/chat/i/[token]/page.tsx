  ...
  return (
    <div dir="rtl" style={{ minHeight: '100vh', display: 'grid', placeItems: 'center', padding: 24, background: '#F1EEE6', fontFamily: "var(--font-cairo), system-ui, sans-serif" }}>
      <div style={{ background: '#fff', borderRadius: 20, padding: '30px 24px', maxWidth: 380, width: '100%', textAlign: 'center', boxShadow: '0 8px 30px rgba(20,35,30,.12)' }}>
        ...
        {st.s === 'need_login' && (
          <>
            ...
            <button onClick={goLogin}
              style={{ background: 'linear-gradient(118deg,#2B4521,#5A6E3A)', color: '#fff', border: 'none', borderRadius: 999, padding: '13px 30px', fontSize: 15, fontWeight: 900, cursor: 'pointer', fontFamily: 'inherit', width: '100%' }}>
              سجّل دخول وكمّل
            </button>
          </>
        )}
        ...
        {st.s === 'ok' && (
          <>
            <div style={{ fontSize: 18, fontWeight: 900, color: '#2B4521', marginBottom: 8 }}>بقيتوا أصحاب!</div>
        ...
        {st.s === 'err' && (
          <>
            ...
            <button onClick={() => router.push('/chat')}
              style={{ background: '#F1EEE6', color: '#2B4521', border: 'none', borderRadius: 999, padding: '12px 26px', fontSize: 14, fontWeight: 800, cursor: 'pointer', fontFamily: 'inherit' }}>
              افتح الشات
            </button>
          </>
        )}
