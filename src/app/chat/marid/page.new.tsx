    ...
    return (
      <div dir="rtl" style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: `radial-gradient(circle at 30% 20%, ${C.greenLt} 0%, ${C.greenDk} 70%)`, padding: 16, fontFamily: "'Cairo', system-ui, sans-serif" }}>
        ...
        <div style={{ background: '#fff', borderRadius: 22, padding: 30, width: '100%', maxWidth: 380, boxShadow: '0 20px 60px rgba(0,0,0,.35)' }}>
          <div style={{ width: 76, height: 76, borderRadius: '50%', margin: '0 auto 12px', background: `linear-gradient(135deg,${C.gold},${C.green})`, display: 'grid', placeItems: 'center', overflow: 'hidden', boxShadow: '0 8px 24px rgba(250, 129, 37,.35)' }}>
        ...
        {messages.map((m) => {
          if (m.role === 'sys') return (
            <div key={m.id} style={{ textAlign: 'center', margin: '10px 0' }}>
              <span style={{ background: 'rgba(250, 129, 37,.10)', color: C.greenDk, fontSize: 12, padding: '5px 12px', borderRadius: 12, display: 'inline-block', maxWidth: '85%' }}>{m.text}</span>
            </div>
          )
            ...
            <div key={m.id} style={{ display: 'flex', justifyContent: mine ? 'flex-start' : 'flex-end', marginBottom: m.reaction ? 16 : 8, position: 'relative' }}>
              <div style={{ position: 'relative', maxWidth: '82%' }}>
                ...
                >
                  {/* اقتباس الرد */}
                  {q && !m.deleted && (
                    <div style={{ borderInlineStart: `3px solid ${C.gold}`, background: 'rgba(250, 129, 37,.06)', borderRadius: 8, padding: '4px 8px', marginBottom: 6, fontSize: 12.5 }}>
            ...
            <div onClick={(e) => e.stopPropagation()} dir="rtl" style={{ background: '#fff', borderRadius: 18, overflow: 'hidden', width: '100%', maxWidth: 300, boxShadow: '0 20px 50px rgba(0,0,0,.35)' }}>
              ...
              {!m.deleted && (
                <div style={{ display: 'flex', justifyContent: 'space-around', padding: '10px 8px', borderBottom: `1px solid ${C.line}`, background: C.cream }}>
                  {REACTIONS.map((e) => (
                    <button key={e} onClick={() => actReact(m, e)} style={{ border: 'none', background: m.reaction === e ? 'rgba(250, 129, 37,.14)' : 'none', borderRadius: '50%', width: 38, height: 38, fontSize: 22, cursor: 'pointer', transition: 'transform .1s' }}>{e}</button>
                  ))}
                </div>
              )}
              {/* الأكشنز */}
...
const btnMain: React.CSSProperties = { background: `linear-gradient(135deg,${C.greenLt},${C.green})`, color: '#fff', border: 'none', borderRadius: '50%', height: 48, fontSize: 18, fontWeight: 700, cursor: 'pointer', boxShadow: '0 3px 10px rgba(250, 129, 37,.35)' }
