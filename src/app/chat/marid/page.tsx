...
// هوية مضمونة، مش واتساب ولا تليجرام: هيدر متدرّج غامق #14231E→#1F6F5F،
...
function linkify(text: string, onDark = false): React.ReactNode[] {
  return (text || '').split(/(https?:\/\/[^\s]+)/g).map((p, i) =>
    /^https?:\/\//.test(p) ? (
      <a key={i} href={p} target="_blank" rel="noopener noreferrer" style={{ color: onDark ? '#CDEFE2' : '#1F6F5F', fontWeight: 600, textDecoration: 'underline', wordBreak: 'break-all' }}>{p}</a>
    ) : (
      <span key={i}>{p}</span>
    )
  )
...
export default function ChatPage() {
  ...
  return (
    <div dir="rtl" style={{ height: '100dvh', display: 'flex', flexDirection: 'column', background: '#F1EEE6', fontFamily: "var(--font-cairo), system-ui, sans-serif" }}>
      ...
      {/* ─── الهيدر: متدرّج غامق #14231E→#1F6F5F + شريط المثبّتات جوّاه ─── */}
      <header style={{ background: 'linear-gradient(135deg,#14231E,#1F6F5F)', color: '#fff', paddingBottom: 12, position: 'relative', zIndex: 25, boxShadow: '0 2px 14px rgba(20,35,30,.28)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '10px 16px 0' }}>
          ...
          <span style={{ position: 'relative', flexShrink: 0 }}>
            <span style={{ width: 42, height: 42, borderRadius: '50%', background: 'radial-gradient(circle at 35% 30%,#2FA084,#1F6F5F)', border: '2px solid rgba(255,255,255,.35)', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
              {/* eslint-disable-next-line @next/next/no-img-element */}
        ...
        <div style={{ maxHeight: 180, overflowY: 'auto', background: '#fff', borderBottom: '1px solid rgba(0,0,0,.06)' }}>
          {pinnedList.map((pm) => (
            <div key={pm.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 14px', borderBottom: '1px solid #F4F1E8', fontSize: 13 }}>
              <span style={{ fontSize: 10.5, fontWeight: 800, color: pm.role === 'user' ? '#2FA084' : '#1F6F5F', flexShrink: 0 }}>{pm.role === 'user' ? 'انت' : 'المارد'}</span>
      ...
      <div
        ...
        style={{ flex: 1, overflowY: 'auto', padding: '16px 14px 8px', background: '#F1EEE6', backgroundImage: 'radial-gradient(circle at 1.5px 1.5px, rgba(31,111,95,.07) 1.5px, transparent 0)', backgroundSize: '26px 26px' }}
      >
        {visible.map((m, i) => {
          ...
          return (
            <Fragment key={m.id}>
              ...
              <div style={{ display: 'flex', gap: 8, alignItems: 'flex-end', justifyContent: 'flex-start', marginBottom: m.reactions?.length ? 18 : 10 }}>
                {!mine && (
                  <span style={{ width: 28, height: 28, borderRadius: '50%', background: 'radial-gradient(circle at 35% 30%,#2FA084,#1F6F5F)', overflow: 'hidden', flexShrink: 0 }}>
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                ...
                <div style={{ position: 'relative', maxWidth: mine ? '78%' : '85%', marginRight: mine ? 36 : 0, minWidth: 0 }}>
                  <div
                    ...
                    style={{
                      background: m.deleted ? '#ECE9DF' : mine ? 'linear-gradient(118deg,#1F6F5F,#2d7a52)' : '#fff',
                      color: m.deleted ? '#8A9690' : mine ? '#fff' : '#14231E',
                      borderRadius: mine ? '18px 18px 5px 18px' : '18px 18px 18px 5px',
                      padding: '11px 14px',
                      boxShadow: m.deleted ? 'none' : mine ? '0 6px 16px -8px rgba(31,111,95,.45)' : '0 1px 2px rgba(20,35,30,.06)',
                  ...
                  >
                    {parent && (
                      <div style={{ background: mine ? 'rgba(255,255,255,.15)' : 'rgba(31,111,95,.06)', borderRight: mine ? '3px solid rgba(255,255,255,.85)' : '3px solid #1F6F5F', borderRadius: 8, padding: '6px 10px', marginBottom: 8 }}>
                        <div style={{ fontSize: 9.5, fontWeight: 800, color: mine ? '#fff' : '#1F6F5F' }}>{parent.role === 'user' ? 'انت' : 'المارد'}</div>
                        ...
                        {url && (
                          <a href={url} target="_blank" rel="noopener noreferrer" style={{ display: 'block', marginTop: 8, textDecoration: 'none', border: mine ? '1px solid rgba(255,255,255,.25)' : '1px solid #EAE5D9', borderRadius: 14, overflow: 'hidden', background: mine ? 'rgba(255,255,255,.12)' : '#FAFAF7' }}>
                            <div style={{ padding: '8px 11px' }}>
                              <div style={{ fontSize: 9.5, fontWeight: 800, color: mine ? '#CDEFE2' : '#1F6F5F' }}>🔗 {domainOf(url)}</div>
      ...
      {maridOn && !sending && !q && !editingId && messages[messages.length - 1]?.role === 'bot' && (
        <div style={{ padding: '8px 14px 0', background: '#F1EEE6' }}>
          <div className="mchat-hs" style={{ display: 'flex', gap: 7, overflowX: 'auto', paddingBottom: 8 }}>
            {QUICKS.map((qq) => (
              <button key={qq} onClick={() => sendQuick(qq)} style={{ flex: 'none', background: '#fff', border: '1.5px solid rgba(31,111,95,.25)', color: '#1F6F5F', fontSize: 11.5, fontWeight: 800, padding: '8px 14px', borderRadius: 999, cursor: 'pointer', fontFamily: 'inherit', whiteSpace: 'nowrap' }}>{qq}</button>
            ))}
          </div>
        </div>
      )}
      ...
      {(replyTo || editingId) && (
        <div style={{ background: '#fff', borderTop: '1px solid rgba(0,0,0,.05)', padding: '8px 12px 0' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'rgba(31,111,95,.06)', borderRight: '3px solid #1F6F5F', borderRadius: 8, padding: '6px 10px' }}>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 9.5, fontWeight: 800, color: '#1F6F5F' }}>{editingId ? '✏️ تعديل الرسالة' : `↩️ رد على ${replyTo?.role === 'user' ? 'نفسك' : 'المارد'}`}</div>
            ...
            <button onClick={() => { setReplyTo(null); setEditingId(null); if (editingId) setInput('') }} aria-label="إلغاء" style={{ border: 'none', background: 'none', color: '#1F6F5F', cursor: 'pointer', fontSize: 17, lineHeight: 1, flexShrink: 0 }}>✕</button>
          </div>
        </div>
      )}
        ...
        <div onClick={() => setForwardMsg(null)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.45)', display: 'grid', placeItems: 'center', zIndex: 50, padding: 20 }}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: '#fff', borderRadius: 18, padding: 20, width: '100%', maxWidth: 340, boxShadow: '0 18px 50px rgba(0,0,0,.3)' }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={() => doForward(forwardMsg)} style={{ flex: 1, background: 'linear-gradient(135deg,#1F6F5F,#2FA084)', color: '#fff', border: 'none', borderRadius: 12, padding: '11px', fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>حوّل هنا</button>
            </div>
          </div>
        </div>
      )}
...
const typDot: React.CSSProperties = { width: 6, height: 6, borderRadius: '50%', background: '#1F6F5F', display: 'inline-block' }
