      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: 12 }}>
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: 40 }}>
            ...
            <button onClick={() => setNonce((n) => n + 1)} style={{ background: '#1F6F5F', color: '#fff', border: 'none', borderRadius: 999, padding: '9px 18px', fontWeight: 800, cursor: 'pointer', fontFamily: 'inherit' }}>🔄 جرّب تاني</button>
          </div>
        ) : (
            ...
            {offers.map((o) => (
              <Link key={o.id} href={`/marketplace/${o.slug}`} style={{ textDecoration: 'none', background: '#fff', borderRadius: 16, overflow: 'hidden', border: '1px solid #EAE5D9', display: 'block', boxShadow: '0 1px 2px rgba(20,35,30,.06)' }}>
                ...
                <div style={{ padding: 10 }}>
                  ...
                  {o.price != null && <div style={{ fontSize: 13, color: '#1F6F5F', fontWeight: 900, marginTop: 2 }}>{o.price.toLocaleString('ar-EG')} ج.م</div>}
                </div>
              </Link>
            ))}
          </div>
