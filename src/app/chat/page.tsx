    ...
    <div dir="rtl" style={{ height: '100dvh', display: 'flex', flexDirection: 'column', background: '#FAFAF7', fontFamily: "var(--font-cairo), system-ui, sans-serif" }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '14px 16px 4px' }}>
          <div style={{ flex: 1, fontSize: 11, fontWeight: 900, color: '#FA8125', letterSpacing: '.3px' }}>محادثاتك الخاصة</div>
          ...
          <button onClick={() => setShowNewDM(true)} style={{ background: '#F1EEE6', color: '#FA8125', borderRadius: 999, padding: '5px 12px', fontSize: 11.5, fontWeight: 800, border: 'none', cursor: 'pointer', fontFamily: 'inherit' }}>➕ محادثة جديدة</button>
        </div>
          ...
          rooms.filter((r) => !hidden.has(r.id)).map((r) => (
            <div key={r.id} style={{ ...rowStyle, paddingLeft: 6 }}>
              <Link href={`/chat/team?room=${r.id}`} style={{ display: 'flex', alignItems: 'center', gap: 12, flex: 1, minWidth: 0, textDecoration: 'none' }}>
                <div style={{ ...avatarStyle, background: 'radial-gradient(circle at 35% 30%,#2FA084,#FA8125)', color: '#fff', display: 'grid', placeItems: 'center', fontSize: 18, fontWeight: 800 }}>{(r.name || '؟').trim().charAt(0)}</div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8 }}>
