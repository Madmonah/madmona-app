...
export default function ChatSettings() {
  ...
  return (
    <div dir="rtl" style={{ height: '100vh', display: 'flex', flexDirection: 'column', background: '#FAFAF7', fontFamily: "var(--font-cairo), system-ui, sans-serif" }}>
      <header style={{ background: 'linear-gradient(135deg,#14231E,#059669)', color: '#fff', padding: '14px 16px', fontSize: 17, fontWeight: 900, boxShadow: '0 2px 14px rgba(20,35,30,.28)' }}>الإعدادات ⚙️</header>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: 16 }}>
        ...
        <div style={{ background: '#fff', borderRadius: 16, border: '1px solid #EAE5D9', overflow: 'hidden', marginBottom: 14, boxShadow: '0 1px 2px rgba(20,35,30,.06)' }}>
          <button onClick={toggleNotif} disabled={!notifSupported || busy} style={rowBtn}>
            ...
            <span style={{ fontSize: 13, color: notifOn ? '#059669' : '#8A9690', fontWeight: 800 }}>{!notifSupported ? 'مش مدعوم' : notifOn ? 'مفعّلة' : 'متوقفة'}</span>
          </button>
          {installEvt && (
            <button onClick={installApp} style={{ ...rowBtn, borderTop: '1px solid #F4F1E8' }}>
              ...
              <span style={{ fontSize: 13, color: '#059669', fontWeight: 800 }}>ثبّت</span>
            </button>
          )}
        ...
        {msg && <div style={{ fontSize: 13, color: '#059669', fontWeight: 700, textAlign: 'center', marginBottom: 14 }}>{msg}</div>}
