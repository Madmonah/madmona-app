...
function linkifyText(text: string): React.ReactNode {
  ...
  async function kickMember(m: Member) {
    ...
    // (٩/٩) مالك زيادة (حساب مكرر لنفس الشركة) ينفع يتشال طول ما فيه مالك تاني — نفس قاعدة remove_room_member
    const ownersCount = members.filter(x => x.member_role === 'owner').length
    if (!active || !canKick || m.is_me || (m.member_role === 'owner' && ownersCount <= 1)) return
  ...
  return (
    <div dir="rtl" style={{ height: '100dvh', display: 'flex', flexDirection: 'column', background: '#F1EEE6', fontFamily: "var(--font-cairo), system-ui, sans-serif" }}>
          ...
          <div onClick={(e) => e.stopPropagation()} style={{ background: '#fff', width: '100%', maxHeight: '70vh', overflowY: 'auto', borderRadius: '16px 16px 0 0', padding: 16 }}>
            ...
            {membersLoading ? (
              ...
              <div key={m.member_id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 4px', borderBottom: '1px solid #f2f2f2' }}>
                ...
                {canKick && !m.is_me && (m.member_role !== 'owner' || members.filter(x => x.member_role === 'owner').length > 1) && (
      ...
      <div ref={scrollRef} style={{ flex: 1, overflowY: 'auto', padding: '14px 12px', background: '#F1EEE6', backgroundImage: 'radial-gradient(circle at 1.5px 1.5px, rgba(250, 129, 37,.07) 1.5px, transparent 0)', backgroundSize: '26px 26px' }}>
        {messages.map((m) => {
