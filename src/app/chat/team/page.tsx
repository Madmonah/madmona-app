...
export default function TeamPage() {
  ...
  if (!ready) return <div dir="rtl" style={{ minHeight: '100vh', display: 'grid', placeItems: 'center', background: 'linear-gradient(160deg,#14231E,#2B4521)', color: '#fff', fontFamily: 'system-ui' }}>لحظة…</div>
  ...
  if (!active) return (
    <div dir="rtl" style={{ height: '100dvh', display: 'flex', flexDirection: 'column', background: '#FAFAF7', fontFamily: "var(--font-cairo), system-ui, sans-serif" }}>
      <header style={{ background: 'linear-gradient(135deg,#14231E,#2B4521)', color: '#fff', padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 10, boxShadow: '0 2px 14px rgba(20,35,30,.28)' }}>
            ...
            {starredMsgs.length === 0 ? (
              ...
              <div key={sm.id} style={{ borderBottom: '1px solid #F1EEE6', padding: '10px 2px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, marginBottom: 2 }}>
                  <span style={{ fontSize: 11, fontWeight: 800, color: '#2B4521' }}>{sm.sender_name || 'عضو'}</span>
  ...
  return (
    <div dir="rtl" style={{ height: '100dvh', display: 'flex', flexDirection: 'column', background: '#F1EEE6', fontFamily: "var(--font-cairo), system-ui, sans-serif" }}>
      <header style={{ background: 'linear-gradient(135deg,#14231E,#2B4521)', color: '#fff', padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 10, boxShadow: '0 2px 14px rgba(20,35,30,.28)', zIndex: 2 }}>
        ...
        <div style={{ background: '#fff', borderBottom: '1px solid #EAE5D9', padding: '10px 14px', maxHeight: '52vh', overflowY: 'auto' }}>
          ...
          {searchHits.map((h) => (
            <button key={h.id} onClick={() => jumpTo(h.id)} style={{ display: 'block', width: '100%', textAlign: 'start', background: 'none', border: 'none', borderBottom: '1px solid #F1EEE6', padding: '10px 4px', cursor: 'pointer', fontFamily: 'inherit' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, marginBottom: 2 }}>
                <span style={{ fontSize: 11, fontWeight: 800, color: '#2B4521' }}>{h.sender_name || 'عضو'}</span>
        ...
        <div onClick={() => setContactDraft(null)} style={{ position: 'fixed', inset: 0, background: 'rgba(20,35,30,.55)', zIndex: 97, display: 'flex', alignItems: 'flex-end' }}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: '#fff', width: '100%', borderRadius: '18px 18px 0 0', padding: '16px 18px 22px' }}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={sendContact} style={{ flex: 1, background: 'linear-gradient(118deg,#2B4521,#5A6E3A)', color: '#fff', border: 'none', borderRadius: 999, padding: '11px 0', fontSize: 14, fontWeight: 900, cursor: 'pointer', fontFamily: 'inherit' }}>ابعت</button>
      ...
      {pollDraft && (
        <div onClick={() => setPollDraft(null)} style={{ position: 'fixed', inset: 0, background: 'rgba(20,35,30,.55)', zIndex: 97, display: 'flex', alignItems: 'flex-end' }}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: '#fff', width: '100%', borderRadius: '18px 18px 0 0', padding: '16px 18px 22px', maxHeight: '82vh', overflowY: 'auto' }}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={createPoll} disabled={busy} style={{ flex: 1, background: 'linear-gradient(118deg,#2B4521,#5A6E3A)', color: '#fff', border: 'none', borderRadius: 999, padding: '11px 0', fontSize: 14, fontWeight: 900, cursor: 'pointer', opacity: busy ? 0.6 : 1, fontFamily: 'inherit' }}>انشر الاستبيان</button>
      ...
      <div ref={scrollRef} style={{ flex: 1, overflowY: 'auto', padding: '14px 12px', background: '#F1EEE6', backgroundImage: 'radial-gradient(circle at 1.5px 1.5px, rgba(43, 69, 33,.07) 1.5px, transparent 0)', backgroundSize: '26px 26px' }}>
        {messages.map((m) => {
          ...
          return (
            <div key={m.id} style={{ display: 'flex', justifyContent: mine ? 'flex-start' : 'flex-end', marginBottom: 9 }}>
              <div
                ...
                style={{ maxWidth: '82%', background: mine ? 'linear-gradient(118deg,#2B4521,#5A6E3A)' : (marid ? '#FFF7E0' : '#fff'), color: mine ? '#fff' : '#14231E', padding: '10px 14px', borderRadius: mine ? '18px 18px 5px 18px' : '18px 18px 18px 5px', boxShadow: mine ? '0 6px 16px -8px rgba(43, 69, 33,.45)' : '0 1px 2px rgba(20,35,30,.06)', whiteSpace: 'pre-wrap', wordBreak: 'break-word', fontSize: 14, fontWeight: 600, lineHeight: 1.65, position: 'relative', userSelect: 'none', WebkitUserSelect: 'none', opacity: m.deleted_at ? 0.75 : 1 }}>
                ...
                {m.reply_to && (() => {
                  ...
                  return (
                    <div style={{ borderInlineStart: `3px solid ${mine ? '#8FE3C8' : '#2FA084'}`, background: mine ? 'rgba(255,255,255,.14)' : '#F1EEE6', borderRadius: 8, padding: '5px 8px', marginBottom: 5, fontSize: 12, fontWeight: 600, opacity: 0.95 }}>
                      <div style={{ fontWeight: 800, fontSize: 10.5, color: mine ? '#CDEFE2' : '#2B4521', marginBottom: 1 }}>{src?.sender_name || 'رسالة'}</div>
                  ...
                  <a href={`https://maps.google.com/?q=${m.lat},${m.lng}`} target="_blank" rel="noreferrer" style={{ display: 'block', textDecoration: 'none', marginBottom: 4 }}>
                    <div style={{ borderRadius: 12, overflow: 'hidden', border: `1px solid ${mine ? 'rgba(255,255,255,.25)' : '#EAE5D9'}`, width: 210 }}>
                      ...
                      <div style={{ padding: '7px 9px', background: mine ? 'rgba(255,255,255,.1)' : '#fff' }}>
                        <div style={{ fontSize: 12, fontWeight: 800, color: mine ? '#fff' : '#2B4521' }}>📍 موقع</div>
                ...
                {m.kind === 'contact' && m.payload && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 9, borderRadius: 12, padding: '8px 10px', marginBottom: 4, background: mine ? 'rgba(255,255,255,.14)' : '#F1EEE6', minWidth: 190 }}>
                    <span style={{ width: 34, height: 34, borderRadius: '50%', background: 'radial-gradient(circle at 35% 30%,#2FA084,#2B4521)', color: '#fff', display: 'grid', placeItems: 'center', fontSize: 15, fontWeight: 800, flexShrink: 0 }}>{String(m.payload.name || 'ج').trim()[0]}</span>
                    <div style={{ flex: 1, minWidth: 0 }}>
                ...
                {m.media_url && m.kind !== 'image' && m.kind !== 'video' && m.kind !== 'audio' && (
                  <a href={m.media_url} target="_blank" rel="noreferrer" style={{ display: 'inline-block', marginBottom: m.body ? 4 : 0, color: mine ? '#CDEFE2' : '#2B4521', textDecoration: 'underline', fontSize: 14 }}>📎 ملف مرفق</a>
                )}
                {m.media_url && (
                ...
                {m.body && !['location', 'poll', 'contact'].includes(m.kind) && (
                  <button onClick={() => addTask(m)} title="حوّل الرسالة لمهمة" style={{ display: 'block', marginTop: 4, background: 'none', border: 'none', color: mine ? '#CDEFE2' : '#2B4521', cursor: 'pointer', fontSize: 11, padding: 0, fontWeight: 800, fontFamily: 'inherit' }}>➕ حوّل لمهمة</button>
                )}
                </>
                )}
        ...
        <div style={{ background: '#fff', borderTop: '1px solid rgba(0,0,0,.05)', maxHeight: 190, overflowY: 'auto' }}>
          ...
          {mentionList.map((mm) => (
            <button key={mm.member_id} onClick={() => pickMention(mm)} style={{ display: 'flex', width: '100%', alignItems: 'center', gap: 10, background: 'none', border: 'none', padding: '9px 16px', cursor: 'pointer', textAlign: 'start', fontFamily: 'inherit' }}>
              <span style={{ width: 30, height: 30, borderRadius: '50%', background: 'radial-gradient(circle at 35% 30%,#2FA084,#2B4521)', color: '#fff', display: 'grid', placeItems: 'center', fontSize: 13, fontWeight: 800, flexShrink: 0 }}>{(mm.member_name || 'ع').trim()[0]}</span>
      ...
      {(replyTo || editing) && (
        <div style={{ background: '#fff', borderTop: '1px solid rgba(0,0,0,.05)', padding: '8px 14px 0', display: 'flex', alignItems: 'center', gap: 8 }}>
          <div style={{ flex: 1, borderInlineStart: '3px solid #2FA084', background: '#F1EEE6', borderRadius: 8, padding: '6px 9px', minWidth: 0 }}>
            <div style={{ fontSize: 10.5, fontWeight: 800, color: '#2B4521' }}>{editing ? '✏️ بتعدّل' : `↩️ رد على ${replyTo?.sender_name || 'رسالة'}`}</div>
