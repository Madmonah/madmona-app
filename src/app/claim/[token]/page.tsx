...
const GREEN_GRAD = 'linear-gradient(135deg,#143A33 0%,#1F6F5F 52%,#2FA084 100%)'
const GOLD_GRAD = 'linear-gradient(120deg,#d4a017,#2FA084,#1F6F5F)'
  ...
  return (
    <div dir="rtl" style={{ minHeight: '100vh', background: '#FAFAF7', fontFamily: 'Cairo, system-ui, sans-serif', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '0 0 48px' }}>
      ...
      <div style={{ width: '100%', maxWidth: 520, padding: '0 16px', marginTop: 22 }}>
        {loading && (
          <div style={{ textAlign: 'center', color: '#1F6F5F', fontWeight: 700, padding: '60px 0' }}>جاري التحميل…</div>
        )}
        ...
        {!loading && !invalid && data && (
          <div style={cardStyle}>
            ...
            {data.cuisine && <div style={{ color: '#1F6F5F', fontWeight: 700, marginBottom: 14 }}>{data.cuisine}</div>}
            ...
            {claimed && (
              <div style={{ textAlign: 'center', paddingTop: 6 }}>
                ...
                <h2 style={{ fontWeight: 900, color: '#1F6F5F', margin: '0 0 6px' }}>تم استلام صفحتك!</h2>
        ...
        <div style={{ textAlign: 'center', color: '#9aa', fontSize: 13, marginTop: 22 }}>
          معاملاتك مضمونة • <a href="https://www.madmonacairo.com" style={{ color: '#1F6F5F', textDecoration: 'none', fontWeight: 700 }}>madmonacairo.com</a>
        </div>
      </div>
    </div>
  )
...
const ghostBtn: React.CSSProperties = { display: 'inline-block', background: '#fff', color: '#1F6F5F', fontWeight: 800, fontSize: 16, padding: '12px 22px', borderRadius: 999, textDecoration: 'none', border: '1.5px solid #1F6F5F', textAlign: 'center' }
