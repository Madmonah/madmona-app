      ...
      <main className="max-w-2xl mx-auto px-4 py-6">
        {step === 'doctors' && (
          <div className="space-y-5">
            <div className="bg-[#1F6F5F]/5 border border-[#1F6F5F]/20 rounded-2xl p-4 flex items-start gap-2">
            ...
            {specialties.map((sp: any) => (
              <section key={sp.specialty}>
                <h2 className="text-sm font-black text-[#1A2E26] mb-2 flex items-center gap-1.5"><Stethoscope className="w-4 h-4 text-[#1F6F5F]" /> {sp.specialty}</h2>
                <div className="space-y-2">
                  {sp.doctors.map((doc: any) => (
                    <div key={doc.employee_id} className="bg-white rounded-2xl border border-gray-100 p-4">
                      <div className="flex items-start gap-3">
                        <div className="w-12 h-12 rounded-xl bg-[#1F6F5F]/10 text-[#1F6F5F] grid place-items-center font-black text-lg flex-shrink-0">{doc.avatar_initial || 'د'}</div>
                        <div className="flex-1 min-w-0">
                          <p className="font-black text-[#1A2E26]">{doc.name}</p>
                          <p className="text-[12px] text-[#1F6F5F] font-bold">{doc.title}</p>
                          ...
                          {(doc.accepted_insurance || []).length > 0 && (
                            <div className="flex items-center flex-wrap gap-1.5 mt-1.5">
                              <BadgeCheck className="w-3.5 h-3.5 text-[#1F6F5F]" />
                              {doc.accepted_insurance.map((ins: string) => (
                                <span key={ins} className="text-[10px] font-bold text-[#1F6F5F] bg-[#1F6F5F]/10 rounded-full px-2 py-0.5">{ins}</span>
                              ))}
                            </div>
                          )}
                        </div>
                      ...
                      <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-100">
                        <div>
                          ...
                          <p className="font-mono font-black text-[#1F6F5F] text-lg">{fmt(doc.consultation_fee_egp)} ج</p>
                        </div>
                        <button
                          onClick={() => { setDoctor(doc); setDate(null); setTime(null); setStep('datetime') }}
                          disabled={!doc.consultation_service_id}
                          className="px-5 py-2.5 rounded-xl bg-[#1F6F5F] text-white font-black text-sm flex items-center gap-1.5 disabled:opacity-40"
                        >
        ...
        {step === 'datetime' && doctor && (
          <div className="space-y-4">
            <button onClick={() => setStep('doctors')} className="text-xs font-bold text-[#6B7280] hover:text-[#1F6F5F] flex items-center gap-1"><ArrowLeft className="w-3.5 h-3.5" /> رجوع للأطباء</button>
            <div className="bg-white rounded-2xl border border-gray-100 p-3 flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-[#1F6F5F]/10 text-[#1F6F5F] grid place-items-center font-black">{doctor.avatar_initial || 'د'}</div>
              <div className="flex-1 min-w-0"><p className="font-black text-[#1A2E26] text-sm">{doctor.name}</p><p className="text-[11px] text-[#1F6F5F]">{doctor.title} · {doctor.branch_name}</p></div>
              <p className="font-mono font-black text-[#1F6F5F]">{fmt(fee)} ج</p>
            </div>
            ...
            <div className="flex gap-2 overflow-x-auto pb-2">
              {dateOptions.map(d => {
                ...
                return (
                  <button key={d.toISOString()} onClick={() => { setDate(d); setTime(null) }} className={`flex-shrink-0 px-3 py-2 rounded-xl text-center min-w-[60px] transition-all ${sel ? 'bg-[#1F6F5F] text-white' : 'bg-white text-[#1A2E26] border border-gray-100 hover:border-[#1F6F5F]'}`}>
              ...
              <div className="bg-white rounded-2xl border border-gray-100 p-4">
                ...
                {loadingSlots ? <div className="py-6 text-center"><Loader2 className="w-5 h-5 text-[#1F6F5F] animate-spin inline" /></div> : (
                  <div className="grid grid-cols-3 md:grid-cols-4 gap-2">
                    ...
                    {slots.map((s: any) => (
                      <button key={s.time} disabled={!s.available} onClick={() => { setTime(s.time); setStep('info') }} className={`px-3 py-2 rounded-lg text-sm font-bold transition-all ${!s.available ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : time === s.time ? 'bg-[#1F6F5F] text-white' : 'bg-[#FAFAF7] text-[#1A2E26] hover:bg-[#1F6F5F]/10'}`}>{s.time.slice(0, 5)}</button>
                    ))}
                  </div>
                )}
              </div>
        ...
        {step === 'info' && (
          <div className="space-y-4">
            <button onClick={() => setStep('datetime')} className="text-xs font-bold text-[#6B7280] hover:text-[#1F6F5F] flex items-center gap-1"><ArrowLeft className="w-3.5 h-3.5" /> رجوع</button>
            ...
            <button onClick={() => setStep('confirm')} disabled={!name || !phone} className="w-full py-3 rounded-xl bg-[#1F6F5F] text-white font-black disabled:opacity-40 flex items-center justify-center gap-2">متابعة <ChevronLeft className="w-4 h-4" /></button>
          </div>
        )}

        {step === 'confirm' && doctor && (
          <div className="space-y-4">
            <button onClick={() => setStep('info')} className="text-xs font-bold text-[#6B7280] hover:text-[#1F6F5F] flex items-center gap-1"><ArrowLeft className="w-3.5 h-3.5" /> رجوع</button>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-5 space-y-3">
              ...
              <div className="pt-3 border-t border-gray-100 space-y-1">
                <div className="flex justify-between items-center"><span className="text-sm font-bold text-[#1A2E26]">سعر الكشف</span><span className="font-mono font-black text-2xl text-[#1F6F5F]">{fmt(fee)} ج</span></div>
                <div className="mt-3 rounded-xl bg-[#1F6F5F]/5 border border-[#1F6F5F]/20 p-3">
                  <div className="flex justify-between items-center"><span className="text-xs font-bold text-[#1A2E26]">عربون جدية (٥٪ غير مسترد)</span><span className="font-mono font-black text-[#1F6F5F]">{fmt(deposit)} ج</span></div>
        ...
        {step === 'done' && result && doctor && (
          <div className="space-y-4 text-center py-8">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-5 space-y-3 text-right">
              ...
              <Row icon={<MapPin className="w-4 h-4" />} label="الفرع" value={doctor.branch_name} />
              <div className="pt-3 border-t border-gray-100 flex justify-between"><span className="text-sm font-bold">سعر الكشف</span><span className="font-mono font-black text-[#1F6F5F]">{fmt(result.price)} ج</span></div>
              {result.deposit_required && (
                <div className="pt-3 border-t border-gray-100 text-right">
                  <div className="flex justify-between items-center"><span className="text-sm font-bold text-[#1A2E26]">عربون الجدية (٥٪)</span><span className="font-mono font-black text-[#1F6F5F]">{fmt(result.deposit_egp)} ج</span></div>
            ...
            <a href={`https://wa.me/${WA}?text=${encodeURIComponent(`السلام عليكم، أنا ${name}، حجزت كشف مع ${doctor.name} يوم ${date?.toLocaleDateString('ar-EG')} الساعة ${time?.slice(0, 5)}`)}`} target="_blank" rel="noopener" className="block w-full py-3 rounded-xl bg-[#1F6F5F] text-white font-black flex items-center justify-center gap-2"><MessageCircle className="w-4 h-4" /> تواصل واتساب لتأكيد العربون</a>
          </div>
        )}
      </main>
    </div>
...
function Row({ icon, label, value, sub }: any) {
  return (
    <div className="flex items-center gap-3">
      <div className="w-9 h-9 rounded-xl bg-[#FAFAF7] grid place-items-center text-[#1F6F5F]">{icon}</div>
      <div className="flex-1 min-w-0">
