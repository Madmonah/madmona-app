...
export default function ClockPage({ params }: { params: { branchCode: string } }) {
  ...
  if (loading) return <div className="min-h-screen bg-[#34D399] flex items-center justify-center"><Loader2 className="w-9 h-9 text-white animate-spin" /></div>
...
function ResultCard({ result, onDone }: any) {
  ...
  return (
    <div className="bg-white rounded-3xl border border-gray-100 p-7 text-center shadow-[0_10px_40px_-12px_rgba(250, 129, 37,0.25)]">
      <div className={`w-20 h-20 rounded-2xl grid place-items-center mx-auto mb-4 ${isIn ? 'bg-[#34D399]' : 'bg-[#1A2E26]'} text-[#04352A]`}>
        {isIn ? <LogIn className="w-10 h-10" /> : <LogOut className="w-10 h-10" />}
      </div>
      <p className="text-[13px] font-bold text-[#059669] mb-1">{isIn ? 'تم تسجيل الحضور ✅' : (result.auto ? 'تم تسجيل خروجك تلقائياً 👋' : 'تم تسجيل الانصراف 👋')}</p>
      ...
      <div className="mt-5 bg-[#FAFAF7] rounded-2xl p-4 space-y-2">
        ...
        {!isIn && result.hours_worked != null && (
          <div className="flex items-center justify-between border-t border-gray-200 pt-2">
            ...
            <span className="font-mono font-black text-[#059669]" dir="ltr">{result.hours_worked} س</span>
          </div>
        )}
        {result.distance_m != null && (
          <div className="flex items-center justify-between border-t border-gray-200 pt-2">
            <span className="text-[12px] text-[#6B7280] flex items-center gap-1.5"><CheckCircle2 className="w-4 h-4 text-[#059669]" /> الموقع متأكد</span>
            <span className="font-mono text-[12px] text-[#6B7280]" dir="ltr">{result.distance_m} م</span>
          </div>
        )}
      </div>

      <button onClick={onDone} className="w-full mt-5 py-3.5 rounded-2xl bg-[#34D399] text-[#04352A] font-black text-sm shadow-lg shadow-[#059669]/20">تمام</button>
    </div>
  )
}
...
function SelfViewCard({ view, onToggle, onClose, liveBanner, onSubmitLeave, onSubmitAdvance }: any) {
  ...
  if (!att) {
    ...
    statusBox = (
      <div className="rounded-2xl bg-[#34D399]/8 border border-[#059669]/15 p-4 flex items-center gap-3">
        <div className="w-10 h-10 rounded-xl bg-[#34D399] grid place-items-center"><LogIn className="w-5 h-5 text-white" /></div>
        <div>
          <p className="text-[13px] font-black text-[#059669]">🟢 إنت داخل دلوقتي</p>
  ...
  return (
    <div className="bg-white rounded-3xl border border-gray-100 p-6 shadow-[0_10px_40px_-12px_rgba(250, 129, 37,0.25)]">
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={onClose} className="text-[12px] font-bold text-[#059669] flex items-center gap-1 active:scale-95"><ArrowRight className="w-4 h-4" /> رجوع</button>
      </div>
      ...
      {view.justClockedIn && (
        <div className="mb-3 rounded-2xl bg-[#34D399] text-[#04352A] p-3 text-center">
      ...
      <div className="mt-3 grid grid-cols-2 gap-2">
        <div className="rounded-2xl bg-[#FAFAF7] p-3 text-center">
          ...
          <p className="text-[15px] font-black text-[#059669] mt-0.5" dir="ltr">{Math.round(view.commission_this_month || 0).toLocaleString('en-US')} ج</p>
        ...
        <div className="rounded-2xl bg-[#FAFAF7] p-3 text-center">
          ...
          <p className="text-[15px] font-black text-[#059669] mt-0.5" dir="ltr">{Math.round((view.tips?.month_total) || 0).toLocaleString('en-US')} ج</p>
      ...
      {Array.isArray(view.today) && view.today.length > 0 && (
        <div className="mt-5">
          <p className="text-[13px] font-black text-[#1A2E26] flex items-center gap-1.5 mb-3"><Calendar className="w-4 h-4 text-[#059669]" /> مواعيدك النهاردة</p>
          <div className="space-y-2">
            {view.today.map((b: any) => (
              <div key={b.booking_id} className="rounded-2xl border border-gray-200 p-3 flex items-center gap-3">
                <span className="font-mono font-black text-[#059669] text-[13px]" dir="ltr">{b.time}</span>
      ...
      <div className="mt-5 flex items-center justify-between">
        <p className="text-[13px] font-black text-[#1A2E26] flex items-center gap-1.5"><ClipboardList className="w-4 h-4 text-[#059669]" /> تاسكات النهاردة</p>
      ...
      <div className="mt-6 border-t border-gray-100 pt-4">
        <p className="text-[13px] font-black text-[#1A2E26] flex items-center gap-1.5 mb-3"><Send className="w-4 h-4 text-[#059669]" /> طلباتي</p>

        <div className="grid grid-cols-2 gap-2 mb-3">
          <div className="rounded-2xl bg-[#FAFAF7] p-3 text-center">
            <p className="text-[10px] text-[#6B7280]">رصيد الإجازة السنوية</p>
            <p className="text-[15px] font-black text-[#059669] mt-0.5" dir="ltr">{Math.max(0, (bal.annual_total || 0) - (bal.annual_used || 0))}<span className="text-[10px] text-[#6B7280]"> / {bal.annual_total || 0}</span></p>
          </div>
          <div className="rounded-2xl bg-[#FAFAF7] p-3 text-center">
            <p className="text-[10px] text-[#6B7280]">رصيد العارضة</p>
            <p className="text-[15px] font-black text-[#059669] mt-0.5" dir="ltr">{Math.max(0, (bal.casual_total || 0) - (bal.casual_used || 0))}<span className="text-[10px] text-[#6B7280]"> / {bal.casual_total || 0}</span></p>
          </div>
        </div>

        {!form && (
          <div className="grid grid-cols-2 gap-2">
            <button onClick={() => { setForm('leave'); setReqMsg(null) }} className="h-12 rounded-2xl bg-[#34D399]/10 text-[#059669] font-bold text-[13px] flex items-center justify-center gap-2 active:scale-95 transition-all"><CalendarDays className="w-4 h-4" /> اطلب إجازة</button>
            <button onClick={() => { setForm('advance'); setReqMsg(null) }} className="h-12 rounded-2xl bg-[#34D399]/10 text-[#059669] font-bold text-[13px] flex items-center justify-center gap-2 active:scale-95 transition-all"><Wallet className="w-4 h-4" /> اطلب سلفة</button>
          </div>
        )}

        {form === 'leave' && (
          <div className="rounded-2xl border border-[#059669]/20 p-3 space-y-2.5">
            <div className="flex items-center justify-between">
            ...
            <div className="grid grid-cols-3 gap-1.5">
              {[['annual', 'سنوية'], ['casual', 'عارضة'], ['sick', 'مرضية']].map(([v, l]) => (
                <button key={v} onClick={() => setLvType(v)} className={`h-9 rounded-xl text-[12px] font-bold transition-all ${lvType === v ? 'bg-[#34D399] text-[#04352A]' : 'bg-[#FAFAF7] text-[#6B7280]'}`}>{l}</button>
              ))}
            </div>
            <div className="grid grid-cols-2 gap-2">
            ...
            <button onClick={doLeave} disabled={reqBusy} className="w-full h-11 rounded-2xl bg-[#34D399] text-[#04352A] font-black text-[13px] flex items-center justify-center gap-2 disabled:opacity-50">{reqBusy ? <Loader2 className="w-4 h-4 animate-spin" /> : <Send className="w-4 h-4" />} ابعت الطلب</button>
          </div>
        )}
        ...
        {form === 'advance' && (
          <div className="rounded-2xl border border-[#059669]/20 p-3 space-y-2.5">
            ...
            <button onClick={doAdvance} disabled={reqBusy} className="w-full h-11 rounded-2xl bg-[#34D399] text-[#04352A] font-black text-[13px] flex items-center justify-center gap-2 disabled:opacity-50">{reqBusy ? <Loader2 className="w-4 h-4 animate-spin" /> : <Send className="w-4 h-4" />} ابعت الطلب</button>
          </div>
        )}

        {reqMsg && (
          <p className={`mt-2 text-[12px] font-bold text-center ${reqMsg.ok ? 'text-[#059669]' : 'text-red-600'}`}>{reqMsg.text}</p>
        )}
          ...
          <div className="mt-3 space-y-1.5">
            {reqList.leave.map((r: any) => (
              <div key={r.id} className="flex items-center justify-between rounded-xl bg-[#FAFAF7] px-3 py-2">
                <span className="text-[12px] text-[#1A2E26] flex items-center gap-1.5"><CalendarDays className="w-3.5 h-3.5 text-[#059669]" /> إجازة {leaveTypeAr(r.type)} · {r.days} يوم</span>
                {statusChip(r.status)}
              </div>
            ))}
            {reqList.advances.map((r: any) => (
              <div key={r.id} className="flex items-center justify-between rounded-xl bg-[#FAFAF7] px-3 py-2">
                <span className="text-[12px] text-[#1A2E26] flex items-center gap-1.5"><Wallet className="w-3.5 h-3.5 text-[#059669]" /> سلفة {Math.round(r.amount || 0).toLocaleString('en-US')} ج</span>
                {statusChip(r.status)}
              </div>
            ))}
          </div>
