  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#FAFAF7] flex items-center justify-center" dir="rtl">
        <Loader2 className="w-12 h-12 animate-spin text-[#FA8125]" />
      </div>
    )
  }
    ...
    return (
      <div className="min-h-screen bg-[#FAFAF7] flex items-center justify-center" dir="rtl">
        <div className="text-center p-8">
          ...
          <Link href="/" className="text-[#FA8125] underline">ارجع للرئيسية</Link>
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-[#FAFAF7]" dir="rtl">
      <header className="bg-white border-b border-gray-100 sticky top-0 z-10">
        <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <a href="https://wa.me/201002229982" target="_blank" rel="noreferrer"
            className="text-sm font-bold bg-[#FA8125] text-white px-4 py-2 rounded-xl hover:bg-[#175a4d]">
            تواصل معانا
          </a>
        </div>
      </header>

      <section className="bg-gradient-to-br from-[#FA8125] via-[#F98F2A] to-[#2FA084] text-white py-16">
      ...
      <section className="max-w-6xl mx-auto px-4 py-12">
        <h2 className="text-2xl font-black text-[#0A0A0A] mb-6 flex items-center gap-2">
          <Building2 className="w-6 h-6 text-[#FA8125]" /> الفروع ({data.branches.length})
        </h2>
      ...
      <section className="bg-white border-y border-gray-100">
        <div className="max-w-6xl mx-auto px-4 py-12">
          <h2 className="text-2xl font-black text-[#0A0A0A] mb-6 flex items-center gap-2">
            <Stethoscope className="w-6 h-6 text-[#FA8125]" /> الأطباء ({data.kpis.doctors_count})
          </h2>
          <div className="space-y-8">
            {data.specialties.map((sp) => (
              <div key={sp.specialty_label_ar}>
                <h3 className="text-lg font-bold text-[#FA8125] mb-3">{sp.specialty_label_ar}</h3>
                  ...
                  {sp.doctors.map((d) => (
                    <div key={d.employee_id} className="bg-[#FAFAF7] rounded-2xl p-4 border border-gray-100">
                      ...
                      <div className="text-sm text-gray-600 mb-2">
                        كشف: <span className="font-bold text-[#FA8125]">{d.consultation_fee_egp} ج</span>
                      </div>
      ...
      {data.recent_bookings && data.recent_bookings.length > 0 && (
        <section className="max-w-6xl mx-auto px-4 py-12">
          <h2 className="text-2xl font-black text-[#0A0A0A] mb-6 flex items-center gap-2">
            <Activity className="w-6 h-6 text-[#FA8125]" /> آخر الحجوزات
          </h2>
      ...
      {data.pnl && data.pnl.length > 0 && (
        <section className="bg-white border-y border-gray-100">
          <div className="max-w-6xl mx-auto px-4 py-12">
            <h2 className="text-2xl font-black text-[#0A0A0A] mb-6 flex items-center gap-2">
              <BarChart3 className="w-6 h-6 text-[#FA8125]" /> إيرادات الفروع
            </h2>
                ...
                return (
                  <div key={p.branch_name} className="bg-[#FAFAF7] rounded-2xl p-4">
                    ...
                    <div className="h-2 bg-white rounded-full overflow-hidden">
                      <div className="h-full bg-gradient-to-l from-[#FA8125] to-[#2FA084]" style={{ width: `${pct}%` }} />
                    </div>
                  </div>
                )
              })}
          ...
          {FEATURES.map((f, i) => {
            ...
            return (
              <div key={i} className="bg-white rounded-2xl p-5 border border-gray-100 hover:shadow-md transition-shadow">
                <div className="w-10 h-10 rounded-xl bg-emerald-50 grid place-items-center mb-3">
                  <Icon className="w-5 h-5 text-[#FA8125]" />
                </div>
      ...
      <section className="bg-gradient-to-br from-[#FA8125] to-[#2FA084] text-white py-12">
      ...
      <section className="max-w-3xl mx-auto px-4 py-16 text-center">
        ...
        <div className="flex gap-3 justify-center flex-wrap">
          <a href="https://wa.me/201002229982?text=أنا%20من%20عيادة%20وحابب%20أعرف%20أكتر%20عن%20مضمونة"
            ...
            className="inline-flex items-center gap-2 bg-[#FA8125] text-white px-8 py-4 rounded-xl font-black text-lg hover:bg-[#175a4d] transition-colors shadow-lg">
            <MessageSquare className="w-5 h-5" /> كلّمنا على واتساب
          </a>
          <Link href={`/clinic/${slug}`}
            className="inline-flex items-center gap-2 bg-white border-2 border-[#FA8125] text-[#FA8125] px-8 py-4 rounded-xl font-black text-lg hover:bg-emerald-50 transition-colors">
...
function KpiCard({ icon: Icon, label, value }: { icon: React.ElementType; label: string; value: string }) {
  return (
    <div className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm">
      <Icon className="w-5 h-5 text-[#FA8125] mb-2" />
