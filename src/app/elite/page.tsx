      ...
      <main className="max-w-md mx-auto px-4 -mt-8 pb-12 space-y-5 relative z-10 md-fade">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-[0_10px_30px_-18px_rgba(26,46,38,0.35)] p-3 flex items-center justify-around text-center">
          {[{ i: ShieldCheck, t: 'أمان كامل' }, { i: Clock, t: 'حجز فوري' }, { i: Star, t: 'خدمة احترافية' }].map((x, k) => (
            <div key={k} className="flex flex-col items-center gap-1 flex-1">
              <x.i className="w-5 h-5 text-[#059669]" />
        ...
        <section>
          ...
          <button onClick={nearest} disabled={geoBusy}
            className="w-full bg-[#34D399] text-[#04352A] rounded-2xl p-4 flex items-center justify-between shadow-lg shadow-[#059669]/20 active:scale-[0.99] transition-transform disabled:opacity-70 mb-3 md-glow">
          ...
          <div className="space-y-2.5">
            {branches.map((br: any) => (
              <button key={br.code} onClick={() => router.push(`/v/${br.code}`)}
                className="w-full bg-white border border-gray-100 rounded-2xl p-4 flex items-center justify-between text-right active:scale-[0.99] transition-all hover:border-[#059669]/40 hover:shadow-md hover:shadow-[#1A2E26]/5 shadow-[0_8px_24px_-16px_rgba(26,46,38,0.3)]">
                <div className="flex items-center gap-3">
                  <div className="w-11 h-11 rounded-xl bg-[#34D399]/10 text-[#059669] grid place-items-center"><MapPin className="w-5 h-5" /></div>
                  <div>
                ...
                <span className="w-8 h-8 rounded-lg bg-[#34D399]/10 text-[#059669] grid place-items-center"><CalendarPlus className="w-4 h-4" /></span>
              </button>
            ))}
          </div>
        </section>
          ...
          <div className="space-y-2.5">
            {ig && (
              <a href={`https://instagram.com/${ig}`} target="_blank" rel="noopener noreferrer"
                ...
                <span className="text-[12px] font-black text-[#059669]">تابعينا</span>
              </a>
            )}
        ...
        <div className="flex flex-col items-center gap-2 pt-2">
          <div className="flex items-center gap-2 text-[#6B7280]">
            ...
            <span className="w-6 h-6 rounded-lg bg-[#34D399] text-[#04352A] grid place-items-center text-[13px] font-black">م</span>
...
function Hero({ logo, gallery = [], tagline, bizName }: any) {
  ...
  return (
    <header className="relative bg-[#34D399] text-[#04352A] overflow-hidden">
      ...
      <div className="absolute inset-0 bg-gradient-to-b from-[#34D399]/75 via-[#34D399]/55 to-[#1A2E26]/92" />
