  ...
  return (
    <div className="min-h-screen bg-[#FAFAF7]" dir="rtl">
      ...
      <main className="max-w-3xl mx-auto px-4 -mt-6 pb-12 space-y-7 relative z-10">
        ...
        {/* ===== TODAY ===== */}
        <section>
          <div className="bg-[#2B4521] text-white rounded-2xl p-5 shadow-lg shadow-[#2B4521]/20 relative overflow-hidden">
            ...
            <div className="space-y-2.5">
              {admins.map((a: any) => (
                <div key={a.supplier_id} className="space-y-2.5">
                  <Link href={`/owner/${a.supplier_id}`} className={`${CARD} p-4 flex items-center justify-between hover:border-[#2B4521]/40 hover:shadow-md transition-all`}>
                    <div className="flex items-center gap-3">
                      <div className="w-11 h-11 rounded-xl bg-[#2B4521]/10 grid place-items-center"><Briefcase className="w-5 h-5 text-[#2B4521]" /></div>
                      <div>
                  ...
                  {(joinReqs[a.supplier_id]?.length > 0) && (
                    <div className="bg-white rounded-2xl border border-[#2B4521]/30 p-4 shadow-[0_10px_30px_-18px_rgba(26,46,38,0.35)]">
                      <div className="flex items-center gap-1.5 mb-3">
                        <UserPlus className="w-4 h-4 text-[#2B4521]" />
                        <p className="text-sm font-black text-[#1A2E26]">طلبات انضمام موظفين</p>
                        <span className="px-1.5 py-0.5 rounded-full bg-[#2B4521] text-white text-[10px] font-bold">{joinReqs[a.supplier_id].length}</span>
                      </div>
                        ...
                        {joinReqs[a.supplier_id].map((r: any) => (
                          <div key={r.id} className="flex items-center justify-between gap-2 border-b border-gray-50 pb-2 last:border-0">
                            <div className="min-w-0">
                              <p className="font-bold text-[#1A2E26] text-sm truncate">{r.full_name} {r.name_match && <span className="text-[10px] text-[#2B4521]">(مطابق لموظف موجود)</span>}</p>
                              <p className="text-[11px] text-[#6B7280] truncate">{r.job_title || 'موظف'} · {r.branch_name} · <span dir="ltr">{r.phone}</span></p>
                            </div>
                            <div className="flex items-center gap-1.5 flex-shrink-0">
                              <button onClick={() => reviewReq(a.supplier_id, r.id, 'approve')} disabled={busyReq === r.id} className="w-9 h-9 rounded-xl bg-[#2B4521] text-white grid place-items-center disabled:opacity-50" title="موافقة">
          ...
          <section>
            ...
            <div className="space-y-2.5">
              {employees.map((emp: any) => (
                <Link key={emp.employee_id} href="/me" className={`${CARD} p-4 flex items-center justify-between hover:border-[#2B4521]/40 hover:shadow-md transition-all`}>
                  <div className="flex items-center gap-3">
                    <div className="w-11 h-11 rounded-xl bg-[#2B4521]/10 grid place-items-center"><Briefcase className="w-5 h-5 text-[#2B4521]" /></div>
                    <div>
              ...
              {pendingReviews.map((p: any) => {
                ...
                return (
                  <div key={p.booking_id} className={`${CARD} p-4`}>
                    ...
                    <div className="flex gap-1 mb-3" dir="ltr">
                      {[1,2,3,4,5].map(n => (
                        <button key={n} onClick={() => setRate(p.booking_id, 'service', n)}>
                          <Star className={`w-7 h-7 ${n <= r.service ? 'fill-[#2B4521] text-[#2B4521]' : 'text-gray-300'}`} />
                        </button>
                      ))}
                    </div>
                        ...
                        <div className="flex gap-1 mb-3" dir="ltr">
                          {[1,2,3,4,5].map(n => (
                            <button key={n} onClick={() => setRate(p.booking_id, 'stylist', n)}>
                              <Star className={`w-6 h-6 ${n <= r.stylist ? 'fill-[#2B4521] text-[#2B4521]' : 'text-gray-300'}`} />
                            </button>
                          ))}
                        </div>
                      </>
                    ...
                    <textarea value={r.comment} onChange={e => setRate(p.booking_id, 'comment', e.target.value)} placeholder="رأيك يهمنا (اختياري)" rows={2} className="w-full px-3 py-2.5 rounded-xl bg-[#FAFAF7] text-sm mb-3 resize-none outline-none focus:bg-white border border-transparent focus:border-[#2B4521]/30 transition-colors" />

                    <button onClick={() => submitReview(p.booking_id)} disabled={busyReview === p.booking_id || !r.service} className="w-full py-3 rounded-2xl bg-[#2B4521] text-white font-black text-sm disabled:opacity-50 flex items-center justify-center gap-2 shadow-lg shadow-[#2B4521]/20 active:scale-[0.99] transition-all">
                ...
                <div className="space-y-2 mb-3">
                  {custBookings.upcoming.map((b: any, i: number) => (
                    <div key={i} className="flex items-center justify-between text-sm border-b border-gray-50 pb-2 last:border-0">
                      ...
                      <span className="px-2 py-0.5 rounded bg-[#2B4521]/10 text-[#2B4521] text-[10px] font-bold">{STATUS_AR[b.status] || b.status}</span>
                    </div>
                  ))}
                </div>
              ...
              {bookBranches.length > 0 ? (
                <div className="space-y-2">
                  ...
                  {bookBranches.map((b: any) => (
                    <Link key={b.branch_id} href={`/book/${b.code}`} className="flex items-center justify-between py-2.5 px-3 rounded-xl bg-[#2B4521]/5 hover:bg-[#2B4521]/10 transition-colors">
                      <span className="text-sm font-bold text-[#1A2E26] flex items-center gap-2"><Calendar className="w-4 h-4 text-[#2B4521]" /> {b.name}</span>
                      <ChevronLeft className="w-4 h-4 text-[#6B7280]" />
                    </Link>
                  ))}
                </div>
              ) : (
                <Link href="/marketplace" className="w-full py-3 rounded-2xl bg-[#2B4521] text-white font-black text-sm flex items-center justify-center gap-2 shadow-lg shadow-[#2B4521]/20">
              ...
              <div className={`${CARD} p-4`}>
                {tipResult ? (
                  <div className="text-center py-2">
                    <div className="w-14 h-14 rounded-full bg-[#2B4521]/10 grid place-items-center mx-auto mb-3"><Gift className="w-7 h-7 text-[#2B4521]" /></div>
                    ...
                    {tipResult.instapay && (
                      <div className="mt-3 bg-[#FAFAF7] rounded-xl p-3 text-right border border-[#2B4521]/15">
                        ...
                        <div className="space-y-1.5">
                          ...
                          <div className="flex items-center justify-between"><span className="text-[12px] text-[#6B7280]">رقم الحساب / إنستاباي</span><span className="font-mono font-black text-[#2B4521] select-all" dir="ltr">{tipResult.instapay}</span></div>
                        </div>
                      </div>
                    )}
                    <button onClick={() => setTipResult(null)} className="mt-4 text-xs font-bold text-[#2B4521]">اكرامية تانية</button>
                  </div>
                ) : (
                  <>
                    ...
                    <div className="flex gap-2 mb-3">
                      {[20, 50, 100].map(a => (
                        <button key={a} onClick={() => setTipAmount(a)} className={`flex-1 py-2.5 rounded-xl font-bold text-sm border transition-all ${tipAmount === a ? 'bg-[#2B4521] text-white border-[#2B4521]' : 'bg-white text-[#1A2E26] border-gray-200'}`}>{a} ج</button>
                      ))}
                    ...
                    <div className="flex gap-2 mb-4">
                      <button onClick={() => setTipMethod('instapay')} className={`flex-1 py-2.5 rounded-xl font-bold text-sm border flex items-center justify-center gap-1.5 ${tipMethod === 'instapay' ? 'bg-[#2B4521] text-white border-[#2B4521]' : 'bg-white text-[#1A2E26] border-gray-200'}`}><Coins className="w-4 h-4" /> إنستاباي</button>
                      <button onClick={() => setTipMethod('cash')} className={`flex-1 py-2.5 rounded-xl font-bold text-sm border flex items-center justify-center gap-1.5 ${tipMethod === 'cash' ? 'bg-[#2B4521] text-white border-[#2B4521]' : 'bg-white text-[#1A2E26] border-gray-200'}`}><Wallet className="w-4 h-4" /> كاش في الفرع</button>
                    </div>

                    <button onClick={sendTip} disabled={tipBusy || !tipEmp || !tipAmount} className="w-full py-3 rounded-2xl bg-[#2B4521] text-white font-black text-sm disabled:opacity-50 flex items-center justify-center gap-2 shadow-lg shadow-[#2B4521]/20 active:scale-[0.99] transition-all">
          ...
          return (
            <section>
              ...
              <div className={`${CARD} p-4`}>
                {orderResult ? (
                  <div className="text-center py-2">
                    <div className="w-14 h-14 rounded-full bg-[#2B4521]/10 grid place-items-center mx-auto mb-3"><ShoppingBag className="w-7 h-7 text-[#2B4521]" /></div>
                    ...
                    {orderResult.instapay && (
                      <div className="mt-3 bg-[#FAFAF7] rounded-xl p-3 text-right border border-[#2B4521]/15">
                        ...
                        <div className="space-y-1.5">
                          <div className="flex items-center justify-between"><span className="text-[12px] text-[#6B7280]">البنك</span><span className="font-bold text-[#1A2E26]">بنك مصر</span></div>
                          <div className="flex items-center justify-between"><span className="text-[12px] text-[#6B7280]">اسم الحساب</span><span className="font-bold text-[#1A2E26]">مضمونة</span></div>
                          <div className="flex items-center justify-between"><span className="text-[12px] text-[#6B7280]">رقم الحساب / إنستاباي</span><span className="font-mono font-black text-[#2B4521] select-all" dir="ltr">{orderResult.instapay}</span></div>
                        </div>
                      </div>
                    )}
                    <button onClick={() => setOrderResult(null)} className="mt-4 text-xs font-bold text-[#2B4521]">اطلب تاني</button>
                  </div>
                ) : (
                  <>
                      ...
                      {products.map((p: any) => (
                        <div key={p.product_id} className="flex items-center justify-between gap-2 border-b border-gray-50 pb-2 last:border-0">
                          <div className="min-w-0">
                            ...
                            <p className="text-[11px] text-[#2B4521] font-bold">{Number(p.price).toLocaleString()} ج</p>
                          </div>
                          {cart[p.product_id] ? (
                            <div className="flex items-center gap-2 flex-shrink-0">
                              ...
                              <button onClick={() => changeQty(p.product_id, 1)} className="w-7 h-7 rounded-lg bg-[#2B4521] text-white grid place-items-center"><Plus className="w-3.5 h-3.5" /></button>
                            </div>
                          ) : (
                            <button onClick={() => changeQty(p.product_id, 1)} className="px-3 py-1.5 rounded-lg bg-[#2B4521]/10 text-[#2B4521] text-xs font-bold flex-shrink-0">ضيف</button>
                          )}
                        </div>
                      ))}
                    </div>
                    ...
                    {total > 0 && (
                      <div className="border-t border-gray-100 pt-3">
                        ...
                        <div className="flex gap-2 mb-3">
                          <button onClick={() => setStoreMethod('instapay')} className={`flex-1 py-2.5 rounded-xl font-bold text-sm border flex items-center justify-center gap-1.5 ${storeMethod === 'instapay' ? 'bg-[#2B4521] text-white border-[#2B4521]' : 'bg-white text-[#1A2E26] border-gray-200'}`}><Coins className="w-4 h-4" /> إنستاباي</button>
                          <button onClick={() => setStoreMethod('cash')} className={`flex-1 py-2.5 rounded-xl font-bold text-sm border flex items-center justify-center gap-1.5 ${storeMethod === 'cash' ? 'bg-[#2B4521] text-white border-[#2B4521]' : 'bg-white text-[#1A2E26] border-gray-200'}`}><Wallet className="w-4 h-4" /> كاش في الفرع</button>
                        </div>
                        <button onClick={placeOrder} disabled={orderBusy} className="w-full py-3 rounded-2xl bg-[#2B4521] text-white font-black text-sm disabled:opacity-50 flex items-center justify-center gap-2 shadow-lg shadow-[#2B4521]/20 active:scale-[0.99] transition-all">
        ...
        <section>
          ...
          <div className="grid grid-cols-2 gap-3">
            <Link href="/add-listing" className={`${CARD} p-5 flex flex-col items-center gap-2 hover:border-[#2B4521]/40 hover:shadow-md transition-all`}>
              <div className="w-11 h-11 rounded-xl bg-[#2B4521]/10 grid place-items-center"><Plus className="w-5 h-5 text-[#2B4521]" /></div>
            ...
            <Link href="/marketplace" className={`${CARD} p-5 flex flex-col items-center gap-2 hover:border-[#2B4521]/40 hover:shadow-md transition-all`}>
              <div className="w-11 h-11 rounded-xl bg-[#2B4521]/10 grid place-items-center"><Search className="w-5 h-5 text-[#2B4521]" /></div>
