    ...
    <div className="min-h-screen bg-[#34D399] flex items-center justify-center p-4" dir="rtl">
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-3xl p-6 shadow-2xl">
          {mode === 'pin' ? (
            <>
              <button onClick={() => { setMode('wa'); setError('') }} className="text-xs font-bold text-[#6B7280] hover:text-[#059669] flex items-center gap-1 mb-3"><ArrowLeft className="w-3.5 h-3.5" /> دخول عادي بواتساب</button>
              <h2 className="text-lg font-black text-[#1A2E26] mb-1 flex items-center gap-1.5"><Briefcase className="w-4 h-4 text-[#059669]" /> دخول الموظفين</h2>
              ...
              <button onClick={employeeLogin} disabled={sending || !phone || pin.length < 3} className="w-full mt-4 py-3 rounded-xl bg-[#34D399] text-[#04352A] font-black text-sm disabled:opacity-50 flex items-center justify-center gap-2">
            ...
            <>
              ...
              <button onClick={() => { setMode('pin'); setError('') }} className="w-full mt-3 py-2.5 rounded-xl border border-[#059669]/25 text-[#059669] font-bold text-[13px] flex items-center justify-center gap-1.5">
