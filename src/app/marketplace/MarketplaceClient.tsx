...
function MarketplaceBrowseContent({ initialListings }: { initialListings?: Listing[] }) {
  ...
  return (
    <div className="min-h-screen gradient-mesh pb-20 md:pb-0" dir={dir}>
      <div className="fixed top-0 right-0 w-[500px] h-[500px] bg-[#FA8125]/5 rounded-full blur-3xl -z-10 pointer-events-none" />
        ...
        <div className="max-w-6xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between gap-3 mb-4">
            ...
            {isAuthed === true ? (
              ...
              <Link
                ...
                className="inline-flex items-center gap-1 px-4 py-2 bg-[#FA8125] text-white rounded-full text-xs font-bold shadow-soft hover:shadow-card hover:-translate-y-0.5 transition-all flex-shrink-0"
              >
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pr-12 pl-4 py-3.5 bg-white/80 backdrop-blur border border-gray-100 rounded-2xl text-sm font-medium focus:outline-none focus:bg-white focus:border-[#FA8125]/40 focus:ring-4 focus:ring-[#FA8125]/10 transition-all shadow-soft"
            />
          </div>
              ...
              <button
                ...
                className={`flex-shrink-0 px-3 py-1.5 rounded-full text-[11px] font-semibold transition-all ${
                  selectedCategorySlug === selectedMid.slug
                    ? 'bg-[#FA8125] text-white shadow-soft'
                    : 'bg-white/80 text-gray-600 hover:bg-white border border-gray-100'
                }`}
              >
                {t('market.all_sections')}
                ...
                <div className="absolute top-full right-0 mt-2 w-52 bg-white rounded-2xl shadow-luxe border border-gray-100 z-50 overflow-hidden animate-scale-in">
                  {(Object.keys(SORT_LABELS) as SortOption[]).map(option => (
                    <button
                      ...
                      className={`w-full text-start px-4 py-2.5 text-xs hover:bg-[#FA8125]/5 font-medium transition-colors ${
                        sortBy === option ? 'bg-[#FA8125]/10 text-[#FA8125]' : 'text-gray-700'
                      }`}
                    >
                ...
                {cityMenuOpen && (
                  <div className="absolute top-full right-0 mt-2 w-52 bg-white rounded-2xl shadow-luxe border border-gray-100 z-50 overflow-hidden max-h-72 overflow-y-auto animate-scale-in">
                    <button
                      ...
                      className={`w-full text-start px-4 py-2.5 text-xs hover:bg-[#FA8125]/5 font-medium transition-colors ${
                        !cityFilter ? 'bg-[#FA8125]/10 text-[#FA8125]' : 'text-gray-700'
                      }`}
                    >
                      {t('market.all_cities')}
                    </button>
                    {cities.map(city => (
                      <button
                        ...
                        className={`w-full text-start px-4 py-2.5 text-xs hover:bg-[#FA8125]/5 font-medium transition-colors ${
                          cityFilter === city ? 'bg-[#FA8125]/10 text-[#FA8125]' : 'text-gray-700'
                        }`}
                      >
                        {city}
                      </button>
        ...
        {!supplierFilter && !inPropertyZone && !!(selectedGroupSlug || selectedCategorySlug) && (
          <div className="mb-5 inline-flex items-center gap-1 bg-white rounded-full p-1 shadow-soft border border-gray-100">
            <button
              ...
              className={`px-4 py-2 rounded-full text-xs font-bold transition-all ${viewMode === 'products' ? 'bg-[#FA8125] text-white shadow-soft' : 'text-gray-600 hover:bg-gray-50'}`}
            >
              🛍️ المنتجات
            </button>
            <button
              onClick={() => switchView('stores')}
              className={`px-4 py-2 rounded-full text-xs font-bold transition-all ${viewMode === 'stores' ? 'bg-[#FA8125] text-white shadow-soft' : 'text-gray-600 hover:bg-gray-50'}`}
            >
              🏬 المتاجر
            </button>
          </div>
        ...
        {viewMode === 'stores' && !supplierFilter && !inPropertyZone && !!(selectedGroupSlug || selectedCategorySlug) ? (
                        ...
                        >
                          <div className="flex items-center gap-4 mb-4">
                            ...
                            <div className="min-w-0 flex-1">
                              <h3 className="font-black text-base text-gray-900 truncate group-hover:text-[#FA8125] transition-colors">{s.name}</h3>
                          ...
                          <div className="flex items-center justify-between pt-3 border-t border-gray-100">
                            ...
                            <span className="inline-flex items-center gap-1 text-[#FA8125] font-bold text-xs group-hover:gap-2 transition-all">
                              <span>{h.n.visit}</span>
          ...
          <div className="bg-white rounded-3xl shadow-soft p-12 md:p-20 text-center">
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 px-5 py-2.5 bg-[#FA8125] text-white rounded-2xl text-sm font-bold shadow-soft hover:shadow-card hover:-translate-y-0.5 transition-all"
            >
              🔄 جرّب تاني
            </button>
          </div>
                  ...
                  <div className="p-5">
                    ...
                    {listing.supplier?.id && listing.supplier?.business_name && (
                      <button
                        ...
                        className="mb-2 inline-flex items-center gap-1 px-2 py-0.5 bg-gray-50 hover:bg-[#FA8125]/10 border border-gray-200 rounded-full text-[10px] font-bold text-gray-600 hover:text-[#FA8125] transition-colors"
          ...
          {listings.length >= visibleLimit && (
            <div ref={loadMoreRef} className="mt-8 text-center">
              <button
                ...
                disabled={loading}
                className="inline-flex items-center gap-2 px-6 py-3 bg-white text-[#FA8125] border border-[#FA8125]/30 rounded-2xl text-sm font-bold shadow-soft hover:shadow-card hover:-translate-y-0.5 transition-all disabled:opacity-50"
              >
  ...
  return (
    <button
      ...
      className={`flex-shrink-0 px-4 py-2 rounded-full text-xs font-semibold transition-all flex items-center gap-1.5 shadow-soft ${
        active
          ? 'bg-[#FA8125] text-white shadow-elevated'
