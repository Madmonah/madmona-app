              ...
              {sortedPhotos.map((p, i) => (
                <button
                  ...
                  className={`flex-shrink-0 w-20 h-20 rounded-xl overflow-hidden transition-all ${
                    i === photoIndex
                      ? 'ring-2 ring-[#FA8125] ring-offset-2 scale-105'
            ...
            <section className="bg-white rounded-3xl shadow-soft p-6 md:p-8 animate-slide-up">
              {listing.category && (
                <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-[#FA8125]/10 rounded-full mb-4">
                  ...
                  <span className="text-xs font-bold text-[#FA8125]">{lang === 'en' && listing.category.name_en ? listing.category.name_en : listing.category.name_ar}</span>
                </div>
              )}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  {listing.stock_quantity !== null && listing.stock_quantity !== undefined && !noDelivery && (
                    <div className="flex items-center justify-between p-3 bg-[#FAFAF7] rounded-xl">
                      ...
                      <span className={`text-sm font-bold ${listing.stock_quantity > 0 ? 'text-[#FA8125]' : 'text-red-600'}`}>
                  ...
                  {listing.branches.map((b, i) => (
                    <div key={i} className="p-4 bg-[#FAFAF7] rounded-2xl border border-[#EFEEE9]">
                      <div className="flex items-center gap-2 mb-1">
                        <MapPin className="w-3.5 h-3.5 text-[#FA8125] flex-shrink-0" />
                      ...
                      {b.phone && (
                        <a href={`tel:${b.phone}`} className="inline-block mt-2 pr-5 text-xs font-semibold text-[#FA8125] hover:underline" dir="ltr">
                          {b.phone}
                        </a>
                      )}
                    </div>
            ...
            {isRealEstate && !isDirectory && (
              <div className="lg:hidden bg-white rounded-3xl shadow-card p-5 space-y-2.5">
                ...
                <button
                  onClick={toggleFavorite}
                  disabled={togglingFav}
                  className={`flex items-center justify-center gap-2 w-full py-3.5 rounded-2xl font-bold text-sm transition-all ${isFavorite ? 'bg-rose-50 text-rose-600 border-2 border-rose-200' : 'bg-[#FA8125] text-white shadow-elevated'}`}
                >
                ...
                <Link
                  href="/real-estate"
                  className="flex items-center justify-center gap-2 w-full py-3.5 rounded-2xl font-bold text-sm bg-white text-[#FA8125] border-2 border-[#FA8125] no-underline"
                >
                ...
                {activeTab === 'details' && (
                  <div className="space-y-6">
                    ...
                    {attributes.length > 0 && (
                      <div>
                        <h3 className="text-sm font-black text-gray-900 mb-3 flex items-center gap-2">
                          <Tag className="w-4 h-4 text-[#FA8125]" />
                      ...
                      <div>
                        ...
                        <div className="space-y-2">
                          {pricing.map(rule => (
                            <div
                              key={rule.id}
                              className="flex items-center justify-between p-4 bg-gradient-to-l from-[#FA8125]/5 to-transparent rounded-xl border border-[#FA8125]/10"
                            >
                              <span className="text-sm font-bold text-gray-700">
                                {PERIOD_LABELS[rule.period_type] ? t(PERIOD_LABELS[rule.period_type]) : rule.period_type}
                              </span>
                              <span className="text-lg font-black text-[#FA8125] tabular">
                  ...
                  <div>
                    {listing.address && (
                      <p className="text-sm md:text-base text-gray-700 mb-4 flex items-start gap-2">
                        <MapPin className="w-5 h-5 text-[#FA8125] flex-shrink-0 mt-0.5" />
                    ...
                    {hasMap && (
                      <>
                        ...
                        <a
                          ...
                          className="mt-4 inline-flex items-center gap-2 px-5 py-2.5 bg-[#FA8125] text-white rounded-xl font-bold text-sm hover:shadow-elevated hover:-translate-y-0.5 transition-all no-underline"
                        >
                          ...
                          <div key={r.id} className="border-b border-gray-100 pb-5 last:border-b-0 last:pb-0">
                            <div className="flex items-center justify-between mb-2">
                              <div className="flex items-center gap-2">
                                <div className="w-9 h-9 bg-[#FA8125]/10 rounded-full flex items-center justify-center flex-shrink-0">
                            ...
                            {r.supplier_response && (
                              <div className="mt-3 mr-11 bg-gradient-to-l from-[#FA8125]/5 to-transparent border border-[#FA8125]/10 rounded-2xl p-4">
              ...
              <div className="bg-white rounded-3xl shadow-card p-6">
                {startingPrice !== null ? (
                  <>
                    ...
                    <p className="text-3xl font-black text-[#FA8125] tabular mb-1">
                ...
                <div className="space-y-2">
                  {canBook && (
                    <Link
                      ...
                      className="flex items-center justify-center gap-2 bg-[#FA8125] text-white py-3.5 rounded-2xl font-bold text-sm shadow-elevated hover:shadow-luxe hover:-translate-y-0.5 transition-all no-underline w-full"
                    >
                  ...
                  {!isDirectory && !isDemo && (
                    <button
                      ...
                      disabled={inquiring}
                      className="flex items-center justify-center gap-2 bg-[#FA8125] text-white py-3.5 rounded-2xl font-bold text-sm shadow-soft hover:shadow-card hover:-translate-y-0.5 transition-all w-full disabled:opacity-60"
                    >
                ...
                {!isDirectory ? (
                  ...
                  <div className="mt-5 pt-5 border-t border-gray-100 space-y-3">
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center justify-center gap-2 bg-[#FA8125] text-white py-2.5 rounded-2xl font-bold text-xs no-underline w-full"
                    >
                      هو ده نشاطك؟ استلمه
                    </a>
              ...
              {isRealEstate && !isDirectory && (
                <div className="bg-white rounded-3xl shadow-card p-6 space-y-2.5">
                  ...
                  <button
                    onClick={toggleFavorite}
                    disabled={togglingFav}
                    className={`flex items-center justify-center gap-2 w-full py-3.5 rounded-2xl font-bold text-sm transition-all ${isFavorite ? 'bg-rose-50 text-rose-600 border-2 border-rose-200' : 'bg-[#FA8125] text-white shadow-elevated hover:shadow-luxe hover:-translate-y-0.5'}`}
                  >
                    <Heart className={`w-4 h-4 ${isFavorite ? 'fill-rose-500 text-rose-500' : ''}`} />
                    {isFavorite ? 'محفوظ في المفضلة' : 'أضف للمفضلة'}
                  </button>
                  <Link
                    href="/real-estate"
                    className="flex items-center justify-center gap-2 w-full py-3.5 rounded-2xl font-bold text-sm bg-white text-[#FA8125] border-2 border-[#FA8125] no-underline hover:bg-[#FA8125]/5"
                  >
                    <Building2 className="w-4 h-4" />
                    المنصة العقارية — قارن وشوف كل المشاريع
                  </Link>
              ...
              {!isDirectory && listing.supplier && (
                <div className="bg-white rounded-3xl shadow-soft p-6">
                  ...
                  <div className="flex items-center gap-3 mb-4">
                    <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-[#FA8125] to-[#F98F2A] flex items-center justify-center flex-shrink-0">
      ...
      <div className="fixed bottom-0 inset-x-0 glass border-t border-white/40 z-50 lg:hidden shadow-luxe">
        <div className="max-w-6xl mx-auto p-3 flex items-center gap-2">
          ...
          {canBook ? (
            <Link
              href={`/marketplace/${listing.slug}/book`}
              className="flex items-center gap-1.5 bg-[#FA8125] text-white px-5 py-3 rounded-2xl font-bold text-sm shadow-elevated hover:shadow-luxe hover:-translate-y-0.5 transition-all flex-shrink-0"
            >
              <Calendar className="w-4 h-4" />
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-1.5 bg-[#FA8125] text-white px-5 py-3 rounded-2xl font-bold text-sm shadow-elevated flex-shrink-0"
            >
              <MessageCircle className="w-4 h-4" />
  ...
  return (
    <button
      ...
      className={`relative px-4 py-3 text-sm font-bold transition-colors ${
        active ? 'text-[#FA8125]' : 'text-gray-500 hover:text-gray-700'
      }`}
    >
      {label}
      {active && (
        <span className="absolute bottom-0 left-2 right-2 h-0.5 bg-[#FA8125] rounded-full" />
      )}
    </button>
  )
}
