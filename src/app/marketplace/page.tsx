...
function MarketplaceFallback({ listings = [] }: { listings?: SSRListing[] }) {
  return (
    <div className="min-h-screen bg-[#FAFAF7]">
      <header className="bg-[#2B4521] text-[#FAF7F0] px-5 py-6">
      ...
      <main className="px-5 py-6 max-w-6xl mx-auto">
        <h2 className="text-lg font-bold mb-4 text-[#2B4521]">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6" aria-hidden>
          {CATEGORY_PREVIEW.map((c) => (
            <div
              key={c.slug}
              className="p-4 rounded-xl bg-white border border-gray-200 flex items-center gap-3 hover:border-[#2B4521] transition-colors"
            >
              <span className="text-2xl">{c.emoji}</span>
              <span className="text-[#2B4521] font-medium"><T k={c.labelKey} /></span>
            </div>
          ))}
        </div>
        ...
        {listings.length > 0 ? (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {listings.map((l) => {
              ...
              return (
                <a key={l.id} href={`/marketplace/${l.slug}`} className="block bg-white rounded-xl border border-gray-200 overflow-hidden hover:border-[#2B4521] transition-colors no-underline">
          ...
          <div className="mt-6 p-4 rounded-xl bg-amber-50 border border-amber-200 text-amber-900">
            ...
            <p className="text-sm">
              ...
              <a
                ...
                className="underline text-[#2B4521] font-bold"
              >
                WhatsApp
              </a>{' '}
              .
