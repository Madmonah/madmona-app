  ...
  if (notEmployee) return (
    <div className="min-h-screen bg-[#FAFAF7] flex items-center justify-center p-4" dir="rtl">
      <div className="text-center bg-white rounded-3xl p-8 border border-gray-100 max-w-sm">
        ...
        <Link href="/home" className="inline-block px-5 py-2.5 rounded-xl bg-[#FA8125] text-white font-bold text-sm">ارجع للرئيسية</Link>
      </div>
    </div>
  )
  ...
  return (
    <div className="min-h-screen bg-[#FAFAF7]" dir="rtl">
      ...
      <main className="max-w-2xl mx-auto px-4 py-6 space-y-5">
        ...
        {data.is_manager && (
          <Link href="/me/team" className="flex items-center justify-between bg-[#FA8125] text-white rounded-2xl p-4 active:scale-[0.99] transition-all">
          ...
          <section className="bg-white rounded-2xl border border-gray-100 p-5">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-1.5">
                <Bell className="w-4 h-4 text-[#FA8125]" />
                <p className="text-sm font-black text-[#1A2E26]">الإشعارات</p>
                {unread > 0 && <span className="text-[10px] font-black bg-[#FA8125] text-white rounded-full min-w-[18px] text-center px-1.5 py-0.5">{fmt(unread)}</span>}
              </div>
              {unread > 0 && <button onClick={markAllRead} className="text-[11px] font-bold text-[#FA8125]">تعليم الكل كمقروء</button>}
            </div>
            <div className="space-y-2">
              {notifs.map((n: any) => (
                <div key={n.id} className={`p-3 rounded-xl border ${n.read ? 'bg-white border-gray-100' : 'bg-[#FA8125]/5 border-[#FA8125]/30'}`}>
                  <div className="flex items-start gap-2.5">
                    {!n.read && <span className="w-2 h-2 rounded-full bg-[#FA8125] flex-shrink-0 mt-1.5" />}
        ...
        <section className="bg-white rounded-2xl border border-gray-100 p-5">
          <div className="flex items-center gap-1.5 mb-3"><MapPin className="w-4 h-4 text-[#FA8125]" /><p className="text-sm font-black text-[#1A2E26]">الحضور والانصراف</p></div>

          {att?.clock_in_at && (
            <div className="flex items-center gap-4 text-sm mb-3 bg-[#FAFAF7] rounded-xl p-3">
              <div><p className="text-[10px] text-[#6B7280]">حضور</p><p className="font-mono font-bold text-[#FA8125]" dir="ltr">{new Date(att.clock_in_at).toLocaleTimeString('ar-EG', { hour: '2-digit', minute: '2-digit' })}</p></div>
          ...
          {clockMsg && (
            <div className={`mb-3 px-3 py-2 rounded-xl text-xs font-bold ${clockMsg.ok ? 'bg-[#FA8125]/10 text-[#FA8125]' : 'bg-red-50 text-red-600'}`}>{clockMsg.text}</div>
          )}

          {clockState === 'done' ? (
            <div className="text-center py-2 text-sm font-bold text-[#FA8125] flex items-center justify-center gap-2"><CheckCircle2 className="w-5 h-5" /> خلصت يومك · شكراً 🙌</div>
          ) : (
            <button onClick={doClock} disabled={clocking} className="w-full py-3 rounded-xl bg-[#FA8125] text-white font-black text-sm disabled:opacity-50 flex items-center justify-center gap-2">
        ...
        <section className="grid grid-cols-2 gap-3">
          <div className="bg-[#FA8125] text-white rounded-2xl p-4">
        ...
        <section className="bg-white rounded-2xl border border-gray-100 p-5">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-1.5"><Gift className="w-4 h-4 text-[#FA8125]" /><p className="text-sm font-black text-[#1A2E26]">البقشيش</p></div>
            <div className="text-left">
              <p className="text-[10px] text-[#6B7280]">الشهر ده</p>
              <p className="font-black font-mono text-[#FA8125]">{fmt(tips.month_total)} ج</p>
            </div>
          </div>
              ...
              {tips.recent.map((t: any, i: number) => (
                <div key={i} className="flex items-center justify-between text-sm border-b border-gray-50 pb-2 last:border-0">
                  <div className="flex items-center gap-2">
                    <Coins className="w-3.5 h-3.5 text-[#FA8125]" />
                  ...
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded ${t.status === 'received' ? 'bg-[#FA8125]/10 text-[#FA8125]' : t.status === 'rejected' ? 'bg-red-50 text-red-600' : 'bg-amber-50 text-amber-700'}`}>
        ...
        <section>
          <div className="flex items-center justify-between mb-3 px-1">
            <div className="flex items-center gap-1.5"><CalendarCheck className="w-4 h-4 text-[#FA8125]" /><p className="text-sm font-black text-[#1A2E26]">مواعيدي النهاردة</p></div>
        ...
        <section className="bg-white rounded-2xl border border-gray-100 p-5">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-1.5"><ListChecks className="w-4 h-4 text-[#FA8125]" /><p className="text-sm font-black text-[#1A2E26]">تاسكات اليوم</p></div>
              ...
              {tasks.map((t: any) => {
                ...
                return (
                  <button key={t.id} onClick={() => toggleTask(t.id, t.status)} disabled={taskBusy === t.id}
                    className={`w-full text-right flex items-start gap-3 p-3 rounded-xl border transition-all ${done ? 'bg-[#FA8125]/5 border-[#FA8125]/30' : 'bg-white border-gray-100 hover:border-[#FA8125]'}`}>
                    {taskBusy === t.id ? <Loader2 className="w-5 h-5 text-[#FA8125] animate-spin flex-shrink-0 mt-0.5" />
                      : done ? <CheckCircle2 className="w-5 h-5 text-[#FA8125] flex-shrink-0 mt-0.5" />
...
function BookingCard({ b, onAction, onPrep, busy, prepBusyKey }: any) {
  ...
  return (
    <div className={`rounded-2xl border p-4 transition-all ${done ? 'bg-[#FA8125]/5 border-[#FA8125]/30' : running ? 'border-[#FA8125] shadow-sm bg-white' : 'border-gray-100 bg-white'}`}>
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-3">
          <div className="px-2.5 py-1 rounded-lg bg-[#FA8125] text-white font-mono font-black text-sm" dir="ltr">{b.time}</div>
          <div>
        ...
        <div className="text-left flex-shrink-0">
          <span className={`text-[10px] font-bold px-2 py-0.5 rounded ${done ? 'bg-[#FA8125] text-white' : running ? 'bg-amber-100 text-amber-700' : 'bg-[#FAFAF7] text-[#6B7280]'}`}>{STATUS_AR[b.status] || b.status}</span>
          <p className="font-mono font-black text-[#FA8125] text-sm mt-1">{fmt(b.price)} ج</p>
        </div>
      </div>
            ...
            {prepItems.map((it: any) => {
              ...
              return (
                <button key={it.key} onClick={() => onPrep(b.booking_id, it.key, !isDone)} disabled={isBusy} className="w-full text-right flex items-center gap-2.5 group">
                  {isBusy ? <Loader2 className="w-5 h-5 text-[#FA8125] animate-spin flex-shrink-0" />
                    : <span className={`w-5 h-5 rounded-md grid place-items-center flex-shrink-0 transition-all ${isDone ? 'bg-[#FA8125] text-white' : 'border border-gray-300 text-transparent group-hover:border-[#FA8125]'}`}><Check className="w-3.5 h-3.5" /></span>}
      ...
      {!done ? (
        <div className="mt-3">
          {running ? (
            <button onClick={() => onAction(b.booking_id, 'complete')} disabled={busy === b.booking_id} className="w-full py-2.5 rounded-xl bg-[#FA8125] text-white font-black text-sm flex items-center justify-center gap-2 disabled:opacity-50">
              {busy === b.booking_id ? <Loader2 className="w-4 h-4 animate-spin" /> : <CheckCircle2 className="w-4 h-4" />} خلّصت الموعد
            </button>
          ) : (
            <button onClick={() => onAction(b.booking_id, 'start')} disabled={busy === b.booking_id} className="w-full py-2.5 rounded-xl bg-[#FA8125]/10 text-[#FA8125] font-black text-sm flex items-center justify-center gap-2 disabled:opacity-50">
              {busy === b.booking_id ? <Loader2 className="w-4 h-4 animate-spin" /> : <Play className="w-4 h-4" />} ابدأ الموعد
            </button>
          )}
        </div>
      ) : (
        <div className="mt-3 text-center text-xs font-bold text-[#FA8125] flex items-center justify-center gap-1.5"><CheckCircle2 className="w-4 h-4" /> اتعمل ✓</div>
      )}
    </div>
  )
}
