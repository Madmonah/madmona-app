    ...
    <div className="space-y-4">
      <div className="bg-white rounded-2xl border border-gray-100 p-4">
        ...
        <div className="mt-3 grid grid-cols-3 gap-2 text-center">
          <div className="rounded-xl bg-[#2B4521]/8 py-2"><p className="text-[18px] font-black text-[#2B4521]">{present}</p><p className="text-[10px] text-[#6B7280]">حضروا</p></div>
      ...
      {loading ? <div className="py-10 text-center"><Loader2 className="w-6 h-6 text-[#2B4521] animate-spin mx-auto" /></div> : (
...
function AttendanceRow({ r, date, open, onOpen, onSaved }: any) {
  ...
  const chip = r.marked_absent
    ...
    : r.state === 'in' ? <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-[#2B4521]/10 text-[#2B4521]">🟢 داخل</span>
  ...
  return (
    <div className={`rounded-2xl border bg-white transition-all ${open ? 'border-[#2B4521]' : 'border-gray-100'}`}>
      <button onClick={onOpen} className="w-full p-3.5 flex items-center justify-between text-right">
        <div className="flex items-center gap-3 min-w-0">
          <div className="w-9 h-9 rounded-xl bg-[#2B4521]/10 grid place-items-center text-[#2B4521] font-black flex-shrink-0">{(r.full_name || '?').slice(0, 1)}</div>
      ...
      {open && (
        <div className="px-3.5 pb-3.5 pt-1 border-t border-gray-100">
          ...
          <div className="flex gap-2 mt-3">
            <button onClick={() => save(false)} disabled={busy} className="flex-1 h-10 rounded-xl bg-[#2B4521] text-white font-black text-[13px] flex items-center justify-center gap-1.5 disabled:opacity-50">
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-2">
        ...
        <button onClick={() => setForm({})} className="h-11 px-4 rounded-xl bg-[#2B4521] text-white font-black text-[13px] flex items-center gap-1.5"><Plus className="w-4 h-4" /> ضيف</button>
      </div>
      ...
      <div className="space-y-2">
        {filtered.map((e: any) => (
          <div key={e.id} className="rounded-2xl border border-gray-100 bg-white p-3.5 flex items-center justify-between">
            <div className="flex items-center gap-3 min-w-0">
              <div className="w-9 h-9 rounded-xl bg-[#2B4521]/10 grid place-items-center text-[#2B4521] font-black flex-shrink-0">{(e.full_name || '?').slice(0, 1)}</div>
              <div className="min-w-0">
                ...
                {canViewSalary && e.salary != null && <p className="text-[11px] font-bold text-[#2B4521] truncate flex items-center gap-1"><Wallet className="w-3 h-3" /> {Number(e.salary).toLocaleString('en-US')} ج</p>}
            ...
            <div className="flex items-center gap-1.5 flex-shrink-0">
              <button onClick={() => setShiftFor(e)} className="h-9 px-2.5 rounded-xl bg-[#FAFAF7] text-[#2B4521] text-[12px] font-bold flex items-center gap-1"><CalendarDays className="w-3.5 h-3.5" /> الشيفت</button>
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4" dir="rtl" onClick={onClose}>
      <div className="bg-white w-full sm:max-w-md rounded-t-3xl sm:rounded-3xl p-5" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-3">
          ...
          <button onClick={save} disabled={busy} className="w-full h-12 rounded-2xl bg-[#2B4521] text-white font-black text-[14px] flex items-center justify-center gap-2 disabled:opacity-50">
  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4" dir="rtl" onClick={onClose}>
      <div className="bg-white w-full sm:max-w-md rounded-t-3xl sm:rounded-3xl p-5 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        {loading ? <div className="py-8 text-center"><Loader2 className="w-6 h-6 text-[#2B4521] animate-spin mx-auto" /></div> : (
          <div className="space-y-2">
            {days.map((d) => (
              <div key={d.dow} className={`rounded-2xl border p-3 ${d.off ? 'bg-[#FAFAF7] border-gray-100' : 'border-gray-200'}`}>
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => upd(d.dow, { off: !d.off })} className={`text-[11px] font-bold px-2.5 py-1 rounded-full ${d.off ? 'bg-red-100 text-red-700' : 'bg-[#2B4521]/10 text-[#2B4521]'}`}>
                    {d.off ? 'إجازة' : 'شغل'}
                  </button>
                </div>
                {!d.off && (
            ...
            <button onClick={save} disabled={busy} className="w-full h-12 rounded-2xl bg-[#2B4521] text-white font-black text-[14px] flex items-center justify-center gap-2 disabled:opacity-50 sticky bottom-0">
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        {filtered.map((s) => {
          ...
          return (
            <div key={s.id} className={`rounded-2xl border bg-white transition-all ${open ? 'border-[#2B4521]' : 'border-gray-100'}`}>
              <button onClick={() => setOpenId(open ? null : s.id)} className="w-full p-3.5 flex items-center justify-between text-right">
                <div className="flex items-center gap-3 min-w-0">
                  <div className="w-9 h-9 rounded-xl bg-[#2B4521]/10 grid place-items-center text-[#2B4521] flex-shrink-0"><Boxes className="w-4 h-4" /></div>
                  <div className="min-w-0">
              ...
              {open && (
                <div className="px-3.5 pb-3.5 pt-1 border-t border-gray-100 space-y-2">
                  ...
                  {canEdit && (
                    <button onClick={() => setAddFor(s)} className="w-full h-10 rounded-xl bg-[#2B4521]/8 text-[#2B4521] font-black text-[13px] flex items-center justify-center gap-1.5">
  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4" dir="rtl" onClick={onClose}>
      <div className="bg-white w-full sm:max-w-md rounded-t-3xl sm:rounded-3xl p-5 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="max-h-48 overflow-y-auto space-y-1 mb-3">
          {filtered.slice(0, 80).map((p) => (
            <button key={p.id} onClick={() => setProductId(p.id)}
              className={`w-full text-right px-3 py-2 rounded-xl border text-[13px] font-bold flex items-center justify-between ${productId === p.id ? 'border-[#2B4521] bg-[#2B4521]/5 text-[#2B4521]' : 'border-gray-200 text-[#1A2E26]'}`}>
        ...
        <button onClick={save} disabled={busy || !productId} className="w-full h-12 mt-3 rounded-2xl bg-[#2B4521] text-white font-black text-[14px] flex items-center justify-center gap-2 disabled:opacity-50">
