      ...
      <div className="max-w-4xl mx-auto">
        <div className="mb-6">
          ...
          <p className="text-sm text-gray-500">
            ...
            <span className="text-[#2B4521] font-bold">{done} مكتمل</span>
          </p>
        </div>
              ...
              <div key={p.id} className="bg-white rounded-3xl border border-gray-100 overflow-hidden shadow-sm">
                ...
                <div className="relative h-48 sm:h-56">
                  {p.cover_url ? (
                    ...
                    <div className="w-full h-full bg-gradient-to-br from-[#2B4521] to-[#2FA084] flex items-center justify-center">
                  ...
                  <div className="absolute bottom-3 right-4 left-4 flex items-end justify-between gap-3">
                    ...
                    {savedId === p.id && (
                      <span className="bg-white/95 text-[#2B4521] text-[11px] font-black px-2.5 py-1 rounded-full flex items-center gap-1 shrink-0">
                ...
                <div className="p-5 space-y-4">
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-xs font-black text-[#1A2E26] flex items-center gap-1.5">
                        <Images className="w-4 h-4 text-[#2B4521]" /> معرض الصور
                        <span className="text-gray-400 font-normal">({photos.length})</span>
                      </span>
                      <label className="text-[11px] font-bold text-[#2B4521] cursor-pointer hover:underline flex items-center gap-1">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    <div>
                      <span className="text-xs font-black text-[#1A2E26] flex items-center gap-1.5 mb-2">
                        <FileText className="w-4 h-4 text-[#2B4521]" /> البروشور
                      </span>
                      {p.brochure_url ? (
                        <a href={p.brochure_url} target="_blank" rel="noopener"
                          className="flex items-center gap-2 bg-[#FAFAF7] border border-gray-100 rounded-xl px-3 py-2.5 text-xs font-bold text-[#2B4521] hover:bg-[#2FA084]/10">
                  ...
                  <a href="/real-estate/market" target="_blank" rel="noopener"
                    className="inline-flex items-center gap-1 text-xs text-[#2B4521] font-bold hover:underline">
...
function VideoBox({
  ...
  return (
    <div>
      <span className="text-xs font-black text-[#1A2E26] flex items-center gap-1.5 mb-2">
        <Video className="w-4 h-4 text-[#2B4521]" /> الفيديو
      </span>
      {p.video_url ? (
        <div className="flex items-center gap-2">
          <a href={p.video_url} target="_blank" rel="noopener"
            className="flex-1 flex items-center gap-2 bg-[#FAFAF7] border border-gray-100 rounded-xl px-3 py-2.5 text-xs font-bold text-[#2B4521] hover:bg-[#2FA084]/10">
        ...
        <div className="space-y-1.5">
          <label className="flex items-center justify-center gap-1.5 border-2 border-dashed border-gray-200 rounded-xl py-2.5 cursor-pointer hover:border-[#2FA084] hover:bg-[#2FA084]/5">
            {busy ? <Loader2 className="w-4 h-4 animate-spin text-[#2B4521]" /> : <Video className="w-4 h-4 text-gray-400" />}
          ...
          <div className="flex gap-1.5">
            ...
            {link.trim() && (
              <button onClick={() => { onLink(p, link.trim()); setLink('') }} disabled={busy}
                className="px-3 py-2 text-xs font-bold rounded-xl bg-[#2B4521] text-white hover:bg-[#185849] disabled:opacity-50">
                احفظ
              </button>
            )}
          </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {changed && (
          <button onClick={() => onSave(p, from, to)} disabled={busy}
            className="px-3.5 py-1.5 text-xs font-black rounded-xl bg-[#2B4521] text-white hover:bg-[#185849] disabled:opacity-50">
            {busy ? '...' : 'احفظ'}
          </button>
        )}
      </div>
