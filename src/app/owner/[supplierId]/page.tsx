    ...
    return (
      <div className="min-h-screen bg-[#FAFAF7] flex items-center justify-center p-4" dir="rtl">
        <div className="bg-white rounded-3xl border border-gray-100 p-8 max-w-md text-center">
          ...
          <button onClick={logout} className="mt-4 px-5 py-2.5 rounded-xl bg-[#1F6F5F] text-white text-sm font-bold">تسجيل خروج</button>
        </div>
      </div>
    )
  }
            ...
            <div className="space-y-2">
              {(kpis?.top_services || []).slice(0, 5).map((s: any, i: number) => (
                <div key={i} className="flex items-center justify-between text-sm">
                  ...
                  <span className="font-mono text-[#1F6F5F] font-bold">{Number(s.revenue || 0).toLocaleString()} ج</span>
                </div>
              ))}
            ...
            <div className="space-y-2">
              {(kpis?.by_branch || []).map((b: any, i: number) => (
                <div key={i} className="flex items-center justify-between text-sm">
                  ...
                  <span className="font-mono text-[#1F6F5F] font-bold">{Number(b.revenue || 0).toLocaleString()} ج</span>
                </div>
              ))}
