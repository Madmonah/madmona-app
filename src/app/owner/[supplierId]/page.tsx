...
export default function OwnerDashboard({ params }: { params: { supplierId: string } }) {
    ...
    return (
      <div className="min-h-screen bg-[#FAFAF7] flex items-center justify-center p-4" dir="rtl">
        <div className="bg-white rounded-3xl border border-gray-100 p-8 max-w-md text-center">
          ...
          <button onClick={logout} className="mt-4 px-5 py-2.5 rounded-xl bg-[#34D399] text-[#04352A] text-sm font-bold">تسجيل خروج</button>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="relative min-h-screen bg-[#FAFAF7] text-[#1A2E26] overflow-x-hidden" dir="rtl">
      ...
      <header className="relative overflow-hidden text-white bg-gradient-to-l from-[#D4A017] via-[#2FA084] to-[#34D399]">
          ...
          <div className="grid grid-cols-3 md:grid-cols-5 gap-3">
            {modules.map(m => (
              <Link
                ...
                className="group bg-white rounded-2xl p-4 border border-black/5 shadow-sm shadow-black/[0.04] flex flex-col items-center gap-2 hover:shadow-md hover:border-[#059669]/30 hover:-translate-y-0.5 transition-all"
              >
                <div className="w-11 h-11 rounded-xl bg-[#34D399]/10 text-[#059669] flex items-center justify-center group-hover:bg-gradient-to-br group-hover:from-[#2FA084] group-hover:to-[#34D399] group-hover:text-[#04352A] transition-colors"><div className="w-5 h-5">{m.icon}</div></div>
            ...
            <div className="space-y-2">
              {(kpis?.top_services || []).slice(0, 5).map((s: any, i: number) => (
                <div key={i} className="flex items-center justify-between text-sm">
                  ...
                  <span className="font-mono text-[#059669] font-bold">{Number(s.revenue || 0).toLocaleString()} ج</span>
                </div>
              ))}
            ...
            <div className="space-y-2">
              {(kpis?.by_branch || []).map((b: any, i: number) => (
                <div key={i} className="flex items-center justify-between text-sm">
                  ...
                  <span className="font-mono text-[#059669] font-bold">{Number(b.revenue || 0).toLocaleString()} ج</span>
                </div>
              ))}
...
function OwnerStat({ icon, label, value, primary }: any) {
  return (
    <div className={`rounded-2xl p-4 border transition-all ${primary ? 'bg-gradient-to-br from-[#D4A017] via-[#2FA084] to-[#34D399] border-transparent text-white shadow-lg shadow-[#059669]/25' : 'bg-white border-black/5 shadow-sm shadow-black/[0.04]'}`}>
