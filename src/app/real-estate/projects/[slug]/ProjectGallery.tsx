          ...
          {urls.map((u, i) => (
            <button
              ...
              className={`shrink-0 rounded-xl overflow-hidden border-2 transition-all ${
                i === active
                  ? 'border-[#1F6F5F] ring-2 ring-[#1F6F5F]/20'
...
function VideoBlock({
  ...
  if (!playing) {
    return (
      <button
        onClick={() => setPlaying(true)}
        className="mt-3 flex items-center justify-center gap-2 w-full bg-white border-2 border-[#1F6F5F]/20 text-[#1F6F5F] font-semibold py-3.5 rounded-2xl hover:bg-[#1F6F5F]/5 hover:border-[#1F6F5F]/40 transition-all"
      >
