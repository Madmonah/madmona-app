...
// Public Supabase client used for client-side file uploads to the
// payment-proofs bucket. Anon-key only (no privileged operations).
const supabasePublic = createClient(
  process.env.NEXT_PUBLIC_SUPABASE_URL!,
  process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!
)

const SPACE_SLUG = 'meeting-room'
const SPACE_NAME = 'غرفة الاجتماعات'
const WHATSAPP_PHONE = '201002229982'
const INSTAPAY_HANDLE = process.env.NEXT_PUBLIC_INSTAPAY_HANDLE || '01002229982'

const CAPACITIES = [
  { id: '4-people', label: 'حتى ٤ أشخاص', priceLabel: '٣٠٠ جنيه/ساعة', price: 300 },
  { id: '8-people', label: 'حتى ٨ أشخاص', priceLabel: '٥٠٠ جنيه/ساعة', price: 500 },
] as const
...
// Generate the next N days as { iso, label } chips for the calendar strip.
...
export default function ReserveMeetingRoomPage() {
  ...
  const [selectedCapacity, setSelectedCapacity] =
    useState<typeof CAPACITIES[number]['id'] | null>(null)
  ...
  // Pricing
  const capacityRow = CAPACITIES.find((c) => c.id === selectedCapacity)
  const totalPrice = capacityRow ? capacityRow.price * selectedDuration : 0

  // Validate that the selected (start, duration) span is fully available
  const spanAvailable = useMemo(() => {
    ...
    return true
  }, [hours, selectedStartHour, selectedDuration])

  // Available durations starting from selectedStartHour (won't go past blocked slots
  // or the operating-end-of-day)
  const availableDurations = useMemo(() => {
    if (selectedStartHour === null) return DURATIONS
  ...
  // ----- File upload (InstaPay screenshot) -----
    ...
    try {
      const res = await fetch('/api/bookings', {
        ...
        body: JSON.stringify({
          ...
          capacityOption: selectedCapacity,
          bookingDate: selectedDate,
          startHour: selectedStartHour,
          endHour: selectedStartHour! + selectedDuration,
        ...
        <div className="max-w-md w-full bg-white rounded-2xl border border-gray-100 p-8 shadow-sm">
          ...
          <dl className="space-y-2.5 mb-6 text-sm">
            ...
            <div className="flex justify-between">
              ...
              <dd className="text-gray-900 font-medium">{capacityRow?.label}</dd>
            </div>
            <div className="flex justify-between">
          ...
          <Link
            href="/"
            className="block text-center text-sm text-gray-500 hover:text-[#1F5F3F]"
          >
            الصفحة الرئيسية
          </Link>
        </div>
      ...
      <main className="max-w-md mx-auto px-4 py-6 pb-32">
        ...
        <section className="mb-6">
          ...
          <div className="grid grid-cols-2 gap-3">
            {CAPACITIES.map((c) => {
              const selected = selectedCapacity === c.id
              return (
                <button
                  key={c.id}
                  type="button"
                ...
                >
                  ...
                  <div className={`text-sm ${selected ? 'text-[#B8860B]' : 'text-gray-500'}`}>
                    {c.priceLabel}
                  </div>
                </button>
              )
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      ...
                      // Reset duration if it would go past available range
                      const slotsAfter = hours.filter(
                        (s) => s.hour >= slot.hour && s.available
                      )
                      let consecutive = 0
                    ...
                    className={`py-2.5 rounded-lg text-sm font-medium transition-colors ${
                      !slot.available
                        ...
                        : 'bg-white border border-gray-200 text-gray-700 hover:border-[#1F5F3F]/40'
                    }`}
                  >
                    {hourLabel(slot.hour)}
                  </button>
        ...
        {/* === InstaPay details + upload === */}
        {paymentMethod === 'instapay' && (
          <section className="mb-6 bg-[#B8860B]/5 border border-[#B8860B]/20 rounded-xl p-4">
            <p className="text-sm text-gray-700 mb-3 leading-relaxed">
              حوّل <span className="font-bold text-[#1F5F3F]">{formatPrice(totalPrice)}</span> على{' '}
              InstaPay لـ:
            </p>
            ...
            <label className="block">
              ...
              <input
                ...
                disabled={uploading}
                className="block w-full text-sm text-gray-500 file:ml-3 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-[#1F5F3F]/10 file:text-[#1F5F3F] hover:file:bg-[#1F5F3F]/20 disabled:opacity-50"
              />
              {uploading && (
                <p className="text-xs text-gray-500 mt-2">جاري رفع الصورة...</p>
              )}
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 px-4 py-3 z-40">
          <div className="max-w-md mx-auto flex items-center gap-3">
            <div className="flex-1">
              ...
              <div className="text-lg font-bold text-[#1F5F3F]">
