...
export default function ReviewPage({ params }: { params: { bookingId: string } }) {
    ...
    return (
      <div className="min-h-screen bg-[#FAFAF7] flex items-center justify-center p-4" dir="rtl">
        <div className="bg-white rounded-3xl border border-gray-100 p-8 max-w-md text-center">
          <CheckCircle2 className="w-16 h-16 text-[#FA8125] mx-auto mb-3" />
  ...
  if (done) {
    return (
      <div className="min-h-screen bg-[#FAFAF7] flex items-center justify-center p-4" dir="rtl">
        <div className="bg-white rounded-3xl border border-gray-100 p-8 max-w-md text-center">
          <div className="w-20 h-20 rounded-full bg-[#FA8125] grid place-items-center mx-auto mb-4">
          ...
          {rating >= 4 && (
            <a href="https://maps.google.com" target="_blank" rel="noopener" className="inline-block mt-4 px-5 py-2.5 rounded-xl bg-[#FA8125] text-white text-sm font-bold">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-5 text-center">
          ...
          <div className="flex justify-center gap-2 mt-5">
            {[1, 2, 3, 4, 5].map(s => (
              ...
              >
                <Star className={`w-10 h-10 ${(hoverRating || rating) >= s ? 'fill-[#FA8125] text-[#FA8125]' : 'text-gray-300'}`} />
              </button>
            ))}
          </div>
          {rating > 0 && (
            <p className="text-sm font-bold text-[#FA8125] mt-3">
            ...
            <div className="flex justify-center gap-1.5">
              {[1, 2, 3, 4, 5].map(s => (
                <button key={s} onClick={() => setStylistRating(s)} className="transition-transform hover:scale-110">
                  <Star className={`w-7 h-7 ${stylistRating >= s ? 'fill-[#FA8125] text-[#FA8125]' : 'text-gray-300'}`} />
                </button>
              ))}
            </div>
          </div>
