...
// 🌍 (٢/٩/٢٠٢٦) الليبل بقى **مفتاح ترجمة** مش نص — بيتترجم وقت العرض.
const TABS: Array<{ key: Tab; labelKey: string; icon: React.ElementType }> = [
  { key: 'overview',  labelKey: 'erp.overview',  icon: LayoutDashboard },
  { key: 'finance',   labelKey: 'erp.finance',   icon: Wallet },
  { key: 'staff',     labelKey: 'erp.staff',     icon: Users },
  { key: 'inventory', labelKey: 'erp.inventory', icon: Package },
  { key: 'products',  labelKey: 'erp.products',  icon: ShoppingBag },
  { key: 'crm',       labelKey: 'erp.crm',       icon: Phone },
]
...
function ErpInner() {
  ...
  // 🌍 (٢ سبتمبر ٢٠٢٦) محمد: «عايز يبقى فيه ترجمة لصفحات الإدارة».
  const { t } = useT()
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!data?.ok) throw new Error(data?.error || t('erp.no_data'))
      setD(data)
    } catch (e) {
      setErr(e instanceof Error ? e.message : t('erp.error'))
  ...
  if (err || !d) {
    return (
      <div dir="rtl" style={{ maxWidth: 480, margin: '80px auto', padding: 24, textAlign: 'center' }}>
        ...
        <p style={{ fontSize: 15, fontWeight: 700, color: INK, margin: '0 0 6px' }}>{err || t('erp.no_data')}</p>
    ...
    <div dir="rtl" style={{ minHeight: '100dvh', background: '#F7F9F7', color: INK, fontFamily: 'inherit' }}>
      ...
      <main style={{ maxWidth: 1080, margin: '0 auto', padding: '18px 16px 60px' }}>
        ...
        {tab === 'overview' && (
          <>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 10 }}>
              <Stat label={t('erp.net_month')} value={`${EGP(net)} ج`} tone={net >= 0 ? 'green' : 'red'} icon={net >= 0 ? TrendingUp : TrendingDown} />
              <Stat label={t('erp.cash')} value={`${EGP(f.cash)} ج`} icon={Wallet} />
              <Stat label={t('erp.present_today')} value={`${N(e.present_today)} / ${N(e.total)}`} icon={Users} />
              <Stat label={t('erp.tasks_pct')} value={`${taskPct}٪`} tone={taskPct >= 60 ? 'green' : taskPct >= 30 ? 'amber' : 'red'} icon={CheckCircle2} />
              <Stat label={t('erp.followups_due')} value={N(c.followups_due)} tone={c.followups_due > 0 ? 'amber' : undefined} icon={Clock} />
              <Stat label={t('erp.low_stock')} value={N(inv.low_stock)} tone={inv.low_stock > 0 ? 'red' : undefined} icon={Package} />
            </div>

            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 14, marginTop: 18 }}>
              <Panel title={t('erp.staff_today')} action={() => setTab('staff')}>
                {e.list.length === 0 ? <Empty text={t('erp.no_staff')} /> : e.list.slice(0, 6).map(m => (
                  <Row key={m.id}>
                    <Dot on={m.present} />
              ...
              <Panel title={t('erp.latest_entries')} action={() => setTab('finance')}>
                {d.journal.length === 0 ? <Empty text={t('erp.no_entries_month')} /> : d.journal.slice(0, 6).map(j => (
                  <Row key={j.id}>
              ...
              <Panel title={t('erp.latest_customers')} action={() => setTab('crm')}>
                {c.recent.length === 0 ? <Empty text={t('erp.no_customers')} /> : c.recent.slice(0, 6).map((r, i) => (
                  <Row key={i}>
                    <Status s={r.status} t={t} />
        ...
        {tab === 'finance' && (
          <>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 10 }}>
              <Stat label={t('erp.revenue_month')} value={`${EGP(f.revenue_month)} ج`} tone="green" />
              <Stat label={t('erp.expense_month')} value={`${EGP(f.expense_month)} ج`} tone="red" />
              <Stat label={t('erp.net')} value={`${EGP(net)} ج`} tone={net >= 0 ? 'green' : 'red'} />
              <Stat label={t('erp.cash')} value={`${EGP(f.cash)} ج`} />
              <Stat label={t('erp.accounts_chart')} value={N(f.accounts)} />
              <Stat label={t('erp.entries_draft')} value={N(f.entries_draft)} tone={f.entries_draft > 0 ? 'amber' : undefined} />
            </div>
            <Panel title={t('erp.latest_entries')} style={{ marginTop: 18 }} link="/supplier/erp/accounting" linkLabel={t('erp.journal_full')}>
              {d.journal.length === 0 ? <Empty text={t('erp.no_entries_start')} /> : (
                <table style={{ width: '100%', fontSize: 13, borderCollapse: 'collapse' }}>
                  <thead><tr style={{ color: MUTED, fontSize: 11.5 }}>
                    <th style={th}>{t('erp.date')}</th><th style={th}>{t('erp.memo')}</th><th style={{ ...th, textAlign: 'left' }}>{t('erp.amount')}</th><th style={th}>{t('erp.status')}</th>
                  </tr></thead>
                  <tbody>{d.journal.map(j => (
                    <tr key={j.id} style={{ borderTop: `1px solid ${LINE}` }}>
                      ...
                      <td style={td}><Pill text={j.status === 'posted' ? t('erp.st_posted') : t('erp.st_draft')} tone={j.status === 'posted' ? 'green' : 'amber'} /></td>
                    </tr>
                  ))}</tbody>
                </table>
              )}
        ...
        {tab === 'staff' && (
          <>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 10 }}>
              <Stat label={t('erp.team')} value={N(e.total)} />
              <Stat label={t('erp.present')} value={N(e.present_today)} tone={e.present_today >= e.total * 0.7 ? 'green' : 'amber'} />
              <Stat label={t('erp.tasks_today')} value={N(e.tasks_today)} />
              <Stat label={t('erp.tasks_done')} value={`${N(e.tasks_done)} (${taskPct}٪)`} tone={taskPct >= 60 ? 'green' : 'amber'} />
            </div>
            <Panel title={t('erp.team')} style={{ marginTop: 18 }} link={b.is_platform ? '/admin/staff' : undefined} linkLabel={t('erp.manage_team')}>
              {e.list.map(m => {
        ...
        {tab === 'inventory' && (
          <>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 10 }}>
              <Stat label={t('erp.items')} value={N(inv.products)} />
              <Stat label={t('erp.low')} value={N(inv.low_stock)} tone={inv.low_stock > 0 ? 'red' : 'green'} />
              <Stat label={t('erp.stock_value')} value={`${EGP(inv.value)} ج`} />
              <Stat label={t('erp.movements_month')} value={N(inv.movements_month)} />
            </div>
            <Panel title={t('erp.items')} style={{ marginTop: 18 }} link="/supplier/erp/materials" linkLabel={t('erp.manage_inventory')}>
              {inv.list.length === 0 ? <Empty text={t('erp.no_items')} /> : (
                <table style={{ width: '100%', fontSize: 13, borderCollapse: 'collapse' }}>
                  <thead><tr style={{ color: MUTED, fontSize: 11.5 }}>
                    <th style={th}>{t('erp.item')}</th><th style={{ ...th, textAlign: 'center' }}>{t('erp.qty')}</th><th style={{ ...th, textAlign: 'left' }}>{t('erp.cost')}</th><th style={{ ...th, textAlign: 'left' }}>{t('erp.sell')}</th>
                  </tr></thead>
        ...
        {tab === 'products' && (
          <>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 10 }}>
              <Stat label={t('erp.listings_published')} value={N(p.listings)} tone="green" />
              <Stat label={t('erp.orders_month')} value={N(p.orders_month)} />
              <Stat label={t('erp.bookings_month')} value={N(p.bookings_month)} />
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 10, marginTop: 18 }}>
              <Shortcut href={b.is_platform ? '/admin/listings' : '/supplier/erp/products'} label={t('erp.listings_products')} desc={t('erp.listings_products_desc')} icon={ShoppingBag} />
              <Shortcut href="/supplier/erp/catalog" label={t('erp.catalog')} desc={t('erp.catalog_desc')} icon={BookOpen} />
              {b.is_platform && <Shortcut href="/real-estate/market" label={t('erp.bourse')} desc={t('erp.bourse_desc')} icon={Building2} />}
            </div>
          </>
        )}
        ...
        {tab === 'crm' && (
          <>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 10 }}>
              <Stat label={t('erp.contacts_all')} value={N(c.contacts)} />
              <Stat label={t('erp.leads_new')} value={N(c.leads_new)} />
              <Stat label={t('erp.interested')} value={N(c.interested)} tone="green" />
              <Stat label={t('erp.followups_due')} value={N(c.followups_due)} tone={c.followups_due > 0 ? 'amber' : undefined} />
              <Stat label={t('erp.calls_today')} value={N(c.calls_today)} />
              <Stat label={t('erp.calls_month')} value={N(c.calls_month)} />
              <Stat label={t('erp.customers_real')} value={N(c.customers)} tone="green" />
            </div>
            <Panel title={t('erp.last_contact')} style={{ marginTop: 18 }} link={b.is_platform ? '/crm' : '/supplier/erp/crm'} linkLabel={t('erp.my_calls')}>
              {c.recent.length === 0 ? <Empty text={t('erp.no_contacts')} /> : c.recent.map((r, i) => (
                <Row key={i}>
                  <Status s={r.status} t={t} />
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ margin: 0, fontSize: 13.5, fontWeight: 700 }}>{r.name || r.phone}</p>
                    <p style={{ margin: 0, fontSize: 11.5, color: MUTED }}>
                      {r.last ? `آخر تواصل ${ago(r.last)}` : t('erp.st_notyet')}
...
// 🌍 (٢/٩) الكومبوننت بره الصفحة فمالوش t — بياخد المترجم كـprop.
function Status({ s, t }: { s: string; t: (k: string) => string }) {
  const map: Record<string, ['green' | 'amber' | 'red', string]> = {
    interested: ['green', t('erp.interested')], won: ['green', t('erp.st_agreed')], contacted: ['amber', t('erp.st_talked')],
    new: ['amber', t('erp.leads_new')], lost: ['red', t('erp.st_refused')], spam: ['red', t('erp.st_wrong')],
  }
