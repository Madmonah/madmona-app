...
export default function SupplierProductsPage() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      /* 🔑 (٢٠ أغسطس ٢٠٢٦) الصفحة دي كانت لسه على الفحص القديم:
         مالك الحساب أو `supplier_staff.can_manage_listings` — يعني موظف
         مضمونة كان بيتقفل عليها حتى بعد ما اتفتحت له صفحة تعديل الإعلان.
         محمد: «أنا عايز الأدمن وموظفين مضمونة يعدّلوا أي إعلان — دول أدمن».

         بقت بتسأل **نفس دالة الـRLS** (`can_edit_supplier_listings`) —
         فاللي الواجهة بتقوله هو اللي الحفظ هيعمله بالظبط، من غير مفاجآت. */
      ...
      <header className="sticky top-0 z-40 glass border-b border-white/40">
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center gap-2">
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 bg-white border border-[#059669]/30 text-[#059669] px-3 py-2 rounded-xl font-bold text-xs shadow-soft hover:shadow-card hover:-translate-y-0.5 transition-all"
          ...
          <button
            onClick={openNew}
            className="inline-flex items-center gap-1.5 bg-[#34D399] text-[#04352A] px-4 py-2 rounded-xl font-bold text-xs shadow-soft hover:shadow-card hover:-translate-y-0.5 transition-all"
          >
        ...
        {products.length === 0 ? (
          <div className="bg-white rounded-3xl shadow-card p-10 text-center max-w-md mx-auto">
            ...
            <div className="flex items-center justify-center gap-2">
              <button
                onClick={() => setShowImport(true)}
                className="inline-flex items-center gap-2 bg-white border-2 border-[#059669] text-[#059669] px-5 py-3 rounded-2xl font-bold shadow-soft hover:-translate-y-0.5 transition-all"
              >
                <FileSpreadsheet className="w-4 h-4" />
                استيراد Excel
              </button>
              <button
                onClick={openNew}
                className="inline-flex items-center gap-2 bg-[#34D399] text-[#04352A] px-5 py-3 rounded-2xl font-bold shadow-elevated hover:-translate-y-0.5 hover:shadow-luxe transition-all"
              >
                <Plus className="w-4 h-4" />
                منتج جديد
              </button>
          ...
          <div className="bg-white w-full md:max-w-md rounded-t-3xl md:rounded-3xl shadow-luxe max-h-[92vh] overflow-y-auto animate-slide-up">
            ...
            <div className="sticky bottom-0 bg-white border-t border-gray-100 p-4 flex gap-2">
              ...
              <button
                onClick={save}
                disabled={saving}
                className="flex-1 bg-[#34D399] text-[#04352A] py-3 rounded-2xl font-bold text-sm shadow-elevated hover:shadow-luxe disabled:opacity-60 transition-all flex items-center justify-center gap-2"
              >
