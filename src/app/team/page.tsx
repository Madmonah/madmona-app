...
import ContactBookSheet from '@/components/ContactBookSheet'
...
export default function TeamPage() {
  ...
  const loadRooms = useCallback(async (myId: string) => {
    ...
    // كتم / أرشفة / تثبيت — من صف عضويتي في كل غرفة
    if (myId) {
      try {
        const { data: mine } = await supabaseBrowser.from('chat_room_members')
          .select('room_id, muted_until, archived_at, pinned_at').eq('profile_id', myId)
        const byRoom = new Map<string, { muted_until: string | null; archived_at: string | null; pinned_at: string | null }>()
        for (const r of (mine || []) as { room_id: string; muted_until: string | null; archived_at: string | null; pinned_at: string | null }[]) byRoom.set(r.room_id, r)
        rooms = rooms.map((r) => {
          const f = byRoom.get(r.id)
          return { ...r, mutedUntil: f?.muted_until || null, archivedAt: f?.archived_at || null, roomPinnedAt: f?.pinned_at || null }
        })
        // المثبّت فوق، والأرشيف بيتفلتر في العرض
        rooms = [...rooms].sort((a, b) => (a.roomPinnedAt ? 0 : 1) - (b.roomPinnedAt ? 0 : 1))
      } catch { /* non-blocking */ }
    }
    setRooms(rooms)
  }, [])
