...
export default function VisitHub({ params }: { params: { branchCode: string } }) {
  ...
  if (loading || checking) return <div className="min-h-screen bg-[#34D399] flex items-center justify-center"><Loader2 className="w-9 h-9 text-white animate-spin" /></div>
...
function Hero({ biz, branchName, tagline, authed, name, gallery = [], logo }: any) {
  ...
  return (
    <header className="relative bg-[#34D399] text-[#04352A] overflow-hidden">
      ...
      <div className="absolute inset-0 bg-gradient-to-b from-[#34D399]/70 via-[#34D399]/55 to-[#1A2E26]/90" />
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-1.5 flex gap-1 mb-4 shadow-[0_8px_24px_-14px_rgba(26,46,38,0.25)]">
        {tabs.map((tb: any) => (
          <button key={tb.k} onClick={() => setTab(tb.k)}
            className={`flex-1 py-2.5 px-1 rounded-xl text-[12px] font-black flex items-center justify-center gap-1 transition-all ${tab === tb.k ? 'bg-[#34D399] text-[#04352A] shadow-md shadow-[#059669]/25' : 'text-[#6B7280] hover:text-[#1A2E26]'}`}>
...
function BookTab({ branchCode, router, setPanel, hasProducts }: any) {
  const { t } = useT()
  const tile = 'bg-white border border-gray-100 rounded-2xl p-4 text-start active:scale-[0.98] transition-all hover:border-[#059669]/40 hover:shadow-md hover:shadow-[#1A2E26]/5'
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        {hasProducts && (
          <button onClick={() => setPanel('products')} className={tile}>
            <div className="w-10 h-10 rounded-xl bg-[#34D399]/10 text-[#059669] grid place-items-center mb-2.5"><ShoppingBag className="w-5 h-5" /></div>
        ...
        <button onClick={() => router.push('/home')} className={tile}>
          <div className="w-10 h-10 rounded-xl bg-[#34D399]/10 text-[#059669] grid place-items-center mb-2.5"><User className="w-5 h-5" /></div>
      ...
      <div className="bg-[#34D399]/[0.06] border border-[#059669]/15 rounded-2xl p-4">
        <p className="text-[11px] font-black tracking-wider uppercase text-[#059669] mb-0.5 flex items-center gap-1.5"><Sparkles className="w-3.5 h-3.5" /> {t('visit.after_service')}</p>
        <p className="text-[12px] text-[#6B7280] mb-3">{t('visit.after_service_sub')}</p>
        <div className="grid grid-cols-2 gap-3">
          <button onClick={() => setPanel('tip')} className={tile}>
            <div className="w-10 h-10 rounded-xl bg-[#34D399]/10 text-[#059669] grid place-items-center mb-2.5"><Gift className="w-5 h-5" /></div>
          ...
          <button onClick={() => setPanel('rate')} className={tile}>
            <div className="w-10 h-10 rounded-xl bg-[#34D399]/10 text-[#059669] grid place-items-center mb-2.5"><Star className="w-5 h-5" /></div>
      ...
      {services.map((s: any) => (
        <button key={s.id} onClick={() => router.push(`/book/${branchCode}?service=${s.id}`)}
          ...
          <div className="text-end flex items-center gap-2">
            <span className="font-mono font-black text-[#059669] text-sm">{Number(s.price_egp || 0).toLocaleString('en-US')} {t('visit.egp')}</span>
            <span className="w-7 h-7 rounded-lg bg-[#34D399]/10 text-[#059669] grid place-items-center"><ChevronRight className="w-4 h-4 rtl:rotate-180" /></span>
          </div>
        </button>
      ))}
    </div>
  ...
  return (
    <div className="space-y-3">
      {ig && (
        <a href={`https://instagram.com/${ig}`} target="_blank" rel="noopener noreferrer"
          ...
          <div className="flex items-center gap-3">
            <div className="w-11 h-11 rounded-xl bg-[#34D399]/10 text-[#059669] grid place-items-center"><Instagram className="w-6 h-6" /></div>
            <div>
              <p className="font-black text-sm text-[#1A2E26]">{t('visit.instagram')}</p>
              <p className="text-[12px] text-[#6B7280]" dir="ltr">@{ig}</p>
            </div>
          </div>
          <span className="text-[12px] font-black text-[#059669]">{t('visit.follow_us')}</span>
        </a>
      )}
      ...
      <a href={`https://wa.me/${WA}`} className="flex items-center justify-between bg-white border border-gray-100 rounded-2xl p-4 active:scale-[0.99] transition-all hover:shadow-md hover:shadow-[#1A2E26]/5">
        <div className="flex items-center gap-3">
          <div className="w-11 h-11 rounded-xl bg-[#34D399]/10 text-[#059669] grid place-items-center"><MessageCircle className="w-6 h-6" /></div>
          <div>
...
function MadmonaTab({ router }: any) {
  ...
  return (
    <div className="space-y-3">
      <div className="bg-[#34D399] text-[#04352A] rounded-2xl p-5 text-center shadow-lg shadow-[#059669]/20">
        <div className="inline-flex items-center gap-1.5 mb-1.5">
          <span className="w-7 h-7 rounded-lg bg-white grid place-items-center text-[#059669] font-black">م</span>
      ...
      {items.map((a: any, i: number) => (
        <button key={i} onClick={a.onClick} className="w-full bg-white border border-gray-100 rounded-2xl p-4 flex items-center justify-between text-start active:scale-[0.99] transition-all hover:border-[#059669]/40 hover:shadow-md hover:shadow-[#1A2E26]/5">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-[#34D399]/10 text-[#059669] grid place-items-center"><a.icon className="w-5 h-5" /></div>
            <div>
...
function TipFlow({ branchCode, stylists, onBack }: any) {
  ...
  if (done) return (
    <SuccessCard onBack={onBack} icon={Heart} title={t('visit.tip_done_title')}>
      <p className="text-sm text-[#6B7280] mb-3">{t('visit.tip_recorded')} <b className="text-[#059669]">{Number(finalAmount).toLocaleString('en-US')} {egp}</b>{done.employee_name ? ` — ${done.employee_name}` : ''}.</p>
      {method === 'instapay' && done.payout_details
        ? <div className="bg-[#FAFAF7] rounded-2xl p-4 text-sm border border-[#059669]/15">
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex items-center justify-between"><span className="text-[12px] text-[#6B7280]">{t('visit.account_or_instapay')}</span><span className="font-mono font-black text-[#059669] select-all" dir="ltr">{done.payout_details}</span></div>
            </div>
          </div>
  ...
  return (
    <Sheet title={t('visit.tip_title')} onBack={onBack}>
      ...
      <div className="grid grid-cols-4 gap-2 mb-2">
        {chips.map((c) => (
          <button key={c} onClick={() => { setAmount(c); setCustom('') }} className={`py-2.5 rounded-xl font-black text-sm border transition-all ${amount === c ? 'bg-[#34D399] text-[#04352A] border-[#059669]' : 'bg-white text-[#1A2E26] border-gray-200'}`}>{c}</button>
        ))}
      </div>
      <input value={custom} onChange={(e) => { setCustom(e.target.value.replace(/[^0-9]/g, '')); setAmount(null) }} inputMode="numeric" placeholder={t('visit.custom_amount_ph')} className="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm mb-4 focus:border-[#059669] outline-none" />
      <p className="text-xs font-bold text-[#1A2E26] mb-2">{t('visit.payment_method')}</p>
      <div className="grid grid-cols-2 gap-2 mb-4">
        <button onClick={() => setMethod('instapay')} className={`py-2.5 rounded-xl font-bold text-sm border ${method === 'instapay' ? 'bg-[#34D399] text-[#04352A] border-[#059669]' : 'bg-white text-[#1A2E26] border-gray-200'}`}>{t('visit.instapay')}</button>
        <button onClick={() => setMethod('cash')} className={`py-2.5 rounded-xl font-bold text-sm border ${method === 'cash' ? 'bg-[#34D399] text-[#04352A] border-[#059669]' : 'bg-white text-[#1A2E26] border-gray-200'}`}>{t('visit.cash')}</button>
      </div>
      <input value={name} onChange={(e) => setName(e.target.value)} placeholder={t('visit.your_name_opt')} className="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm mb-4 focus:border-[#059669] outline-none" />
      <button onClick={submit} disabled={busy || !finalAmount} className="w-full py-3.5 rounded-2xl bg-[#34D399] text-[#04352A] font-black text-sm flex items-center justify-center gap-2 disabled:opacity-40 shadow-lg shadow-[#059669]/20 active:scale-[0.99] transition-all">
  ...
  return (
    <Sheet title={t('visit.rate_title')} onBack={onBack}>
      ...
      <div className="flex items-center justify-center gap-2 mb-5">
        {[1, 2, 3, 4, 5].map((n) => (
          <button key={n} onClick={() => setRating(n)}><Star className={`w-9 h-9 transition-all ${n <= rating ? 'fill-[#059669] text-[#059669] scale-110' : 'text-gray-300'}`} /></button>
        ))}
      </div>
      <textarea value={comment} onChange={(e) => setComment(e.target.value)} rows={3} placeholder={t('visit.comment_ph')} className="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm mb-3 focus:border-[#059669] outline-none resize-none" />
      <input value={name} onChange={(e) => setName(e.target.value)} placeholder={t('visit.your_name_opt')} className="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm mb-4 focus:border-[#059669] outline-none" />
      <button onClick={submit} disabled={busy || rating < 1} className="w-full py-3.5 rounded-2xl bg-[#34D399] text-[#04352A] font-black text-sm flex items-center justify-center gap-2 disabled:opacity-40 shadow-lg shadow-[#059669]/20 active:scale-[0.99] transition-all">
        ...
        {products.map((p: any) => (
          <div key={p.id} className="flex items-center justify-between bg-[#FAFAF7] rounded-xl px-3.5 py-3">
            <div className="flex items-center gap-2.5">
              <div className="w-9 h-9 rounded-lg bg-[#34D399]/10 text-[#059669] grid place-items-center"><ShoppingBag className="w-4 h-4" /></div>
              <div>
            ...
            <p className="font-mono font-black text-[#059669] text-sm">{Number(p.selling_price_egp || 0).toLocaleString('en-US')} {t('visit.egp')}</p>
          </div>
        ))}
      </div>
      <a href={`https://wa.me/${WA}?text=${encodeURIComponent('عايزة أطلب منتج من ' + branch)}`} className="w-full py-3.5 rounded-2xl bg-[#34D399] text-[#04352A] font-black text-sm flex items-center justify-center gap-2 shadow-lg shadow-[#059669]/20">
...
function SuccessCard({ icon: Icon, title, children, onBack }: any) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 p-7 text-center shadow-[0_8px_24px_-14px_rgba(26,46,38,0.25)]">
      <div className="w-16 h-16 rounded-2xl bg-[#34D399]/10 text-[#059669] grid place-items-center mx-auto mb-4"><Icon className="w-8 h-8" /></div>
...
function Chip({ active, onClick, children }: any) {
  return (
    <button onClick={onClick} className={`px-3 py-1.5 rounded-full text-[12px] font-bold border transition-all ${active ? 'bg-[#34D399] text-[#04352A] border-[#059669]' : 'bg-white text-[#1A2E26] border-gray-200'}`}>{children}</button>
  )
}
