        ...
        <div className="max-w-md mx-auto">
          {state.s === 'locating' && (
            <div className="text-center py-14">
              <div className="inline-grid place-items-center w-16 h-16 rounded-2xl bg-[#34D399]/10 text-[#059669] mb-4"><Navigation className="w-8 h-8 animate-pulse" /></div>
          ...
          {state.s === 'found' && (
            <div>
              <div className="bg-white rounded-2xl border border-gray-100 p-6 text-center shadow-sm mb-3">
                <div className="inline-grid place-items-center w-14 h-14 rounded-2xl bg-[#34D399]/10 text-[#059669] mb-3"><MapPin className="w-7 h-7" /></div>
                ...
                <button onClick={() => router.push(`/v/${state.code}`)} className="w-full mt-5 py-3.5 rounded-xl bg-[#34D399] text-[#04352A] font-black text-sm">يلا ادخلي الفرع</button>
              </div>
          ...
          {state.s === 'picker' && (
            <div>
              ...
              <div className="space-y-2">
                {branches.map((b: any) => (
                  <button key={b.code} onClick={() => router.push(`/v/${b.code}`)} className="w-full bg-white border border-gray-100 rounded-xl p-4 flex items-center justify-between text-right active:scale-[0.99] transition-transform hover:border-[#059669]/40">
                    <div>
              ...
              <button onClick={locate} className="w-full mt-4 py-3 rounded-xl bg-[#34D399]/10 text-[#059669] font-black text-sm flex items-center justify-center gap-2"><Navigation className="w-4 h-4" /> جرّبي تحديد الموقع تاني</button>
            </div>
          )}
        </div>
      </main>
