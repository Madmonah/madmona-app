...
const ROLE_COLORS: Record<string, string> = {
  ...
  supplier: 'bg-[#B8860B]/15 text-[#B8860B]',
  customer: 'bg-[#1F5F3F]/10 text-[#1F5F3F]',
}
  ...
  return (
    <div className="bg-white rounded-3xl shadow-soft p-5 md:p-6">
      <div className="flex items-center gap-3 mb-4">
        <div className="w-10 h-10 rounded-2xl bg-[#1F5F3F]/10 flex items-center justify-center flex-shrink-0">
      ...
      {currentPhone && (
        <div className="bg-[#1F5F3F]/5 border border-[#1F5F3F]/20 rounded-2xl p-3 mb-3 flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-white shadow-soft flex items-center justify-center flex-shrink-0">
            <Check className="w-5 h-5 text-[#1F5F3F]" />
          </div>
          <div className="flex-1 min-w-0">
          ...
          <span className="text-[10px] font-black text-[#1F5F3F] bg-white px-2 py-1 rounded-full whitespace-nowrap">
            نشط الآن
          </span>
        </div>
      )}
                ...
                {isRemoving ? (
                  ...
                  <div className="flex items-center gap-1">
                    <button
                      ...
                      className="inline-flex items-center gap-1 bg-[#1F5F3F] hover:bg-[#1F5F3F]/90 text-white text-xs font-bold px-3 py-1.5 rounded-lg disabled:opacity-50 transition-colors"
                    >
                      {isSwitching ? (
                        <>
      ...
      {currentPhone ? (
        <button
          ...
          className="w-full inline-flex items-center justify-center gap-2 px-4 py-3 border-2 border-dashed border-gray-200 hover:border-[#1F5F3F] hover:bg-[#1F5F3F]/5 hover:text-[#1F5F3F] text-gray-600 rounded-2xl text-sm font-bold transition-all"
        >
        ...
        <Link
          href="/auth/login"
          className="w-full inline-flex items-center justify-center gap-2 px-4 py-3 bg-[#1F5F3F] text-white rounded-2xl text-sm font-bold no-underline hover:bg-[#1F5F3F]/90 transition-colors"
        >
          <Plus className="w-4 h-4" />
          إضافة حساب
        </Link>
