          ...
          {(step === 'preview' || step === 'sending') && (
            <div className="space-y-4">
              <p className="text-xs font-bold text-gray-600">
                <FileSpreadsheet className="inline w-4 h-4 text-[#1F6F5F] ml-1" />
                {fileName} — <span className="text-[#1F6F5F]">{valid.length} صنف جاهز</span>
              ...
              <div className="bg-[#FAFAF7] rounded-2xl p-4 space-y-3">
                ...
                <input value={name} onChange={(e) => setName(e.target.value)} placeholder="اسمك *"
                  className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-[#1F6F5F] outline-none text-sm font-bold bg-white" />
                <input value={phone} onChange={(e) => setPhone(e.target.value.replace(/[^\d+]/g, ''))} placeholder="رقم الواتساب * (01xxxxxxxxx)" dir="ltr"
                  className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-[#1F6F5F] outline-none text-sm font-bold bg-white text-left" />
