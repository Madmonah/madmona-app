...
export default function ContactBookSheet({ onClose, onOpenDM }: { onClose: () => void; onOpenDM?: (phone: string) => void }) {
  ...
  const badge: Record<Status, { t: string; bg: string; c: string }> = {
    friend: { t: '🤝 صاحب', bg: '#E4F3EC', c: '#059669' },
  ...
  const pill = { background: '#F1EEE6', color: '#059669', border: 'none', borderRadius: 999, padding: '6px 12px', fontSize: 11.5, fontWeight: 800, cursor: 'pointer', fontFamily: 'inherit' } as const
        ...
        <div onClick={(e) => e.stopPropagation()} dir="rtl" style={{ background: '#fff', width: '100%', height: '88vh', display: 'flex', flexDirection: 'column', borderRadius: '18px 18px 0 0', fontFamily: 'var(--font-cairo), system-ui, sans-serif' }}>
          ...
          <div style={{ padding: '14px 16px 11px', borderBottom: '1px solid #F1EEE6' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginTop: 9 }}>
              ...
              <span style={{ flex: 1 }} />
              <span style={{ fontSize: 12.5, fontWeight: 900, color: '#059669' }}>{nSel} / {picked.length}</span>
            </div>
          </div>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '4px 16px' }}>
            ...
            {view.map(({ p, i }) => (
              <div key={i} onClick={() => setChecked((cc) => ({ ...cc, [i]: !cc[i] }))}
                ...
                <span style={{ width: 22, height: 22, borderRadius: 7, flexShrink: 0, display: 'grid', placeItems: 'center', background: checked[i] ? 'linear-gradient(118deg,#059669,#34D399)' : '#fff', border: checked[i] ? 'none' : '1.5px solid #D4D9D6', color: '#fff', fontSize: 13, fontWeight: 900 }}>{checked[i] ? '✓' : ''}</span>
                <span style={{ width: 34, height: 34, borderRadius: '50%', background: 'radial-gradient(circle at 35% 30%,#2FA084,#059669)', color: '#fff', display: 'grid', placeItems: 'center', fontSize: 14, fontWeight: 800, flexShrink: 0 }}>{(p.name || '').trim()[0] || '؟'}</span>
          ...
          <div style={{ padding: '11px 16px 16px', borderTop: '1px solid #F1EEE6' }}>
            <button onClick={confirmPicked} disabled={busy || nSel === 0}
              style={{ width: '100%', background: 'linear-gradient(118deg,#059669,#34D399)', color: '#fff', border: 'none', borderRadius: 999, padding: '13px 0', fontSize: 14.5, fontWeight: 900, cursor: 'pointer', opacity: busy || nSel === 0 ? 0.5 : 1, fontFamily: 'inherit' }}>
              ➕ ضيف المختارين ({nSel})
            </button>
    ...
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(20,35,30,.55)', zIndex: 98, display: 'flex', alignItems: 'flex-end' }}>
      <div onClick={(e) => e.stopPropagation()} dir="rtl" style={{ background: '#fff', width: '100%', maxHeight: '85vh', overflowY: 'auto', borderRadius: '18px 18px 0 0', padding: 16, fontFamily: "'Cairo', system-ui, sans-serif" }}>
        ...
        <button onClick={inviteFriend} disabled={busy}
          style={{ display: 'flex', alignItems: 'center', gap: 11, width: '100%', background: 'linear-gradient(118deg,#059669,#34D399)', color: '#fff', border: 'none', borderRadius: 16, padding: '14px 16px', cursor: 'pointer', opacity: busy ? 0.6 : 1, fontFamily: 'inherit', textAlign: 'start', marginBottom: 8, boxShadow: '0 6px 18px -8px rgba(250, 129, 37,.5)' }}>
        ...
        <div style={{ display: 'flex', gap: 6, marginBottom: 9 }}>
          <button onClick={addManual} disabled={busy || !phone.trim()} style={{ flex: 1, background: 'linear-gradient(118deg,#059669,#34D399)', color: '#fff', border: 'none', borderRadius: 999, padding: '10px 0', fontSize: 13.5, fontWeight: 900, cursor: 'pointer', opacity: busy || !phone.trim() ? 0.55 : 1, fontFamily: 'inherit' }}>➕ ضيف للدفتر</button>
          <button onClick={importFromPhone} disabled={busy} style={{ background: '#F1EEE6', color: '#059669', border: 'none', borderRadius: 999, padding: '10px 15px', fontSize: 13, fontWeight: 800, cursor: 'pointer', opacity: busy ? 0.55 : 1, fontFamily: 'inherit', whiteSpace: 'nowrap' }}>📲 من تليفوني</button>
        </div>
        ...
        <button onClick={() => vcfRef.current?.click()} disabled={busy}
          style={{ display: 'block', width: '100%', background: '#fff', border: '1.5px dashed #2FA084', color: '#059669', borderRadius: 12, padding: '11px 0', fontSize: 13, fontWeight: 800, cursor: 'pointer', opacity: busy ? 0.55 : 1, fontFamily: 'inherit', marginBottom: 8 }}>
