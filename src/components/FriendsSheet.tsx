    ...
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(20,35,30,.55)', zIndex: 65, display: 'flex', alignItems: 'flex-end' }}>
      <div onClick={(e) => e.stopPropagation()} dir="rtl" style={{ background: '#fff', width: '100%', maxHeight: '78vh', overflowY: 'auto', borderRadius: '18px 18px 0 0', padding: 16, fontFamily: "var(--font-cairo), system-ui, sans-serif" }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 5 }}>
          ...
          {onOpenBook && (
            <button onClick={() => { onClose(); onOpenBook() }} style={{ background: 'linear-gradient(118deg,#059669,#34D399)', color: '#fff', border: 'none', borderRadius: 999, padding: '7px 14px', fontWeight: 800, fontSize: 12.5, cursor: 'pointer', fontFamily: 'inherit' }}>📕 دفتري</button>
          )}
        ...
        {rows.map((r) => (
          <div key={r.friend_id} style={row}>
            ...
            <button onClick={() => onOpenDM(r.friend_phone)} style={{ ...pill, background: '#059669', color: '#fff' }}>💬 كلّمه</button>
...
const ava: React.CSSProperties = { width: 40, height: 40, borderRadius: '50%', background: 'radial-gradient(circle at 35% 30%,#2FA084,#059669)', color: '#fff', display: 'grid', placeItems: 'center', fontSize: 16, fontWeight: 800, flexShrink: 0 }
