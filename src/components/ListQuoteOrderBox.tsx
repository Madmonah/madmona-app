...
export default function ListQuoteOrderBox({
  ...
  const fld = 'w-full px-3 py-2.5 bg-[#FAFAF7] border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-[#1F6F5F]/50 focus:bg-white'
      ...
      <div className="bg-white rounded-3xl shadow-card p-6">
        ...
        {step === 'sent' && (
          <div className="text-center py-4">
            ...
            <button onClick={() => orderId && ref && refresh(orderId, ref)} className="inline-flex items-center gap-2 text-xs font-bold text-[#1F6F5F] hover:underline">
        ...
        {(step === 'quoted' || step === 'paid') && order && (
          <>
            ...
            <div className="space-y-1 text-sm mb-4">
              ...
              <div className="flex justify-between font-black text-[#1F6F5F] text-base pt-1 border-t border-gray-100"><span>الإجمالي</span><span className="tabular">{Number(order.total_amount).toLocaleString('ar-EG')} ج</span></div>
            </div>

            {step === 'quoted' && (
              <button onClick={accept} disabled={busy} className="w-full flex items-center justify-center gap-2 bg-[#1F6F5F] text-white py-3.5 rounded-2xl font-bold text-sm shadow-elevated hover:-translate-y-0.5 transition-all disabled:opacity-50">
            ...
            {step === 'paid' && (
              <div className="bg-[#1F6F5F]/5 border border-[#1F6F5F]/15 rounded-2xl p-4">
                <div className="flex items-center gap-2 mb-2">
                  <CheckCircle className="w-4 h-4 text-[#1F6F5F]" />
                  <p className="text-sm font-black text-[#1F6F5F]">اتأكد الطلب — باقي الدفع</p>
                </div>
  ...
  return (
    <div className="bg-white rounded-3xl shadow-card p-6">
      ...
      <button onClick={addLine} className="inline-flex items-center gap-1.5 text-xs font-bold text-[#1F6F5F] hover:underline mb-4">
      ...
      <button onClick={submit} disabled={busy} className="mt-4 w-full flex items-center justify-center gap-2 bg-[#1F6F5F] text-white py-3.5 rounded-2xl font-bold text-sm shadow-elevated hover:shadow-luxe hover:-translate-y-0.5 transition-all disabled:opacity-50">
