...
export default function MyAssetsCard() {
  ...
  if (data.needs_phone) {
    return (
      <div className="bg-gradient-to-br from-[#2B4521] to-[#2FA084] rounded-3xl shadow-soft p-6 text-white">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <Link
              ...
              className="inline-flex items-center gap-1.5 bg-white text-[#2B4521] font-bold text-xs px-4 py-2.5 rounded-full no-underline hover:bg-white/90 transition-colors"
            >
              أضف رقمي دلوقتي
            </Link>
          </div>
      ...
      <div className="px-6 py-4 space-y-3">
        {meetings.map((m) => (
          <div key={m.id} className="flex items-start gap-3">
            <div className="w-10 h-10 rounded-2xl bg-[#2FA084]/10 text-[#2B4521] flex items-center justify-center shrink-0">
        ...
        >
          <div className="flex gap-2.5 overflow-x-auto pb-1 -mx-1 px-1">
            {data.projects.slice(0, 8).map((p) => (
              ...
              >
                <div className="relative w-32 h-20 rounded-xl overflow-hidden bg-gradient-to-br from-[#2B4521] to-[#2FA084] mb-1.5">
                  {p.cover_url ? (
        ...
        >
          <div className="space-y-1.5">
            {data.businesses.map((b) => (
              ...
              >
                <div className="w-8 h-8 rounded-lg bg-[#2B4521]/10 flex items-center justify-center shrink-0 overflow-hidden">
                  {b.logo_url ? (
                    /* eslint-disable-next-line @next/next/no-img-element */
                    <img src={b.logo_url} alt="" className="w-full h-full object-cover" />
                  ) : (
                    <Store className="w-4 h-4 text-[#2B4521]" />
                  )}
                </div>
                <div className="flex-1 min-w-0">
        ...
        >
          <div className="space-y-1.5">
            ...
            {data.listings.length > 5 && (
              <Link
                href="/supplier/marketplace"
                className="block text-center text-[10px] font-bold text-[#2B4521] py-1.5 no-underline hover:underline"
              >
  ...
  return (
    <div className="px-5 py-4 border-b border-gray-50 last:border-0">
      <div className="flex items-center justify-between mb-2.5">
        ...
        <Link
          href={href}
          className="text-[10px] font-bold text-[#2B4521] no-underline hover:underline"
        >
          تحكّم
        </Link>
      </div>
