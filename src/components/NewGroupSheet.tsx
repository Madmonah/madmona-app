    ...
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(20,35,30,.55)', zIndex: 66, display: 'flex', alignItems: 'flex-end' }}>
      <div onClick={(e) => e.stopPropagation()} dir="rtl" style={{ background: '#fff', width: '100%', maxHeight: '84vh', display: 'flex', flexDirection: 'column', borderRadius: '18px 18px 0 0', padding: 16, fontFamily: 'var(--font-cairo), system-ui, sans-serif' }}>
          ...
          {friends.map((f) => {
            ...
            return (
              <button key={f.friend_id} onClick={() => toggle(f.friend_id)}
                ...
                <span style={{ width: 22, height: 22, borderRadius: 6, border: on ? 'none' : '2px solid #D9D3C4', background: on ? '#FA8125' : 'transparent', color: '#fff', display: 'grid', placeItems: 'center', fontSize: 13, fontWeight: 900, flexShrink: 0 }}>
                  {on ? '✓' : ''}
                </span>
                {f.friend_avatar ? (
                  ...
                  <span style={{ width: 36, height: 36, borderRadius: '50%', background: 'radial-gradient(circle at 35% 30%,#2FA084,#FA8125)', color: '#fff', display: 'grid', placeItems: 'center', fontSize: 15, fontWeight: 800, flexShrink: 0 }}>
        ...
        <button onClick={create} disabled={busy || picked.size === 0 || name.trim().length < 2}
          style={{ marginTop: 12, width: '100%', background: '#FA8125', color: '#fff', border: 'none', borderRadius: 14, padding: '13px', fontWeight: 900, fontSize: 15, cursor: 'pointer', fontFamily: 'inherit', opacity: (busy || picked.size === 0 || name.trim().length < 2) ? .5 : 1 }}>
