...
export default function NewsletterBox({ source = 'newsletter', compact = false }: { source?: string; compact?: boolean }) {
  ...
  if (state === 'done') return <div className="rounded-2xl bg-[#34D399]/15 border border-[#34D399]/40 p-4 text-sm font-bold text-[#04352A]">تمام ✓ أول قصة هتوصلك قريب — ولو ماعجبتكش، لينك الإلغاء في آخر كل رسالة.</div>
  return (
    <form onSubmit={submit} className={`rounded-2xl border border-[#E8E4D8] bg-white ${compact ? 'p-4' : 'p-5'}`} dir="rtl">
      <p className="font-black text-[#1A2E26]">📚 قصة حقيقية من عالم البيزنس — كل أسبوع في إيميلك</p>
