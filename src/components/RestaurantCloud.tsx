...
// 🍽️ أيقونة القسم لما مفيش صورة.
// 🐞 (١٨/٩/٢٠٢٦) محمد: «عايز كل منيوهات المطاعم تبقى زي منيو لمونة بالظبط».
//    التصميم كان **واحد فعلًا** (نفس الكومبوننت لكل المطاعم) — اللي بيفرق هو الكارت:
//    لمونة كل أقسامها ليها إيموچي، والباقي شوكة رمادية `🍽️`. السبب إن الخريطة اتكتبت
//    على مقاس لمونة (مطعم عصائر: كريب · موهيتو · سموذي · زبادو) فأقسام المطاعم المصرية
//    والتركية كلها كانت بتقع على الافتراضي. وفيه ٣ أنماط مكسورة بالحرف:
//    «سندوتش» مابيطابقش `ساندوتش` · «الشوربات» مابيطابقش `شوربة` · «سموزي» مابيطابقش `سموذي`.
// ✅ الخريطة اتبنت من **أسماء الأقسام الحقيقية في الداتابيز** (٧٠ قسم في ٢٩ مطعم منشور)،
//    والمطابقة بقت على نص متطبّع (أ/إ/آ→ا · ة→ه · ى→ي · ال التعريف) فالجمع والمفرد بيتطابقوا.
// ⚠️ الترتيب مقصود: الأخص الأول («كريب الدجاج» يطلع كريب مش دجاج).
const normCat = (s: string) => s.toLowerCase()
  .replace(/[أإآ]/g, 'ا').replace(/ة/g, 'ه').replace(/ى/g, 'ي')
  .replace(/[ً-ْـ]/g, '').replace(/\s+/g, ' ').trim()
const EMOJI: [RegExp, string][] = [
  // أخص حاجة الأول — قبل أي نمط فيه «ساخنة/باردة» عشان مايخطفهاش
  [/مقبلات|مازه|مقبل/, '🫒'],
  // مشروبات باردة وعصائر
  [/crepe|كريب/, '🥞'], [/mojito|موهيتو|موخيتو|كوكتيل/, '🍹'], [/milkshake|ميلك ?شيك|شيك/, '🥛'],
  [/smoothie|سموذي|سموزي|سموثي/, '🥤'], [/energy|طاقه/, '⚡'], [/frappe|فرابيه|فرابي/, '🧋'],
  [/juice|عصير|عصاير|عصائر|فريش/, '🍊'], [/fruit|فواكه|فاكهه/, '🍓'], [/zabado|زبادو|زبادي/, '🍧'],
  [/ice ?cream|ايس ?كريم|ايس/, '🍨'],
  // مشروبات ساخنة
  [/coffee|قهوه|كافيه|اسبريسو/, '☕'], [/tea|شاي/, '🍵'],
  // ⚠️ ممنوع نمط عام زي /ساخنه$/ — «المقبلات الساخنة» كانت بتطلع ☕ بدل 🫒 (اتكشف بلقطة بعد النشر).
  //    لازم يتقيّد بكلمة «مشروب» صراحةً، والأقسام الأخص (مقبلات · سلطات · شوربة) قبله في الترتيب.
  [/hot ?drink|مشروبات ساخنه|مشروب ساخن/, '☕'],
  [/cold ?drink|مشروبات بارده|مشروب بارد/, '🥤'],
  // العام ييجي بعد الأخص — وبعد «مقبلات» اللي في أول القايمة
  [/drink|مشروب/, '🥤'],
  // حلويات
  [/chocolate|شوكولا/, '🍫'], [/sweet|dessert|حلوي|حلو\b|الحلو/, '🍰'], [/bakery|مخبازه|مخبوز|عيش|خبز/, '🥖'],
  // أكل رئيسي
  [/pizza|بيتزا|بيده|بيدا/, '🍕'], [/burger|برجر|برغر/, '🍔'],
  [/منقوش|مناقيش|فطاير|فطير/, '🫓'], [/رقاق|صواني|صينيه/, '🥘'], [/طواجن|طاجن/, '🍲'],
  [/grill|مشوي|مشاوي|مشويات|شوايه|فحم/, '🍖'], [/لحوم|لحمه|كباب|كفته|مضغوط/, '🥩'],
  [/seafood|بحري|سمك|جمبري|اسماك/, '🦐'], [/chicken|فراخ|دجاج/, '🍗'],
  [/salad|سلطه|سلطات/, '🥗'], [/soup|شوربه|شوربات|شربه/, '🍲'],
  [/breakfast|فطار|فطور/, '🍳'], [/بيض/, '🥚'],
  [/طعميه|فلافل/, '🧆'], [/فول/, '🥙'], [/قلابه|طاسه|طاجين/, '🍳'], [/pasta|مكرونه|باستا/, '🍝'],
  [/rice|ارز|رز\b/, '🍚'], [/sandwich|ساندوتش|سندوتش|سندوتشات/, '🥪'],
  [/عربي|شرقي|فلاحي|بلدي/, '🍛'], [/snack|سناك|مقرمشات/, '🍟'],
  [/وجبات|وجبه|كومبو/, '🍱'], [/عرض|عروض|offer|deal/, '🎉'],
  [/جانبيه|اضافات|extra/, '🍟'], [/رئيسي|رئيسيه|main/, '🍛'], [/kids|اطفال/, '🧒'],
]
const catEmoji = (name: string) => {
  const n = normCat(name)
  return (EMOJI.find(([re]) => re.test(n)) || [null, '🍽️'])[1]
}
