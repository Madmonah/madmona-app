    ...
    <form onSubmit={handleSubmit} className="space-y-6 max-w-2xl mx-auto">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-4">
            {form.photo_urls.map((url, idx) => (
              <div key={idx} className="relative group rounded-xl overflow-hidden border border-gray-200 aspect-square">
                ...
                {idx === 0 && (
                  <div className="absolute top-2 right-2 bg-[#B8860B] text-white text-[10px] px-2 py-1 rounded-full font-bold">
                    رئيسية
                  </div>
                )}
        ...
        <label className="block">
          <div className={`flex items-center justify-center gap-2 px-4 py-3 border-2 border-dashed rounded-xl cursor-pointer transition-colors ${
            uploading
              ...
              : 'border-gray-300 bg-[#FAFAF7] text-gray-700 hover:border-[#1F5F3F] hover:bg-[#1F5F3F]/5'
          }`}>
            {uploading ? (
              <>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <select
              ...
              className="w-full px-4 py-3 border border-gray-200 rounded-xl bg-white focus:outline-none focus:ring-2 focus:ring-[#1F5F3F]/30"
            >
      ...
      <div className="flex flex-col sm:flex-row gap-3 sticky bottom-4 bg-[#FAFAF7] py-3 border-t border-gray-200 -mx-4 px-4 sm:relative sm:bottom-auto sm:py-0 sm:border-0 sm:mx-0 sm:px-0">
        <button
          ...
          className="flex-1 flex items-center justify-center gap-2 bg-[#1F5F3F] text-white py-3 rounded-xl font-semibold hover:bg-[#1F5F3F]/90 disabled:opacity-50"
        >
          {submitting ? (
            <Loader2 className="w-4 h-4 animate-spin" />
          ) : (
    ...
    <div>
      ...
      <div className="relative">
        <input
          ...
          placeholder="—"
          className="w-full px-3 py-2.5 pl-12 border border-gray-200 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-[#1F5F3F]/30 focus:border-[#1F5F3F]"
        />
