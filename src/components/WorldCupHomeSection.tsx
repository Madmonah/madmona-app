...
export default function WorldCupHomeSection() {
  ...
  if (!loaded || !data?.ok || (data?.matches?.length ?? 0) === 0) {
    ...
    return (
      <Link
        href="/world-cup"
        className="flex items-center gap-3 bg-gradient-to-l from-[#2B4521] to-[#2FA084] text-white rounded-2xl px-4 py-3.5 shadow-lg hover:-translate-y-0.5 transition-all"
      >
  ...
  return (
    <section className="bg-white rounded-3xl shadow-lg overflow-hidden border border-[#2B4521]/10">
      {/* header */}
      <div className="bg-gradient-to-l from-[#2B4521] to-[#2FA084] text-white px-4 py-3 flex items-center gap-2.5">
        ...
        {rows.map(({ label, m, live }) => {
          ...
          return (
            <Link
              key={m.id}
              href="/world-cup"
              className={`flex items-center gap-2 px-4 py-2.5 hover:bg-gray-50/70 transition ${isEgypt ? 'bg-[#2B4521]/5' : ''}`}
            >
              {/* label */}
              <span className={`w-14 flex-shrink-0 text-[9px] font-black text-center px-1 py-1 rounded-lg leading-tight ${
                live ? 'bg-red-50 text-red-500' : m.status === 'finished' ? 'bg-gray-100 text-gray-400' : 'bg-[#2B4521]/8 text-[#2B4521]'
              }`}>
              ...
              <span className="w-14 flex-shrink-0 text-center">
                {played ? (
                  ...
                  <span className="text-[10px] font-black text-[#2B4521] tabular">{timeStr(m.utc_date)}</span>
                )}
              </span>
      ...
      <Link
        ...
        className="flex items-center gap-2 px-4 py-2.5 bg-[#FAFAF7] hover:bg-[#2B4521]/5 transition border-t border-gray-100"
      >
