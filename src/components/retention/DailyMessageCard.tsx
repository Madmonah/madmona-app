...
const categoryStyles: Record<string, { bg: string; border: string; emoji: string }> = {
  greeting:     { bg: 'bg-[#FAFAF7]', border: 'border-[#2B4521]/30', emoji: '👋' },
        ...
        <div className="flex-1 min-w-0">
          ...
          {message.cta_label && message.cta_url && (
            <Link
              ...
              className="inline-block mt-3 px-4 py-2 rounded-xl bg-[#2B4521] text-white font-semibold text-sm hover:bg-[#2B4521]/90 transition-colors"
            >
              {message.cta_label} ←
            </Link>
          )}
          ...
          <button
            type="button"
            onClick={handleShare}
            className={`inline-flex items-center gap-1.5 ${message.cta_label && message.cta_url ? 'ms-2 mt-3' : 'mt-3'} px-3 py-2 rounded-xl bg-white border border-[#2B4521]/30 text-[#2B4521] font-semibold text-xs hover:bg-[#2B4521]/5 transition-colors`}
