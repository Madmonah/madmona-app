...
${opts.savedMediaUrl ? `\n📎 الملف اللي بعته اتحفظ هنا:\n${opts.savedMediaUrl}\nلو هتسجّل إعلان أو مشروع، مرّر الرابط ده في image_urls كصورة.\n\n🧾 لو الصورة فيها منيو أو قائمة أسعار: اقرا كل صنف وسعره من الصورة نفسها (إنت شايفها) واكتبهم كلهم بالتفصيل. (قاعدة ١٨ أغسطس — بتشيل قاعدة «صنف صنف» القديمة): المطعم/النشاط ده **create_listing_draft واحد** باسمه، والأصناف بأسعارها اللي في الصورة تتكتب كاملة في الـdescription — الفريق بيحوّلها منيو على صفحته. ⛔ مش نداء لكل صنف (ده اللي خلّى رقاق المدق يظهر ٣٤ مرة). الأسعار اللي في الصورة هي المصدر، متخترعش.\n` : ''}
