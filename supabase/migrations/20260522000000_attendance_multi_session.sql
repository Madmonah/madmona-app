-- =====================================================================
-- Multi-session attendance (LIVE-aligned, 2026-05-22).
-- Each employee can clock in/out multiple times per day. Implemented as
-- MULTIPLE rows in attendance_logs per (employee, date): each clock-in
-- inserts a new row, each clock-out closes the latest OPEN row. The
-- attendance_logs UNIQUE(employee_id,date) constraint is intentionally
-- absent to allow this. Includes geofence enforcement, rejected-attempt
-- logging, owner out-of-range alerts, and per-movement WhatsApp messages.
--
-- NOTE: public.attendance_sessions exists in prod from an older migration
-- but is UNUSED by this design (kept harmless/idempotent for rebuilds).
-- This file mirrors the live function/view verbatim so a future deploy
-- never regresses attendance to a sessions-based or single-row version.
-- =====================================================================

-- (kept, unused) sessions table -- idempotent for fresh rebuilds ------
CREATE TABLE IF NOT EXISTS public.attendance_sessions (
  ...
  clock_in_lat numeric, clock_in_lng numeric, clock_in_distance_m numeric,
  clock_out_lat numeric, clock_out_lng numeric, clock_out_distance_m numeric,
  clock_in_method text,
  created_at timestamptz NOT NULL DEFAULT now()
);
ALTER TABLE public.attendance_sessions ADD COLUMN IF NOT EXISTS hours_worked numeric;
CREATE INDEX IF NOT EXISTS idx_att_sessions_emp_date ON public.attendance_sessions(employee_id, date DESC);
CREATE UNIQUE INDEX IF NOT EXISTS uniq_open_session_per_employee ON public.attendance_sessions(employee_id) WHERE clock_out_at IS NULL;

-- Multi-session clock function (multiple attendance_logs rows/day) -----
...
AS $function$
DECLARE
  v_branch RECORD; v_employee RECORD; v_attendance RECORD;
  v_action text; v_hours numeric; v_distance numeric; v_max_radius int;
BEGIN
  SELECT b.id AS branch_id, b.supplier_id, b.name AS branch_name, s.business_name,
         b.latitude, b.longitude, COALESCE(b.geofence_radius_meters,100) AS geofence_radius,
  ...
  IF v_branch.geofence_enabled AND v_branch.latitude IS NOT NULL THEN
    ...
    IF p_accuracy_m IS NOT NULL AND p_accuracy_m < 500 THEN v_max_radius := v_max_radius + p_accuracy_m::int; END IF;
    IF v_distance > v_max_radius THEN
      BEGIN
        ...
        BEGIN
          ...
          VALUES (v_branch.supplier_id,v_branch.branch_id,p_branch_code,v_emp,p_phone_or_pin,v_who,ROUND(v_distance),v_max_radius,p_lat,p_lng,p_accuracy_m);
          ...
          IF v_alerted=0 THEN
            ...
            FROM supplier_admins sa WHERE sa.supplier_id=v_branch.supplier_id AND sa.active=true AND sa.role IN ('owner','manager') AND sa.phone IS NOT NULL;
          END IF;
        END;
      EXCEPTION WHEN OTHERS THEN NULL; END;
      RETURN jsonb_build_object('ok',false,'error','إنت بعيد عن الفرع بـ '||ROUND(v_distance)::text||' متر · لازم تكون قريب من المكان','reason_code','out_of_range','distance_m',ROUND(v_distance,0),'max_radius_m',v_max_radius,'accuracy_m',p_accuracy_m);
    END IF;
  END IF;
  ...
  WHERE supplier_id=v_branch.supplier_id AND status='active'
    AND (pin_code=p_phone_or_pin OR phone=p_phone_or_pin OR phone='+'||p_phone_or_pin) LIMIT 1;
  IF v_employee.id IS NULL THEN RETURN jsonb_build_object('ok',false,'error','الرقم ده مش مسجل في الفرع'); END IF;

  -- MULTI-SESSION: latest OPEN row -> close (clock_out); none -> new row (clock_in)
  SELECT * INTO v_attendance FROM attendance_logs
    WHERE employee_id=v_employee.id AND date=CURRENT_DATE AND clock_out_at IS NULL
  ...
  ELSE
    ...
    v_hours := ROUND(EXTRACT(EPOCH FROM (now()-v_attendance.clock_in_at))/3600.0 - (COALESCE(v_attendance.break_minutes,0)/60.0),2);
    v_action:='clock_out';
  END IF;
...
-- Report view: per-row (per-session) in/out + rejected attempts --------
